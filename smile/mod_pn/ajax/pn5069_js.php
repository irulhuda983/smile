...
<script language="javascript">
		...
		success: function(data)
		{
			...
			if (jdata.dtListSegmen.DATA)
			{
							for($i=0;$i<(jdata.dtListSegmen.DATA.length);$i++)
				{
						...
						//set list status klaim ----------------------------------------------
			if (jdata.dtListStatusKlaim.DATA)
			{
							for($i=0;$i<(jdata.dtListStatusKlaim.DATA.length);$i++)
				{
								//tampilkan semua pilihan ----------------------------
								$("#search_txt2d").append('<option value="'+jdata.dtListStatusKlaim.DATA[$i]['KODE']+'">'+jdata.dtListStatusKlaim.DATA[$i]['KODE']+'</option>');
				}	
			}																										
			}else{
			//gagal --------------------------------------------------------------
	...
	function loadSelectedRecord(v_kode_klaim, fn)
	{
		...
		$.ajax({
            ...
            window.document.getElementById("tabid3").style.display = 'block';
            window.document.getElementById("tabid4").style.display = 'none';
						...
						{
						 	//----------------------- set layout jht -------------------------
							$("#span_page_title").html('PN5030 - DATA KLAIM JAMINAN HARI TUA');						
							$("#span_judul_tab1").html('Informasi Agenda Klaim JHT');
							$("#span_judul_tab2").html('Penetapan Manfaat Biaya dan Santunan');
							$("#span_judul_tab3").html('Pembayaran Klaim');
							
							//set layout informasi klaim -------------------------------------
							$('#formtab1a').load('../ajax/pn5069_view_tabinfoklaim.php?kode_form=PN5069', getValueArr(jdata.data.INFO_KLAIM));
							...
							//set layout pembayaran klaim ------------------------------------				
							$('#formtab3a').load('../ajax/pn5069_view_pembayaran_grid.php', {DATA_BYR : getValueArr(jdata.dataByrMnfLumpsum.DATA)});
							
							//----------------------- end set layout jht ---------------------
						}else if (v_jenis_klaim=="JKP")
						{
						 	//----------------------- set layout jkp -------------------------
							$("#span_page_title").html('PN5030 - DATA KLAIM JAMINAN KEHILANGAN PEKERJAAN');						
							...
							$('#formtab1c').load('../ajax/pn5069_view_profil_aktifitas_jkp.php',getValueArr(jdata.data.INFO_KLAIM));
							$('#formtab1d').load('../ajax/pn5069_view_tabadministrasi.php', {DATA_DOKUMEN : getValueArr(jdata.data.DATA_DOKUMEN)});
							
							//set layout penetapan manfaat -----------------------------------
							$('#formtab2a').load('../ajax/pn5069_view_penetapanmanfaat.php', {DATA_MANFAAT : getValueArr(jdata.dataTapMnfLumpsum.DATA), DATA_PENERIMA : getValueArr(jdata.dataPenerimaMnfLumpsum.DATA), DATA_DOKUMEN_TAMBAHAN : getValueArr(jdata.data.DATA_DOKUMEN_TAMBAHAN),DATA_SUMBER : getValueArr(v_jenis_klaim)});
						...
						{
						 	//------------------------ set layout jht/jkm --------------------
							$("#span_page_title").html('PN5030 - DATA KLAIM JHT/JKM');				
						...
						{
						 	//------------------------ set layout jkk ------------------------
							$("#span_page_title").html('PN5030 - DATA KLAIM JAMINAN KECELAKAAN KERJA');
                ...
                }else
                {
                  ...
                  }else if (v_kode_sebab_klaim == "SKK21")
                  {
                    //pemulangan tki bermasalah ---------------------
						...
						{
						 	//------------------------ set layout jkm ------------------------
							$("#span_page_title").html('PN5030 - DATA KLAIM JAMINAN KEMATIAN');				
              ...
              if (v_kode_segmen=="JAKON")
              {
               	$('#formtab1c').load('../ajax/pn5069_view_jakon_tk.php', getValueArr(jdata.data.INFO_KLAIM));
						...
						{
						 	//------------------------ set layout jpn ------------------------
							$("#span_page_title").html('PN5030 - DATA KLAIM JAMINAN PENSIUN');
            		...
            		window.document.getElementById("tabid3").style.display = 'block';
  							...
  							$("#span_judul_tab3").html('Pembayaran Lumpsum');						
							}else
							{
                ...
                window.document.getElementById("tabid4").style.display = 'block';
								window.document.getElementById("tabid5").style.display = 'block';
								window.document.getElementById("tabid6").style.display = 'block';
													
  							$("#span_judul_tab4").html('Penetapan Manfaat Berkala');
  							$("#span_judul_tab5").html('Konfirmasi Berkala');	
								$("#span_judul_tab6").html('Pembayaran Berkala');							
							}else
							{
                window.document.getElementById("tabid4").style.display = 'none';
								window.document.getElementById("tabid5").style.display = 'none';
							...
							{						
  							...
  							//set layout pembayaran klaim lumpsum --------------------------
  							$('#formtab3a').load('../ajax/pn5069_view_pembayaran_grid.php', {DATA_BYR : getValueArr(jdata.dataByrMnfLumpsum.DATA)});							
							}
							...
							if (v_flag_berkala == "Y")
							{
  							...
  							$('#formtab4a').load('../ajax/pn5069_view_penetapanmanfaat_jpnberkala.php', {DATA_MANFAAT : getValueArr(jdata.dataTapMnfBerkala.DATA), DATA_PENERIMA : getValueArr(jdata.dataPenerimaMnfBerkala.DATA), DATA_DOKUMEN_TAMBAHAN : getValueArr(jdata.data.DATA_DOKUMEN_TAMBAHAN)});						
								
								//set layout konfirmasi jp berkala -----------------------------
								$('#formtab5a').load('../ajax/pn5069_view_jpn_konfirmasiberkala.php', getValueArr(jdata.dataKonfByrBerkala));
								
								//set layout pembayaran jp berkala -----------------------------
								$('#formtab6a').load('../ajax/pn5069_view_jpn_pembayaranberkala.php', getValueArr(jdata.dataKonfByrBerkala));
							}														
    ...
    var order_type 	 = $("#order_type").val();
		...
		var kantor_aktif = '<?=$gs_kantor_aktif;?>';
		...
		//update 30/08/2021 utk menampilkan data Kantor Cabang Lain/Semua Kantor 
		//harus mengisikan No.Ref, Kode Klaim atau parameter pencarian lainya	
		//update 16/09/2021, utk login menggunakan kantor 0 (nasional) maka tidak mandatory
		var v_is_mandatory_searchby = 'T';
		if (kantor_aktif =='0' && kantor_aktif.length == 1)
		{
		 	v_is_mandatory_searchby = 'T'; 
		} else 
		{
		 	v_is_mandatory_searchby = 'Y';
		}
			
		if ((v_is_mandatory_searchby == 'Y') && (kategori == '2' || kategori == '3') && (search_by == '' || search_txt == ''))
		{
			alert('Untuk menampilkan data klaim Kantor Cabang Lain/Semua Kantor, harap mengisikan No.Ref, Kode Klaim atau kriteria pencarian data lainnya..');
		}else 
		{		
  		asyncPreload(true);
			$.ajax({
  				...
  				try {
  					...
  					if (jdata.ret == 1) {
  						...
  						for(var i = 0; i < jdata.data.length; i++){
  							...
  							let ket_tipe_klaim = getValue(jdata.data[i].KET_TIPE_KLAIM)=='JHT JMO' ? 'JHT' : getValue(jdata.data[i].KET_TIPE_KLAIM) ;
  
  							html_data += '<tr>';
  							html_data += '<td style="text-align: left;">' + getValue(jdata.data[i].KODE_KLAIM_DISPLAY) + '</td>';
  							html_data += '<td style="text-align: left;">' + getValue(jdata.data[i].TGL_TRANS) + '</td>';
  							...
  							html_data += '<td style="text-align: left;">' + ket_tipe_klaim + '</td>';
  							html_data += '<td style="text-align: left;">' + getValue(jdata.data[i].TGL_KEJADIAN) + '</td>';
  							...
  							html_data += '<td style="text-align: left;">' + getValue(jdata.data[i].DISPLAY_KODE_KLAIM_PERTAMA) + '</td>';
	...
	function doGridTask(v_editid, v_task, v_tgl1, v_tgl2, v_kategori)
  {
		document.formreg.task.value = v_task;
		document.formreg.editid.value = v_editid;
		document.formreg.kode_klaim.value = v_editid;
		document.formreg.tmp_tglawaldisplay.value = v_tgl1;
		document.formreg.tmp_tglakhirdisplay.value = v_tgl2;
	...
	function fl_js_doBack2Grid() {			 
		...
		v_tgl1 = $("#tmp_tglawaldisplay").val();
		v_tgl2 = $("#tmp_tglakhirdisplay").val();
		v_kategori = $("#tmp_rg_kategori").val();

		//update 30/08/2021, setiap tutup arahkan ke penetapan kantor cabang sendiri
		v_kategori = '1';
				
		window.location.href = '?tglawaldisplay='+v_tgl1+'&tglakhirdisplay='+v_tgl2+'&rg_kategori='+v_kategori+'&mid=<?=$mid;?>';
	}				
	//end custom -----------------------------------------------------------------
...
?>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      $('#tabid'+<?=$ls_activetab;?>).addClass("active");	// menambahkan class active pada link yang diklik
...
<script language="javascript">
	...
	//call switch kode_klaim -----------------------------------------------------
	function fl_js_callform_switch_kodeklaim()
	{
	 	var v_kode_klaim = $("#kode_klaim_display").val();
		var c_mid = '<?=$mid;?>'
		showForm('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5069_view_switchkodeklaim.php?kode_klaim='+v_kode_klaim+'&mid='+c_mid+'','',500,200,'yes'); 
	}
	
	function fl_js_switch_kodeklaim(p_kode_klaim)
	{
    document.formreg.task.value = "view";
    document.formreg.kode_klaim.value = p_kode_klaim;
    try {
    		document.formreg.onsubmit();
    } catch(e){}
    document.formreg.submit();

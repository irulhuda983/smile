...
<script language="javascript">
    ...
    $(window).bind("resize", function(){
		...
		let v_kode_tipe_penerima = $('#kode_tipe_penerima').val();
		let v_kd_prg = $('#kd_prg').val();
		let v_kode_transfer = $('#kode_transfer').val();
		
		fl_js_lumpverif_detilSiapBayar(v_mode_trx, v_kode_klaim, v_kode_tipe_penerima, v_kd_prg, v_kode_transfer, null)
	});
...
<script language="javascript">
  function fl_js_lumpverif_detilSiapBayar(v_mode_trx, v_kode_klaim, v_kode_tipe_penerima, v_kd_prg, v_kode_transfer, fn)
  {					
    var fn;
    if (v_kode_klaim == '' || v_kode_tipe_penerima == '' || v_kd_prg == '' || v_kode_transfer == '') {
    ...
    $.ajax({
      ...
      data: {
        tipe: 'fjq_ajax_val_lumpmonitoring_getpaymentstatus',
      ...
      success: function(data){
        try {
          ...
          {          
						...
						$('#span_page_title').html('STATUS TRANSFER PEMBAYARAN KLAIM - KODE TRANSFER '+v_kode_transfer);
        		...
        		}else
        		{
      			...
      			window.document.getElementById("span_byr_rinci").style.display = 'block';	
						...
						window.parent.Ext.WindowManager.getActive().parent.fl_js_lumpmonitoring_filter();

	...
	<form name="form_1" id="form_1" role="form" method="post">
		<fieldset>
			...
			<div class="form-row_kiri">
				...
				<input type="text" id="v_kode_parameter" name="v_kode_parameter" size="30" maxlength="100" tabindex="3">
				<input type="hidden" id="type" name="type" value="<?=$_REQUEST['task']?>">
			</div>
			<div class="clear"></div>
			<div class="form-row_kiri">
				<label>Nama parameter</label>
				<input type="text" id="v_nama_parameter" name="v_nama_parameter" size="60" maxlength="100" tabindex="3">
			</div>
			<div class="clear"></div>
			<div class="form-row_kiri">
				<label>Jenis Parameter</label>
				<select name="v_jenis_parameter" id="v_jenis_parameter">
			...
			<div class="form-row_kiri">
				...
				<select name="v_jenis_return_parameter" id="v_jenis_return_parameter">
					<option value="">-------pilih-------</option>
...
?>
	<div id="formKiri">
		<fieldset><legend>Data Setup parameter</legend>
			<div class="form-row_kanan">
				Search By&nbsp;
				<select name="f_pilihan" id="f_pilihan">
					<option value="">----------Pilih----------</option>
					<option value="kode_parameter">Kode parameter</option>
					<option value="nama_parameter">Nama parameter</option>
					<option value="jenis_parameter">Jenis Parameter</option>
					<option value="jenis_return_parameter">Jenis Return Parameter</option>
				</select>
			...
			<table class="table table-striped table-bordered row-border hover" id="table_1" cellspacing="0" width="100%">
				<thead>
					<tr>
						...
						<th scope="col" width="10%" class="align-left">Kode parameter</th>
						<th scope="col" width="25%" class="align-left">Nama parameter</th>
						<th scope="col" width="10%" class="align-center">Jenis Parameter</th>
						<th scope="col" width="5%" class="align-center">Jenis Return Parameter</th>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		?>	
			preload(true);
			window.kode_parameter = '<?=$_REQUEST["id"];?>';
			window.type = '<?=$_REQUEST["task"];?>';
			getLovVariable();
			$.ajax({
				type: 'POST',
				url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn1006_action.php?'+Math.random(),
				data: { 
						type : "view",
						keyword : window.kode_parameter
					...
					if(window.type=='view'){
						...
						$('#v_kode_parameter').css('background', '');
						$('a[id*="btn_save"]').hide();
					}else{
						//set readonly for view
						$("#form_1 :input").removeAttr('disabled', 'disabled');
						$('#v_kode_parameter').attr('readonly','readonly');
						$('#v_kode_parameter').attr('title', 'Field ini adalah kunci jadi tidak bisa diupdate');
						$('#v_kode_parameter').css('background', '#dcdada');
						$('a[id*="btn_save"]').show();
					}
					
					$('#v_kode_parameter').val(jdata.KODE_PARAMETER);
					$('#v_nama_parameter').val(jdata.NAMA_PARAMETER);
					$('#v_jenis_parameter option[value="'+jdata.JENIS_PARAMETER+'"]').prop('selected', true);
					$('#v_jenis_return_parameter option[value="'+jdata.JENIS_RETURN_PARAMETER+'"]').prop('selected', true);
		...
		$('#btn_view').click(function() {
			if($('input[name^="CHECK"][type=checkbox]:checked').length>0)
			{
				if($('input[name^="CHECK"][type=checkbox]:checked').length<2){
					window.kode_parameter = $('input[name^="CHECK"][type=checkbox]:checked').attr("kode_parameter");
					window.location='pn1006.php?task=view&id='+window.kode_parameter;
				}else{
		...
		$('#btn_edit').click(function() {
			if($('input[name^="CHECK"][type=checkbox]:checked').length>0)
			{
				if($('input[name^="CHECK"][type=checkbox]:checked').length<2){	
					window.kode_parameter = $('input[name^="CHECK"][type=checkbox]:checked').attr("kode_parameter");
						window.location='pn1006.php?task=edit&id='+window.kode_parameter;
				}else{
		...
		$("#btn_delete").click(function() {
			if($('input[name^="CHECK"][type=checkbox]:checked').length>0)
			{
				if($('input[name^="CHECK"][type=checkbox]:checked').length<2){	
					window.kode_parameter = $('input[name^="CHECK"][type=checkbox]:checked').attr("kode_parameter");
					deleteData(window.kode_parameter);
				}else{
					alert('Untuk melakukan edit hanya bisa dipilih satu record');
				}
			} else {
		...
		$('#btn_new').click(function() {
			window.location='pn1006.php?task=new';
		});
				...
				$.ajax({
					...
					success: function(data) {
						...
						if(jdata.ret == '0'){
							...
							window.location='pn1006.php?task=view&id='+$('#v_kode_parameter').val();
						} else {
...
function loadTable(){
	...
	window.table_1 = $('#table_1').DataTable({
		...
		"destroy"			: true,
        "ajax"				: {
        	"url"	: "http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn1006_action.php",
        	"type": "POST",
        	"data" : function(e) { 
        		e.type = 'query';
	...
	window.table_1.on( 'draw.dt', function () {
		$('input[name*="DETAIL"]').click(function() {
			window.kode_parameter = $(this).attr('kode_parameter');
			NewWindow4('http://<?=$HTTP_HOST;?>/mod_pn/form/pn10011.php?kode_parameter='+window.kode_parameter,'Detail Sebab Klaim Tipe  '+window.kode_parameter,'75%','93%','no');
		});
	});
}

function deleteData(kode_parameter){
	if(confirm("Anda yakin ingin menghapus data ini ? ")){
		...
		$.ajax({
			...
			data: {
				type : "delete",
				v_kode_parameter : kode_parameter
			},
			success: function(data) {
				preload(false);
				jdata = JSON.parse(data);
...
function getLovVariable(){
	$.ajax({
		type: 'POST',
		url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn1006_action.php?'+Math.random(),
		data: {
			type : "lov_jenis_parameter"
		},
		success: function(data) {
			preload(false);
			jdata = JSON.parse(data);
			if(jdata.ret == '0'){
				...
				for(i=0;i<jdata.data.length;i++){
					htmlJenisParameter += "<option value='"+jdata.data[i].KODE+"'>"+jdata.data[i].KODE+"</option>";
				}
				$('#v_jenis_parameter').html(htmlJenisParameter);
			} else {
				alert(jdata.msg);
			}
		}
...
function detailTipeKlaim(kode_parameter){
	win = new window.parent.Ext.window.Window({
          ...
          modal: true,
          autoLoad: {
            url: 'http://<?=$HTTP_HOST;?>/mod_pn/form/pn10011.php?id='+kode_parameter+"&"+Math.random(),
            scripts: true
          },
          dockedItems: [{
            xtype: 'toolbar',

...
Deskripsi:
-----------
File ini dipergunakan untuk pembatalan pembayaran klaim
...
if($trigersubmit=='2')
{
	...
	for($i=0, $max_i=ExtendedFunction::count($cebox); $i<$max_i; $i++) 
	{
		if (${"s".$cebox[$i]} !="")
    {
				if (${"jenis".$cebox[$i]}=="BERKALA")
				{
  				//pembatalan klaim berkala -------------------------------------------
      		$qry = "BEGIN SIJSTK.P_PN_PN5040.X_BATAL_PEMBAYARAN_BERKALA('".${"kode_pembayaran".$cebox[$i]}."',nvl('$username',user),:p_sukses,:p_mess);END;";											 	
          $proc = $DB->parse($qry);				
          ...
          $ls_sukses = $p_sukses;	
      		$ls_mess = $p_mess;	
  				...
  				if ($ls_sukses=="1")
  				{		
  					$msg .= "Pembatalan Pembayaran Berkala untuk kode pembayaran ".${"kode_pembayaran".$cebox[$i]}." berhasil..."."<br>";
  				}else
  				{
  					$ls_error = "1";
  					$msg .= "Pembatalan Pembayaran Berkala untuk kode pembayaran ".${"kode_pembayaran".$cebox[$i]}." gagal..."."<br>";
  				}				
				}else
				{
  				//pembatalan klaim lumpsum -------------------------------------------
      		$qry = "BEGIN SIJSTK.P_PN_PN5040.X_BATAL_PEMBAYARAN('".${"kode_pembayaran".$cebox[$i]}."',nvl('$username',user),:p_sukses,:p_mess);END;";											 	
          $proc = $DB->parse($qry);				
          oci_bind_by_name($proc, ":p_sukses", $p_sukses,32);
      		oci_bind_by_name($proc, ":p_mess", $p_mess,1000);
          $DB->execute();				
          $ls_sukses = $p_sukses;	
      		$ls_mess = $p_mess;	
  				...
  				if ($ls_sukses=="1")
  				{		
  					$msg .= "Pembatalan Pembayaran Lumpsum untuk kode pembayaran ".${"kode_pembayaran".$cebox[$i]}." berhasil..."."<br>";
  				}else
  				{
  					$ls_error = "1";
  					$msg .= "Pembatalan Pembayaran Lumpsum untuk kode pembayaran ".${"kode_pembayaran".$cebox[$i]}." gagal..."."<br>";
  				}
				}		 
		}	
	}
  echo "<script language=\"JavaScript\" type=\"text/javascript\">";
  echo "window.location.replace('?&mid=$mid&msg=$msg&ls_error=$ls_error');";
  echo "</script>";
}
//--------------------- end button action ------------------------------------
...
function batalPembayaran() {
	window.open('../ajax/pn5047_popupbatal.php','batal','width=400,height=100,top=100,left=100,scrollbars=yes')
}		
</script>
<?
...
if (strlen($gs_kantor_aktif)==3) 
{
	...
	$filterkantor_berkala = "and a.kode_kantor = '$gs_kantor_aktif' ";
}else
{
 	...
 	$filterkantor_berkala = "and a.kode_kantor in ".
              						"(	select kode_kantor from sijstk.ms_kantor ".
              						"		start with kode_kantor = '$gs_kantor_aktif' ".
              						"		connect by prior kode_kantor = kode_kantor_induk ".
              						"	) ";									
...
if ($ls_rg_kategori=="2")
{
  //BERKALA --------------------------------------------------------------------
	$sql = 	"select ". 
          "    'BERKALA' jenis, a.kode_pembayaran, a.kode_klaim, a.kode_klaim||'-'||(select to_char(blth_proses,'mmyyyy') from sijstk.pn_klaim_berkala_rekap where kode_klaim = a.kode_klaim and no_konfirmasi=a.no_konfirmasi and no_proses=a.no_proses) klaim_id, ".
					"		 c.kode_penerima_berkala kode_tipe_penerima, a.kode_buku, a.kd_prg, (select nm_prg from sijstk.ms_prg where kd_prg=a.kd_prg) nm_prg, a.kode_kantor kode_kantor_pembayar, a.nom_pembayaran, 'T' flag_spo, ".
          "    (select nama_penerima from sijstk.pn_klaim_penerima_berkala where kode_klaim=a.kode_klaim and kode_penerima_berkala = c.kode_penerima_berkala and rownum=1) nama_penerima, ".
          "    a.kode_kantor,b.kpj, to_char(a.tgl_pembayaran,'dd/mm/yyyy') tgl_pembayaran, ".
          "    decode (nvl(b.kode_pointer_asal, 'x'), 'PROMOTIF', b.nama_pelaksana_kegiatan,(decode(b.kode_segmen,'JAKON', b.nama_tk, b.nama_tk))) nama_pengambil_klaim, ".
          "    d.nama_tipe_klaim||' '|| b.kode_pointer_asal ket_tipe_klaim, ".
          "    b.kode_segmen, b.kode_kantor_tk, b.status_klaim, b.kode_tipe_klaim, b.kode_sebab_klaim, b.kode_pointer_asal, b.id_pointer_asal  ".
          "from sijstk.pn_klaim_pembayaran_berkala a, sijstk.pn_klaim b, sijstk.pn_klaim_berkala c, sijstk.pn_kode_tipe_klaim d ".
          "where a.kode_klaim = b.kode_klaim and a.kode_klaim = c.kode_klaim and a.no_konfirmasi = c.no_konfirmasi ".
          "and nvl(a.status_batal,'T')='T' and b.kode_tipe_klaim = d.kode_tipe_klaim ".
          "and a.tgl_pembayaran between trunc(to_date('$ld_tgl_trans1_display','dd/mm/yyyy'),'dd') and trunc(to_date('$ld_tgl_trans2_display','dd/mm/yyyy'),'dd') ".
  				$filterkantor_berkala.
          $filtersearch;
}else
{
 	//LUMPSUM --------------------------------------------------------------------
  $sql = 	"select ". 
          "    'LUMPSUM' jenis, a.kode_pembayaran, a.kode_klaim, a.kode_klaim klaim_id, a.kode_tipe_penerima, a.kode_buku, a.kd_prg, (select nm_prg from sijstk.ms_prg where kd_prg=a.kd_prg) nm_prg, a.kode_kantor_pembayar, a.nom_pembayaran, nvl(a.flag_spo,'T') flag_spo, ".
          ...
          "    d.nama_tipe_klaim||' '|| b.kode_pointer_asal ket_tipe_klaim, ".
          "    b.kode_segmen, b.kode_kantor_tk, b.status_klaim, b.kode_tipe_klaim, b.kode_sebab_klaim, b.kode_pointer_asal, b.id_pointer_asal  ".
          "from sijstk.pn_klaim_pembayaran a, sijstk.pn_klaim b, sijstk.pn_kode_tipe_klaim d ".
          "where a.kode_klaim = b.kode_klaim and b.kode_tipe_klaim = d.kode_tipe_klaim ".
...
$othervar		= "&mid=".$mid."&pilihsearch=".$pilihsearch."&searchtxt=".$searchtxt."&f=".$_GET['f']."&o=".$o."&rg_kategori=".$ls_rg_kategori."&tgl_trans1_display=".$ld_tgl_trans1_display."&tgl_trans2_display=".$ld_tgl_trans2_display;
...
<table class="captionform">
  ...
  <tr>
      ...
      ?>
      <input type="radio" name="rg_kategori" value="1" onclick="this.form.submit()"  <?=$sel1;?>>&nbsp;<font color="#ffffff" size="2px;" family="Verdana">LUMPSUM</font>&nbsp;&nbsp;&nbsp;&nbsp;
      <input type="radio" name="rg_kategori" value="2" onclick="this.form.submit()"  <?=$sel2;?>>&nbsp;<font color="#ffffff" size="2px;" family="Verdana">BERKALA</font>
			</h3>    
		</td>
  </tr>
...
<?
$sortvar    = "&mid=".$mid."&pilihsearch=".$pilihsearch."&searchtxt=".$searchtxt."&rg_kategori=".$ls_rg_kategori."&tgl_trans1_display=".$ld_tgl_trans1_display."&tgl_trans2_display=".$ld_tgl_trans2_display;
$o = $o=='ASC' ? 'DESC' : 'ASC';		
echo "<table  id=mydata cellspacing=0>";
echo "<tr>";
echo "<th class=awal>&nbsp;<input type=checkbox name=toggle value=\"\" onclick=\"checkAll(".$jmlrow."); \" /></th>";
echo "<th style=width:120px;>&nbsp;<a href=\"$PHP_SELF?f=a.kode_pembayaran&o=$o$sortvar\"><b>Kode Byr</b></a></th>";
echo "<th style=width:120px;>&nbsp;<a href=\"$PHP_SELF?f=a.kode_klaim&o=$o$sortvar\"><b>Kode Klaim</b></a></th>";
echo "<th>&nbsp;<a href=\"$PHP_SELF?f=a.tgl_pembayaran&o=$o$sortvar\"><b>Tgl Byr</b></a></th>";
...
echo "<th>&nbsp;<a href=\"$PHP_SELF?f=b.kpj&o=$o$sortvar\"><b>No. Ref</b></a></th>";
echo "<th>&nbsp;<a href=\"$PHP_SELF?f=b.kpj&o=$o$sortvar\"><b>Nama</b></a></th>";
echo "<th>&nbsp;<a href=\"$PHP_SELF?f=b.kpj&o=$o$sortvar\"><b>Penerima</b></a></th>";
...
{	    	
	...
	echo "<input type=hidden id=kode_klaim".$i." name=kode_klaim".$row["KODE_PEMBAYARAN"]." value=".$row["KODE_KLAIM"].">";	
	echo "<input type=hidden id=kode_pembayaran".$i." name=kode_pembayaran".$row["KODE_PEMBAYARAN"]." value=".$row["KODE_PEMBAYARAN"].">";
	echo "<input type=hidden id=jenis".$i." name=jenis".$row["KODE_PEMBAYARAN"]." value=".$row["JENIS"].">";	
	echo "<input type=checkbox id=cb".$i." name=s".$row["KODE_PEMBAYARAN"]." value=".$row["KODE_PEMBAYARAN"]." onclick=\"isChecked(this.checked);\"></td>";
	if ($row["JENIS"]=="LUMPSUM")
	{
	  echo "<td>&nbsp;<a href=# onclick=\"NewWindow('../ajax/pn5004_pembayaran_entry.php?task=view&kode_pembayaran=".$row["KODE_PEMBAYARAN"]."&mid=$ls_mid&sender=pn5047.php','info',900,600,'yes')\">".$row["KODE_PEMBAYARAN"]."</a></td>";
	}else
	{
	 	echo "<td>&nbsp;<a href=# onclick=\"NewWindow('../ajax/pn5048_view_pembayaran.php?task=View&kode_klaim=".$row["KODE_KLAIM"]."&dataid=".$row["KODE_KLAIM"]."&mid=$ls_mid&sender=pn5047.php&activetab=1','info',1045,600,'yes')\">".$row["KODE_PEMBAYARAN"]."</a></td>";	 
	}
	echo "<td>&nbsp;<a href=# onclick=\"NewWindow('../ajax/pn5048_view_pembayaran.php?task=View&kode_klaim=".$row["KODE_KLAIM"]."&dataid=".$row["KODE_KLAIM"]."&mid=$ls_mid&sender=pn5047.php&activetab=1','info',1045,600,'yes')\">".$row["KLAIM_ID"]."</a></td>";
...
if ($ln_cnt >= "1")
{
  ?>
	<div>
			 <input type="button" class="btn green" name="batalpembayaran" onclick="batalPembayaran();" value="      BATAL PEMBAYARAN KLAIM      ">
	</div>
...
}else
{
  ?>
  <div>
			 <input type="button" class="btn green" name="batalpembayaran" onclick="alert('Anda tidak memiliki wewenang untuk melakukan Pembatalan Pembayaran Klaim...!!!');" value="      BATAL PEMBAYARAN KLAIM      ">
	</div>
  <?
}
?>

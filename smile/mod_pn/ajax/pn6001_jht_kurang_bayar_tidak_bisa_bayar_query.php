...
if($TYPE=='l')
{
  ...
  $order .= $by;
	...
	$sql = "SELECT A.* FROM (SELECT ROWNUM
  no,
a.kode_klaim,
TO_CHAR (A.tgl_klaim, 'dd/mm/yyyy')
  tgl_klaim,
A.kpj,
A.nama_tk
  AS nama_pengambil_klaim,
...
A.kantor_penetapan
  kode_kantor,
A.nom_kurang_bayar  
FROM mis.mis_oprkeu_krgbyr@to_olap_ro A
WHERE A.kantor_penetapan = '$KD_KANTOR'{$condition}
AND A.blth_laporan=trunc(sysdate,'MM')
AND NOT EXISTS ( SELECT NULL FROM PN.PN_KURANG_BAYAR_TDL B WHERE B.KODE_KLAIM=A.KODE_KLAIM AND B.STATUS_BATAL='T') 
{$order}) A WHERE 1=1 AND NO BETWEEN ".$start." and ".$end." ";
  			
	$queryTotalRows = "select count(1) from (select rownum
  no,
a.kode_klaim,
...
a.kantor_penetapan
  kode_kantor,
  a.nom_kurang_bayar
from mis.mis_oprkeu_krgbyr@to_olap_ro a where a.kantor_penetapan='$KD_KANTOR' and a.blth_laporan=trunc(sysdate,'MM')
AND NOT EXISTS ( SELECT NULL FROM PN.PN_KURANG_BAYAR_TDL B WHERE B.KODE_KLAIM=A.KODE_KLAIM AND B.STATUS_BATAL='T')
)";
  //echo $queryTotalRows;
...
}else if($TYPE=='p')
{
    ...
    (select nama_kantor from ms.ms_kantor where kode_kantor=a.kode_kantor) nama_kantor,
      ...
      a.jenis_identitas, a. kode_pointer_asal,
      (select b.kode_tipe_penerima from pn.pn_klaim_penerima_manfaat b where b.kode_klaim = a.kode_klaim) kode_tipe_penerima,
      (select b.nama_penerima from pn.pn_klaim_penerima_manfaat b where b.kode_klaim = a.kode_klaim) nama_penerima,
      (select b.bank_penerima from pn.pn_klaim_penerima_manfaat b where b.kode_klaim = a.kode_klaim) bank_penerima,
      (select b.no_rekening_penerima from pn.pn_klaim_penerima_manfaat b where b.kode_klaim=a.kode_klaim) no_rekening_penerima,
      (select b.nama_rekening_penerima from pn.pn_klaim_penerima_manfaat b where b.kode_klaim=a.kode_klaim) nama_rekening_penerima,
      (select b.nom_manfaat_utama from pn.pn_klaim_penerima_manfaat b where b.kode_klaim=a.kode_klaim) nom_manfaat_utama,
      (select b.nom_manfaat_tambahan from pn.pn_klaim_penerima_manfaat b where b.kode_klaim=a.kode_klaim) nom_manfaat_tambahan,
      (select b.nom_manfaat_gross from pn.pn_klaim_penerima_manfaat b where b.kode_klaim=a.kode_klaim) nom_manfaat_gross,
      (select b.nom_manfaat_netto from pn.pn_klaim_penerima_manfaat b where b.kode_klaim=a.kode_klaim) nom_manfaat_netto,
      (select b.kode_bank_pembayar from pn.pn_klaim_penerima_manfaat b where b.kode_klaim=a.kode_klaim) kode_bank_pembayar,
      (select tarif_bunga_saldo from kn.kn_tarif_bunga 
                          where jenis='K' and kd_prg = '1'
                          and to_char(sysdate,'yyyymmdd') between to_char(tgl_efektif,'yyyymmdd') and to_char(tgl_akhir,'yyyymmdd')
                          and rownum = 1) nom_tingkat_pengembangan,
      (select tarif_bunga_iuran from kn.kn_tarif_bunga 
                          where jenis='K' and kd_prg = '1'
                          and to_char(sysdate,'yyyymmdd') between to_char(tgl_efektif,'yyyymmdd') and to_char(tgl_akhir,'yyyymmdd')
                          and rownum = 1) nom_tingkat_pengembangan_kor,
  (select nom_kurang_bayar
  from mis.mis_oprkeu_krgbyr@to_olap_ro b
 where b.kode_klaim = a.kode_klaim
       and blth_laporan = (select max (blth_laporan)
                             from mis.mis_oprkeu_krgbyr@to_olap_ro c
                            where c.kode_klaim = b.kode_klaim)) nom_kurang_bayar
...
if($resultBank->ret == "0"){ 
  ...
  //   for($i = 0; $i < $resultBank->numrows; $i++){
  //     $trfJenis = false;
  //     for($j = 0; $j < count($resultBank->data[$i]->METODE_TRANSFER);$j++){

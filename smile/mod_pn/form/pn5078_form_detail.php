`<?php
    ...
    $formUtama = "/mod_pn/form/pn5078.php"; # inisialisasikan lokasi file utama
    ...
    function encrypt_decrypt_pn5078_detail($action, $string)
    {
        /* =================================================
     * ENCRYPTION-DECRYPTION
     * =================================================
     * ENCRYPTION: encrypt_decrypt_pn5078_detail('encrypt', $string);
     * DECRYPTION: encrypt_decrypt_pn5078_detail('decrypt', $string) ;
     */
        $output_pn5078_detail = false;
        $encrypt_method_pn5078_detail = "AES-256-CBC";
        $secret_key_pn5078_detail = 'WS-SERVICE-KEY';
        $secret_iv_pn5078_detail = 'WS-SERVICE-VALUE';
        // hash
        $key = hash('sha256', $secret_key_pn5078_detail);
        // iv - encrypt method AES-256-CBC expects 16 bytes - else you will get a warning
        $iv = substr(hash('sha256', $secret_iv_pn5078_detail), 0, 16);
        if ($action == 'encrypt') {
            $output_pn5078_detail = base64_encode(openssl_encrypt($string, $encrypt_method_pn5078_detail, $key, 0, $iv));
        } else {
            if ($action == 'decrypt') {
                $output_pn5078_detail = openssl_decrypt(base64_decode($string), $encrypt_method_pn5078_detail, $key, 0, $iv);
            }
        }
        return $output_pn5078_detail;
    }

    $kode_fiktif_klaim  = htmlspecialchars(checkParam('kode_fiktif_klaim'));
    $status_proses        = (htmlspecialchars(checkParam('status_proses'))  == "") ? 'sudah' : htmlspecialchars(checkParam('status_proses'));
    $status_proses_pop_up        = (htmlspecialchars(checkParam('status_proses'))  == "" ? true : false);
    $search_by            = htmlspecialchars(checkParam('search_by'));
    $search_txt            = htmlspecialchars(checkParam('search_txt'));
    $gs_pagetitle         = "PN5078 - Approval Potensi Klaim Fiktif - " . ($status_proses == 'belum' ? 'Belum Diproses' : 'Sudah Diproses');
    $disabled           = 'disabled';
    $disabledApproval   = ($status_proses == 'sudah' ? 'disabled' : 'required');
...
<script language="javascript">
    $(document).ready(function() {
        ...
        $('#formSubmit').submit(function(e) {
            e.preventDefault();
            var status_persetujuan = $('#status_persetujuan').find(':selected').val();
            if (status_persetujuan == '') {
                alert('Status Persetujuan tidak boleh kosong');
                return;
            }
            var keterangan_persetujuan = $('#keterangan_persetujuan').val();
            if (keterangan_persetujuan == '') {
                alert('Keterangan Persetujuan tidak boleh kosong');
                return;
            }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var url = 'http://<?= $HTTP_HOST; ?><?= $formDetail ?>?kode_fiktif_klaim=' + kode_fiktif_klaim + '&status_proses=sudah&search_by=&search_txt=';
    ...
    function filter() {
        ...
        var url = "<?= $formAjax ?>" + Math.random()
        var kode_fiktif_klaim = "<?= encrypt_decrypt_pn5078_detail('encrypt', $kode_fiktif_klaim) ?>"
        var status_proses = "<?= $status_proses ?>"
        var status_proses_pop_up = "<?= $status_proses_pop_up ?>"
        $.ajax({
            type: 'POST',
            url: url,
            data: {
                ...
                status_proses_pop_up: status_proses_pop_up,
    ...
    function setData(jdata) {
        ...
        if (jdata.ret == 0) {
            ...
            if ("<?= $disabledApproval ?>" == "disabled") {
                $('#status_persetujuan').val(row.STATUS_APPROVAL)
                ...
                $('#tgl_persetujuan').val("<?= date("d-m-Y") ?>")
                $('#status_persetujuan').val("");
                $('#petugas_persetujuan').val('<?= $USER ?>')
                $('#nama_petugas_persetujuan').val('<?= $NAMA ?>')
            }
            ...
            $('#petugas_verifikasi').val('<?= $USER ?>')
            $('#nama_petugas_verifikasi').val('<?= $NAMA ?>')
            $('#tgl_sla_perpanjangan_verifikasi_kasus').val(row.TGL_SLA2_VERIFIKASI_KASUS)
            $('#tgl_sla_verifikasi_kasus').val(row.TGL_SLA1_VERIFIKASI_KASUS)
    ...
    function newWindow(no_antrian, kode_antrian, jenis_antrian = null, kode_tipe_penerima = null) {
        var url = "http://<?= $HTTP_HOST ?>/mod_il/ajax/il1001_detil.php?kode_antrian=" + no_antrian + "&sender=PN5078";
        if (kode_antrian.includes('EC5013')) {
            url = "http://<?= $HTTP_HOST ?>/mod_ec/form/ec5013_detail.php?kode_booking=" + no_antrian + "&sender=PN5078";
        } else if (kode_antrian.includes('EC5095')) {
            url = "http://<?= $HTTP_HOST ?>/mod_ec/ajax/ec5095_detail.php?kode_booking=" + no_antrian + "&status_piloting=Y&jenis_antrian=" + jenis_antrian + "&kode_tipe_penerima=" + kode_tipe_penerima + "&sender=PN5078";
        }
                    ...
                    <div id="konten" style="width: 100%;">
                        <form id="formSubmit">
                            <fieldset>
                                ...
                                <div class="div-row">
                                    <div class="div-col" style="width:49%; max-height: 100%;">
                                        ...
                                        <div class="r_frm">
                                            <input type="text" id="petugas_pengajuan" name="petugas_pengajuan" style="width: 80px;" disabled>
                                            <input type="text" id="nama_petugas" name="nama_petugas" style="width: 211px;" disabled>
                                        </div>
                                        <div class="div-row-clear"></div>
                                        <div class="l_frm" style="width:220px"><label for="kantor_pengajuan">Kantor Pengajuan</label></div>
                                        <div class="r_frm">
                                            <input type="text" id="kantor_pengajuan" name="kantor_pengajuan" style="width: 50px;" disabled>
                                            <input type="text" id="nama_kantor" name="nama_kantor" style="width: 241px;" disabled>
                                        </div>
                                        <div class="div-row-clear"></div>
                            ...
                            <fieldset>
                                ...
                                <div class="row">
                                    <div class="column">
                                        ...
                                        <div class="r_frm row">
                                            <select name="pengecekan_kasus" id="pengecekan_kasus" style="width: 305px;" <?= $disabled ?> onchange="submitActive()">
                                        ...
                                        <div class="r_frm">
                                            <select name="hasil_verifikasi_kasus" id="hasil_verifikasi_kasus" style="width: 305px;" <?= $disabled ?> onchange="submitActive()">
                                            </select>
                                        </div>
                                        <div class="div-row-clear"></div>
                                        ...
                                        <div class="r_frm row">
                                            <textarea onkeyup="submitActive()" name="keterangan_verifikasi_kasus" id="keterangan_verifikasi_kasus" style="width: 300px;" rows="5" <?= $disabled ?>></textarea>
                                        </div>
                                        <div class="div-row-clear"></div>
                                    </div>
                                </div>
                            ...
                            <fieldset>
                                ...
                                <div class="row">
                                    <div class="column">
                                        <div class="l_frm" style="width:220px"><label for="kode_bank_penerima">Status Persetujuan <?= $disabledApproval == 'required' ? '<sup style="color:red">*</sup>' : NULL ?></label></div>
                                        <div class="r_frm row">
                                            <!-- <input type="text" class="input-disabled" name="status_persetujuan" id="status_persetujuan" style="width: 300px;" disabled> -->
                                            <select name="status_persetujuan" id="status_persetujuan" style="width: 305px;" <?= ($status_proses == 'sudah' ? 'disabled' : '') ?>>
                                                <option value="" selected disabled>-- Pilih --</option>
                                        ...
                                        <div class="div-row-clear"></div>
                                        <div class="l_frm" style="width:220px"><label for="keterangan_persetujuan">Keterangan Persetujuan <?= $disabledApproval == 'required' ? '<sup style="color:red">*</sup>' : NULL ?></label></div>
                                        <div class="r_frm row">
                                            <textarea name="keterangan_persetujuan" id="keterangan_persetujuan" style="width: 300px;" rows="5" <?= ($status_proses == 'sudah' ? 'disabled' : '') ?>></textarea>
                                        </div>
                                        <div class="div-row-clear"></div>
                                        <div class="l_frm" style="width:220px"></div>
                                        <?php if (htmlspecialchars(checkParam('status_proses'))  !== "") { ?>
                                            <?php if ($disabledApproval == 'disabled') { ?>
                                                <div class="r_frm">
                                            ...
                                            <?php } else { ?>
                                                <div class="div-row-clear"></div>
                                                <div class="l_frm" style="width:220px"><label style="text-align:right;">&nbsp;</label></div>
                                                <div class="r_frm">
                                                    <input type="checkbox" id="setuju" name="setuju" value="Y" onclick="submitActive()">
                                                ...
                                                <div class="l_frm" style="width:220px"><label style="text-align:right;">&nbsp;</label></div>
                                                <div class="r_frm">
                                                    <input type="hidden" readonly id="id" name="id" value="<?= encrypt_decrypt_pn5078_detail('encrypt', $kode_fiktif_klaim) ?>">
                                                    <input type="submit" name="btn_submit" style="width: 100px" id="btn_submit" value="SUBMIT" onclick="submitData()" />
                                                    <input type="button" name="btn_kembali" style="width: 100px" class="btn green" id="btn_kembali" value="TUTUP" onclick="kembali()" />
                                                </div>
                                            <?php } ?>
                                        <?php } ?>

...
$ls_nama_kondisi_terakhir	= $_POST['v_nama_kondisi_terakhir'];
...
if($ls_type=="new"){
	$sql = "insert into sijstk.pn_kode_kondisi_terakhir (
	...
	$sql = "update sijstk.pn_kode_kondisi_terakhir
			   set nama_kondisi_terakhir = '$ls_nama_kondisi_terakhir',
			       no_urut = '$ls_no_urut',
			 ...
			 where kode_kondisi_terakhir = '$ls_kode_kondisi_terakhir'";
	$DB->parse($sql);
	if($DB->execute()){ 
	...
	$sql="select kode_kondisi_terakhir,
		       nama_kondisi_terakhir,
		       no_urut,
		       status_nonaktif,
		       to_char(tgl_nonaktif,'dd/mm/yyyy') tgl_nonaktif,
		       keterangan
		  from sijstk.pn_kode_kondisi_terakhir
		 where kode_kondisi_terakhir='".$ls_search_keyword."'";
    $DB->parse($sql);
    if($DB->execute()){ 
	...
	$sql="delete from sijstk.pn_kode_kondisi_terakhir
      		where kode_kondisi_terakhir ='".$ls_kode_kondisi_terakhir."'";
    $DB->parse($sql);
	if($DB->execute()){ 
	...
	if($order[0]['column']=='0'){
	    ...
	    $order = "ORDER BY NAMA_KONDISI_TERAKHIR ";
	...
	$sql = "";
	$sql="select * from (select rownum no, kode_kondisi_terakhir,
		       nama_kondisi_terakhir,
		       status_nonaktif,
		       tgl_nonaktif,
		       keterangan
	...
	$queryTotal = "SELECT COUNT (1) FROM SIJSTK.PN_KODE_KONDISI_TERAKHIR WHERE 1=1  ".$condition;
	// print_r($sql);
	// print_r($querySql);
	...
	if($DB->execute()){ 
		...
		{
			$data['ACTION'] = '<button class="btn green" name="view_kondisi_terakhir_'.$data['KODE_KONDISI_TERAKHIR'].'" id="view_kondisi_terakhir_'.$data['KODE_KONDISI_TERAKHIR'].'" kode_kondisi_terakhir="'.$data['KODE_KONDISI_TERAKHIR'].'"  title="Klik untuk view detil data"><i class="fa fa-search" aria-hidden="true"></i></button>'.' '.' <button class="btn green" name="edit_kondisi_terakhir_'.$data['KODE_KONDISI_TERAKHIR'].'" id="edit_kondisi_terakhir_'.$data['KODE_KONDISI_TERAKHIR'].'" kode_kondisi_terakhir="'.$data['KODE_KONDISI_TERAKHIR'].'"  title="Klik untuk edit data"><i class="fa fa-pencil" aria-hidden="true"></i></button> <button class="btn hapus" name="hapus_kondisi_terakhir_'.$data['KODE_KONDISI_TERAKHIR'].'" kode_kondisi_terakhir="'.$data['KODE_KONDISI_TERAKHIR'].'" title="Klik untuk hapus data"><i class="fa fa-trash" aria-hidden="true"></i></button>
				';

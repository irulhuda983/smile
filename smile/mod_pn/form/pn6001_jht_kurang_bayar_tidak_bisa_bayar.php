<?php 
$leaf_name="pn6001_jht_kurang_bayar_tidak_bisa_bayar";
$php_file_name="pn6001_jht_kurang_bayar_tidak_bisa_bayar";
...
if($_REQUEST['kd_agenda']!='')
{
  ...
  // Data JHT Kurang Bayar
    $sql_data ="select * from pn.pn_kurang_bayar_data where kode_agenda_tindaklanjut='{$_REQUEST['kd_agenda']}'"; 
    $DB->parse($sql_data);
    if($DB->execute())
        if($row = $DB->nextrow())
        {
  ...
  // Alasan tidak bisa dibayarkan
    ...
    (select url_path_dok_signed from pn.pn_arsip_dokumen b where a.kode_klaim=b.id_dokumen and kode_jenis_dokumen='JD106') path_url_digital_arsip
...
<fieldset ><legend>Tindaklanjut JHT Kurang Bayar Yang Tidak Dapat Dibayarkan</legend>
...
<input type="hidden" style="width: 290px;" value="" <?=$readonly;?> id="nom_tingkat_pengembangan_kor" name="nom_tingkat_pengembangan_kor" readonly />
    ...
    <fieldset><legend>Pemilihan Klaim</legend>
        ...
        <div class="f_2" style="width: 430px !Important">
            ...
            <a href="#" onclick="NewWindow('pn6001_jht_kurang_bayar_tidak_bisa_bayar_lov.php','pn6001_jht_kurang_bayar',1070,505,1)">
    ...
    <fieldset><legend>Informasi Klaim</legend>
        ...
        <div class="f_1">Nominal Manfaat &nbsp;:</div>
        <div class="f_2"><input type="text" style="background-color: #F5F5F5; width: 290px;" readonly value="" id="nom_manfaat_netto" /></div>
        <div class="f_1">Nominal Kurang Bayar &nbsp;:</div>
        <div class="f_2"><input type="text" style="background-color: #F5F5F5; width: 290px;" readonly value="" id="nom_kurang_bayar" /></div>
    </fieldset>
     ...
     <fieldset style="min-width:1000px;"><legend>Tindak lanjut jaminan kurang bayar yang tidak dapat dibayarkan</legend>
            ...
            <input type="hidden" style="background-color:  #F5F5F5; width: 10px;" readonly id="path_url" name="path_url">
            <input type="hidden" style="background-color:  #F5F5F5; width: 10px;" readonly id="mime_type" name="mime_type">
  ...
  <!-- <fieldset><legend>Penetapan JHT Kurang Bayar</legend>
	...
	$sql_submit = "
			...
			FROM PN.PN_KURANG_BAYAR_TDL
			WHERE KODE_AGENDA = '".$_REQUEST['kd_agenda']."'
			AND NVL(STATUS_SUBMIT_TINDAK_LANJUT,'Y') = 'Y'
	...
	$sql_approve = "
			...
			FROM PN.PN_KURANG_BAYAR_TDL
			WHERE KODE_AGENDA = '".$_REQUEST['kd_agenda']."'
			AND NVL(STATUS_TINDAK_LANJUT,'Y') = 'Y'
      AND NVL(STATUS_SUBMIT_TINDAK_LANJUT,'Y') = 'Y'
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if(jdata.INFORMASI_KLAIM)
            {
                ...
                let nom_kurang_bayar = format_uang(jdata.INFORMASI_KLAIM.NOM_KURANG_BAYAR);
                let nom_manfaat_netto = format_uang(jdata.INFORMASI_KLAIM.NOM_MANFAAT_NETTO)
                $("#nom_kurang_bayar").val(nom_kurang_bayar);
                $("#nom_manfaat_netto").val(nom_manfaat_netto);

                let kode_tipe_penerima = jdata.INFORMASI_KLAIM.KODE_TIPE_PENERIMA;
                let kode_kantor_pembayar = jdata.INFORMASI_KLAIM.KODE_KANTOR;
                let kode_klaim = jdata.INFORMASI_KLAIM.KODE_KLAIM;
                let kode_pointer_asal = jdata.INFORMASI_KLAIM.KODE_POINTER_ASAL;

                lovKodeBank(kode_tipe_penerima, kode_kantor_pembayar, kode_klaim, kode_pointer_asal);
...
function lovKodeBank(kode_tipe_penerima, kode_kantor_pembayar, kode_klaim, kode_pointer_asal){
...
$(document).ready(function(){ 
    ...
    let kode_klaim                  = "<?php echo $ls_klaim ?>";
    ...
    if(ls_jml_approve_koreksi_data=="0"){
      ...
      let params = 'kode_klaim=' + kode_klaim+'&kode_agenda='+kode_agenda + '&user='+user + '&report=kcp&ACTION_TYPE=CETAK_BERITA_ACARA';   
      window.open('http://<?=$HTTP_HOST;?>/mod_pn/ajax/<?=$php_file_name;?>_action.php?' + params);
    
        
      }else{
        let params = 'kode_klaim=' + kode_klaim+'&kode_agenda='+kode_agenda + '&user='+user + '&report=kci&ACTION_TYPE=CETAK_BERITA_ACARA';
         window.open('http://<?=$HTTP_HOST;?>/mod_pn/ajax/<?=$php_file_name;?>_action.php?' + params);
  ...
  let wsLinkDownload= "<?php echo $wsFileSmile ?>";
    ...
    function encrypt_decrypt_x($action, $string)
    {
      ...
      */
      ...
      WHERE id_dokumen = '{$ls_klaim}' AND kode_jenis_dokumen='JD106'
      UNION ALL
      SELECT '$ls_klaim'
      pemilik,
      (SELECT nama_dokumen

...
if($TYPE=='query'){
    ...
    if($ls_search_pfilter!=""){
        if($ls_search_pfilter=='PUTUS')
            $condition .= " AND STATUS='PUTUS' ";
        else if($ls_search_pfilter=='BATAL')
            $condition .= " AND STATUS='BATAL' ";
    } else
    ...
    $sql = "select 
             ...
             from (SELECT ROWNUM as NO_URUT,KODE_RTW_KLAIM,
                    ...
                    where A.KODE_KANTOR_RTW='{$_SESSION['gs_kantor_aktif']}'
                    and ( (A.STATUS_RTW_KLAIM='SR1' and A.STATUS='PUTUS') or (A.STATUS_RTW_KLAIM='SR2' and A.STATUS='BATAL')or (A.STATUS_RTW_KLAIM='SR3' and A.STATUS='BATAL'))
                     {$condition}   {$order}) 

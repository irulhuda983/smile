...
if($ls_type=="new"){
	...
	$sql = "";
	$sql="select * from (select rownum no, kode_tipe_klaim,
		       nama_tipe_klaim,
		       no_urut,
		       status_nonaktif,
		       tgl_nonaktif,
	...
	$queryTotal = "SELECT COUNT (1) FROM SIJSTK.PN_KODE_TIPE_KLAIM WHERE 1=1 ".$condition;
	// print_r($sql);
	...
	if($DB->execute()){ 
		...
		{
		    $data['ACTION'] = '<input type="checkbox" class="btn green" id="CHECK_'.$data['KODE_TIPE_KLAIM'].'" name="CHECK_'.$data['KODE_TIPE_KLAIM'].'" kode_tipe_klaim="'.$data['KODE_TIPE_KLAIM'].'"  title="Pilih untuk pengaturan data">
		';
			$data['DETAIL'] = '<input type="button" class="btn green" id="DETAIL_'.$data['KODE_TIPE_KLAIM'].'" name="DETAIL_'.$data['KODE_TIPE_KLAIM'].'" value="Detail" kode_tipe_klaim="'.$data['KODE_TIPE_KLAIM'].'"  title="Klik untuk detail data">
		';

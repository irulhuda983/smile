...
//form serialize
$ls_kode_tipe_klaim		= $_POST['c_kode_tipe_klaim'];
$ls_kode_sebab_klaim	= $_POST['v_kode_sebab_klaim'];
$ls_nama_sebab_klaim	= $_POST['v_nama_sebab_klaim'];
...
if($ls_type=="new"){
	$sql = "insert into pn.pn_kode_sebab_klaim (
				kode_tipe_klaim,
				kode_sebab_klaim,
                nama_sebab_klaim,
                no_urut,
	...
	$sql = "update sijstk.pn_kode_sebab_klaim
			   set nama_sebab_klaim = '$ls_nama_sebab_klaim',
			       no_urut = '$ls_no_urut',
	...
	$sql="select kode_sebab_klaim,
		 ...
		 where kode_sebab_klaim='".$ls_search_keyword."'";
    $DB->parse($sql);
    if($DB->execute()){ 
	...
	$sql="delete from sijstk.pn_kode_sebab_klaim
      		where kode_sebab_klaim ='".$ls_kode_sebab_klaim."'";
    $DB->parse($sql);
	if($DB->execute()){ 
	...
	if($order[0]['column']=='0'){
	    ...
	    $order = "ORDER BY NAMA_SEBAB_KLAIM ";
	...
	$sql = "";
	$sql="select * from (select rownum no, kode_sebab_klaim,
		       nama_sebab_klaim,
		       no_urut,
		       status_nonaktif,
		       tgl_nonaktif,
		       keterangan
		  from sijstk.pn_kode_sebab_klaim
		 where 1 = 1 and kode_tipe_klaim='$ls_kode_tipe_klaim'  ".$condition." ".$order.")
		 where 1=1 and no between ".$start." and ".$end;

	$queryTotal = "SELECT COUNT (1) FROM SIJSTK.PN_KODE_SEBAB_KLAIM WHERE 1=1 and kode_tipe_klaim='$ls_kode_tipe_klaim' ".$condition;
	// print_r($sql);
	// print_r($querySql);
	...
	if($DB->execute()){ 
		...
		// ';
			$data['ACTION'] = '<button class="btn green" name="view_sebab_klaim_'.$data['KODE_SEBAB_KLAIM'].'" id="view_sebab_klaim_'.$data['KODE_SEBAB_KLAIM'].'" kode_sebab_klaim="'.$data['KODE_SEBAB_KLAIM'].'"  title="Klik untuk view detil data"><i class="fa fa-search" aria-hidden="true"></i></button>'.' '.' <button class="btn green" name="edit_sebab_klaim_'.$data['KODE_SEBAB_KLAIM'].'" id="edit_sebab_klaim_'.$data['KODE_SEBAB_KLAIM'].'" kode_sebab_klaim="'.$data['KODE_SEBAB_KLAIM'].'"  title="Klik untuk edit data"><i class="fa fa-pencil" aria-hidden="true"></i></button> <button class="btn hapus" name="hapus_sebab_klaim_'.$data['KODE_SEBAB_KLAIM'].'" kode_sebab_klaim="'.$data['KODE_SEBAB_KLAIM'].'" title="Klik untuk hapus data"><i class="fa fa-trash" aria-hidden="true"></i></button>
				'.'   '.'<button class="btn green" id="CHECK_'.$data['KODE_SEBAB_KLAIM'].'" name="DOKUMEN_'.$data['KODE_SEBAB_KLAIM'].'" kode_sebab_klaim="'.$data['KODE_SEBAB_KLAIM'].'"  title="Klik untuk detil dokumen"><i class="fa fa-file-text" aria-hidden="true"></i> Dokumen</button>&nbsp
				    <button value="Segmen" class="btn green" id="SEGMEN_'.$data['KODE_SEBAB_KLAIM'].'" name="SEGMEN_'.$data['KODE_SEBAB_KLAIM'].'" kode_sebab_klaim="'.$data['KODE_SEBAB_KLAIM'].'"  title="Klik untuk detil segmen"><i class="fa fa-university" aria-hidden="true"></i> Segmen</button>
				';

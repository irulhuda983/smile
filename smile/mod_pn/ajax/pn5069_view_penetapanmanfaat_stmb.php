                ...
                <table id="tblMnf" width="100%" class="table-data2">
                  <thead>								
                    <tr>
                      ...
                      <th style="text-align:center;" colspan="5" class="hr-single-bottom">PENETAPAN NILAI MANFAAT STMB</th>
                      <th style="text-align:center;" colspan="1"></th>
                    </tr>					
                    <tr class="hr-double-bottom">
                      <th style="text-align:center;width:50px;">No</th>
                      <th style="text-align:center;width:150px;">Tipe Penerima</th>
                      <th style="text-align:center;width:150px;">Periode STMB</th>
                      <th style="text-align:center;width:70px;">Jml Hari</th>
                      <th style="text-align:center;width:70px;">Jam Kerja</th>
                      <th style="text-align:right;width:100px;">Nilai STMB</th>
                    ...
                    <tr class="nohover-color">
                    	<td colspan="8" style="text-align: center;">-- data tidak ditemukan --</td>
									...
									<tfoot>
                    <tr>
                      <td style="text-align:right;" colspan="3"><i>Total Manfaat:</i>
                      ...
                      <td style="text-align:center"><span id="mnf_tot_jml_hari"></span></td>
											<td style="text-align:center"><span id="mnf_tot_jml_jamkerja"></span></td>
	...
	function loadSelectedRecord(v_kode_klaim, v_kode_manfaat, fn)
	{
		...
		$.ajax({
            ...
            var html_mnf = "";
						...
						var v_tot_d_jml_jamkerja = 0;
						var v_tot_d_biaya_disetujui = 0;		
            ...
            { 
              for(var i = 0; i < jdata.data.DATA.length; i++)
              {
                ...
                html_mnf += '<td style="text-align: center;">' + getValue(jdata.data.DATA[i].STMB_TGL_AWAL)+' s/d '+getValue(jdata.data.DATA[i].STMB_TGL_AKHIR) + '</td>';
								html_mnf += '<td style="text-align: center;">' + fl_js_format_nondesimal(getValueNumber(jdata.data.DATA[i].STMB_JML_HARI)) + '</td>';
                html_mnf += '<td style="text-align: center;">' + fl_js_format_nondesimal(getValueNumber(jdata.data.DATA[i].STMB_JML_JAMKERJA)) + '</td>';
                ...
                html_mnf += '<td style="text-align: center;">' 
                            ...
                            + getValue(jdata.data.DATA[i].NO_URUT) + '\')"><img src="../../images/indent_right.gif" border="0" alt="Rincian Manfaat" align="absmiddle" /><font style="text-align:right; font: 10px Verdana, Arial, Helvetica, sans-serif;"> VIEW </font></a>' + '';
								html_mnf += ' &nbsp;|&nbsp;' 
                            + '<a href="javascript:void(0)" onclick="fl_js_tap_mnf_detil(\'' 
                            + getValue(jdata.data.DATA[i].KODE_KLAIM) + '\', \'' 
                            + getValue(jdata.data.DATA[i].KODE_MANFAAT) + '\', \''  
                            + getValue(jdata.data.DATA[i].NO_URUT) + '\')"><img src="../../images/indent_right.gif" border="0" alt="Rincian Manfaat" align="absmiddle" /><font style="text-align:right; font: 10px Verdana, Arial, Helvetica, sans-serif;"> DETIL </font></a>' + '</td>';
                html_mnf += '</tr>';
                
                v_tot_d_jml_hari				+= parseFloat(getValueNumber(jdata.data.DATA[i].STMB_JML_HARI),4);													 
								v_tot_d_jml_jamkerja		+= parseFloat(getValueNumber(jdata.data.DATA[i].STMB_JML_JAMKERJA),4);
								v_tot_d_biaya_disetujui	+= parseFloat(getValueNumber(jdata.data.DATA[i].NOM_BIAYA_DISETUJUI),4);	 
              }
              ...
              $("#mnf_tot_jml_hari").html(fl_js_format_nondesimal(v_tot_d_jml_hari));
							$("#mnf_tot_jml_jamkerja").html(fl_js_format_nondesimal(v_tot_d_jml_jamkerja));
              $("#mnf_tot_biaya_disetujui").html(format_uang(v_tot_d_biaya_disetujui));			
            }

  ...
  function encrypt_decrypt($action, $string)
 {
    ...
    if($ls_btn_task=="upload"){
      ...
      if (isset($_POST['upload'])){
        ...
        if ($size > 1000000){
            $status_size=1;
            $msg_size=' Ukuran dokumen melebihi batas maksimal sebesar 1 Megabytes';
        }else{
            $status_size=0;
            $msg_size=' ';
        }
        ...
        if($status_tipe==1 && $status_size==0){
            ...
            $cek_dokumen="SELECT URL FROM SIJSTK.PN_KLAIM_DOKUMEN WHERE KODE_KLAIM='".$ls_kode_klaim."' AND KODE_DOKUMEN  = '".$ls_kode_dokumen."' ";

                                    
            $DB->parse($cek_dokumen);
            $DB->execute();
            $row = $DB->nextrow();
            $ls_path_url_exist = $row["URL"];
            
            if(strlen($ls_path_url_exist)>0){
            ...
            $ls_mime_type = $_FILES["lob_upload"]["type"];   
            $curl = curl_init();
                ...
                curl_setopt_array($curl, array(
                    ...
                    CURLOPT_POSTFIELDS => array(
                                        'file' => new CurlFile($ls_file['tmp_name'],$ls_file['type'],$ls_file['name']),
										'namaBucket' => 'smile',
                ...
                $ls_path_url= $result->path;

                if($ext=='pdf'){
                $ls_path_url_encrypt=encrypt_decrypt('encrypt',$ls_path_url);    
                $result_cek_pdf = api_json_call_get($wsIpLapakAsikOnline."/upload/validPdf?data=".$ls_path_url_encrypt);
                $result_status_cek_pdf=$result_cek_pdf->status;
                }else{
                $result_status_cek_pdf=true;    
                }
                 
                    if($result_status_cek_pdf){
                
                    $STATUS_UPLOAD="SUKSES";
                    $sql = "  UPDATE SIJSTK.PN_KLAIM_DOKUMEN
                    SET   
                    NAMA_FILE=:p_filename,
                    URL=:p_path_url,
                    ...
                    ";
                    $param_bv = [':p_filename' => $FILENAME,':p_path_url' => $ls_path_url, ':p_mime_type' => $ls_mime_type,':p_user' => $user,':p_kode_klaim' => $ls_kode_klaim,':p_kode_dokumen' => $ls_kode_dokumen];
                    $proc = $DB->parse($sql);
                    ...
                    $DB->execute();
                        ...
                        $headers1 = array(
                          ...
                          $str_array1 = explode("/", $ls_path_url);
                          $namaBucket1 = $str_array[1];
                          $namaFolder1 = $str_array[2]."/".$str_array[3]."/".$str_array[4];
                          $namaFile1 =  $str_array[6];
                
                          $data_delete1 = array(
                            'namaBucket'      =>  $namaBucket1,
                            'namaFolder'     =>  $namaFolder1,
                            'file'      => $namaFile1
                          );
                            
                          $result_delete_kpj = api_json_call($wsIpStorage."/object/delete", $headers1, $data_delete1);
                          $STATUS_UPLOAD="CORRUPT";
                    } 
        ...
        if($STATUS_UPLOAD=="SUKSES" && $status_size==0 && $status_tipe==1){
            ...
            $msg = "Upload dokumen berhasil, session dilanjutkan...";  
                ...
                // echo "window.opener.location.reload();";             /mod_pn/form/pn5040.php?task=Edit&kode_klaim=KL18020800003139&dataid=KL18020800003139&activetab=3#
                echo "window.close();";
                echo "window.opener.location.href='http://".$HTTP_HOST."/mod_pn/form/pn5040.php?task=Edit&kode_klaim=".$ls_kode_klaim."&dataid=".$ls_kode_klaim."&activetab=11';";
                echo "window.opener.refreshActivePage('".$FILENAME."');";
                // echo "window.opener.fl_js_set_status_diserahkan(".$ls_no_urut.")";
                echo "</script>"; 

        }else if ($STATUS_UPLOAD="CORRUPT" && $status_size==0 && $status_tipe==1){
                $msg = "Upload dokumen gagal. Silahkan pastikan dokumen tidak rusak/ corrupt ataupun memiliki password terlebih dahulu sebelum mengupload.";
...
<form name="fpop" method="post" action="pn5040_uploadlampirandokumen.php" enctype="multipart/form-data">
    <!-- LOCAL JAVASCRIPTS -->	
    ...
    <script type="text/javascript">
		...
		function tutup() {
			window.close();
            window.opener.location.href='http://<?=$HTTP_HOST;?>/mod_pn/form/pn5040.php?task=Edit&kode_klaim=<?=$ls_kode_klaim;?>&dataid=<?=$ls_kode_klaim;?>&activetab=11';

...
$php_file_name='pn6001_pelepasan_flag_potensi_klaim_fiktif';
...
$p_sender           = !isset($_GET['sender']) ? $_POST['sender'] : $_GET['sender'];
...
if(isset($_GET['kd_agenda'])){
    $kd_agenda = $_GET['kd_agenda'];      
        $sql = "
                SELECT AA.nama_lengkap,
                    ...
                    AA.status_approval,
                    AA.flag_draft
                FROM PN.PN_AGENDA_LEPAS_FIKTIF AA, KN.KN_PENDUDUK BB
        ...
        $sql_dokumen = "SELECT A.KODE_TK,
                                ...
                                A.PETUGAS_UBAH
                        FROM PN.PN_AGENDA_LEPAS_FIKTIF_DOK A
    ...
    if ($p_no_identitas != ""){     
        $sql = "
                ...
                FROM PN.PN_FIKTIF_KLAIM_KEPESERTAAN AA, PN.PN_FIKTIF_KLAIM BB
                WHERE   AA.KODE_FIKTIF_KLAIM=BB.KODE_FIKTIF_KLAIM   
                    AND AA.NOMOR_IDENTITAS = '$p_no_identitas'
                    AND BB.STATUS_APPROVAL = 'Y'
                    AND BB.STATUS_BATAL = 'T'
    ...
    <table aria-describedby="dokumen_pendukung" id="tblrincian1" width="95%" cellspacing="0" border="0" align="center" bordercolor="#C0C0C0" background-color= "#ffffff">
        <tbody>	
            ...
            while ($row_dok = $DB->nextrow()){	
                ...
                if($ls_path_url_dok != ''){
                    $d_u_dokumen                = '<span><a href = "#" onclick="downloadFileSmile('."'".$ls_path_url_dok."'".')"><font style="display:block;color:#0080FF;"><b>Download</b></a></span>';
...
<div class="form-row_kiri" width="100%">
    ...
    <?php if(($ses_reg_role == '3' || $ses_reg_role == '4' || $ses_reg_role == '27') && $p_viewfrom == ''){?>
...
<script type="text/javascript">
    ...
    function hapus_kartu(kode_agenda, kode_tk){
        var tanya = confirm("Apakah anda yakin akan menghapus data ini ?");
         ...
         if(tanya === true) {
            ...
            $.ajax({
                type: 'POST',
                url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn6001_pelepasan_flag_potensi_klaim_fiktif_action.php?'+Math.random(),
                data: {
                    TYPE				    : 'HAPUS_KARTU',
    ...
    function downloadFileSmile(path_url){
        ...
        $.ajax({
            type: 'POST',
            url: "../ajax/pn6001_pelepasan_flag_potensi_klaim_fiktif_action.php?"+Math.random(),
            data: {
    ...
    async function uploadFile(file){
        ...
        $json_spec = array(
            "kodeDokumen" => "",
            "bucketName" => "smile",
            "folderName" => "klaimfiktif/".$gs_kantor_aktif.'/'.date("Ym"),
            "idDokumen" => ""
            );
    ...
    function submit_setuju(){
		...
		$.ajax({
			...
			success: function(data){
				...
				if (jdata.ret == 0){
                    window.parent.Ext.notify.msg('Penyimpanan data berhasil dengan kode agenda '+jdata.dataid+', session dilanjutkan...', jdata.msg);
                    window.location='pn6001.php?task=Submit&path='+path+'&kd_agenda='+jdata.dataid+'&kd_perihal_detil='+kd_perihal_detil;
...
function gw_newWindow(mypage, myname, w, h, scroll) { // confirm(gw_openwin_param1);
    ...
    function fl_js_get_lov_by(lov) { 
        ...
        if (lov == "NO_IDENTITAS") {
            NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/form/pn6001_pelepasan_flag_potensi_klaim_fiktif_lov.php?'+params,'',800,500,1);
        } 
    }

            ...
            <?echo "<tr bgcolor=#".($i%2 ? "f3f3f3" : "ffffff").">";?>	
              <td style="text-align:center; font: 10px Verdana, Arial, Helvetica, sans-serif;"><?=$n;?></td>
							...
							<td>
  							...
  							{
  							?>
                <a href="#" onClick="NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5043_pembayaran_entry.php?task=New&kode_klaim=<?=$ls_kode_klaim;?>&kode_tipe_penerima=<?=$row["KODE_TIPE_PENERIMA"];?>&kd_prg=<?=$row["KD_PRG"];?>&form_root=<?=$ls_form_root;?>&root_sender=pn5043.php&sender=pn5043_pembayaran.php&id_pointer_asal=<?=$ls_id_pointer_asal;?>&kode_pointer_asal=<?=$ls_kode_pointer_asal;?>&tglawaldisplay=<?=$ld_tglawaldisplay;?>&tglakhirdisplay=<?=$ld_tglakhirdisplay;?>','Entry Pembayaran Klaim',850,600,'no')"><img src="../../images/check.png" border="0" alt="Entry Pembayaran" align="absmiddle" />&nbsp;Entry </a>									
  							<?
  							}
  							?>
  							<?
  							if ($row['KODE_PEMBAYARAN']!="")
  							{
    							?>
                  |
									<a href="#" onClick="NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5043_pembayaran_entry.php?task=view&kode_pembayaran=<?=$row["KODE_PEMBAYARAN"];?>&form_root=<?=$ls_form_root;?>&root_sender=pn5043.php&sender=pn5043_pembayaran.php&id_pointer_asal=<?=$ls_id_pointer_asal;?>&kode_pointer_asal=<?=$ls_kode_pointer_asal;?>&tglawaldisplay=<?=$ld_tglawaldisplay;?>&tglakhirdisplay=<?=$ld_tglakhirdisplay;?>','View Pembayaran Klaim',850,600,'no')"><img src="../../images/check.png" border="0" alt="Entry Pembayaran" align="absmiddle" />&nbsp;View </a>
									<a href="#" onClick="NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5048_cetak.php?kode_klaim=<?=$row["KODE_KLAIM"];?>&kode_pembayaran=<?=$row["KODE_PEMBAYARAN"];?>&mid=<?=$mid;?>&tglawaldisplay=<?=$ld_tglawaldisplay;?>&tglakhirdisplay=<?=$ld_tglakhirdisplay;?>','Cetak Penetapan Klaim',700,480,'no')"><img src="../../images/printx.png" border="0" alt="Entry Pembayaran" align="absmiddle" style="height:20px;"/>&nbsp;Cetak </a>									
    							<?
  							}
  							?>								
              </td>

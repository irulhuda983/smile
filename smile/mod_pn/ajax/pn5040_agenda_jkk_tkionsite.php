...
<div id="formKiri" style="width:870px;">
  <fieldset><legend>Informasi Kecelakaan</legend>
    ...
    <label style = "text-align:right;">Jam Kecelakaan &nbsp; *</label>		 	    				
      ...
      <? 
        $param_bv = [];
          ...
          $param_bv[':p_jkk_kode_jam_kecelakaan'] = $ls_jkk_kode_jam_kecelakaan;
					$sql = "select kode, keterangan from sijstk.ms_lookup where tipe=:p_tipe and kode=:p_jkk_kode_jam_kecelakaan order by seq";
        }else
  			{
          $param_bv[':p_tipe'] = 'KLMJAMKERJ';
          $param_bv[':p_status'] = 'Y';
    ...
    <label style = "text-align:right;">Jenis Kasus &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; *</label>		 	    				
      ...
      <?
        $param_bv = [];
  			if ($ls_status_submit_agenda =="Y")
  			{
          $param_bv[':p_jkk_kode_jenis_kasus'] = $ls_jkk_kode_jenis_kasus;
					$sql = "select kode_jenis_kasus, nama_jenis_kasus from sijstk.pn_kode_jenis_kasus where kode_jenis_kasus=:p_jkk_kode_jenis_kasus";
        }else
  			{
            ...
            $param_bv[':p_status'] = 'T';
            $param_bv[':p_kode_jenis_kasus'] = 'KS02';
					 	 $sql = "select kode_jenis_kasus, nama_jenis_kasus from sijstk.pn_kode_jenis_kasus where kode_tipe_klaim = :p_kode_tipe_klaim and nvl(status_nonaktif,'T')= :p_status and kode_jenis_kasus <> :p_kode_jenis_kasus order by no_urut";	
					}else
					{	 
            $param_bv[':p_kode_tipe_klaim'] = $ls_kode_tipe_klaim;
            $param_bv[':p_status'] = 'T';
      ...
      <label style = "text-align:right;">Lokasi Kecelakaan &nbsp; *</label>		 	    				
        ...
        $param_bv = [];
    			if ($ls_status_submit_agenda =="Y")
    			{
            $param_bv[':p_jkk_kode_lokasi_kecelakaan'] = $ls_jkk_kode_lokasi_kecelakaan;
						$sql = "select kode_lokasi_kecelakaan, nama_lokasi_kecelakaan from sijstk.pn_kode_lokasi_kecelakaan where kode_lokasi_kecelakaan=:p_jkk_kode_lokasi_kecelakaan";
          }else
    			{
            $param_bv[':p_status'] = 'T';
	...
	<span id="jkk_span_kecelakaan_kerja" style="display:none;">
      ...
      <label style = "text-align:right;">Tindakan Bahaya &nbsp;</label>		 	    				
        <select size="1" id="jkk_kode_tindakan_bahaya" name="jkk_kode_tindakan_bahaya" value="<?=$ls_jkk_kode_tindakan_bahaya;?>" tabindex="1" class="select_format" style="width:455px;" >
        <option value="">-- Pilih Tindakan Bahaya --</option>
        <? 
        $sql = "select kode, keterangan from sijstk.ms_lookup where tipe=:p_tipe and nvl(aktif,'T')= :p_status order by seq";
        $proc = $DB->parse($sql);
        $pram_bv = [':p_tipe' => 'KLMTINDBHY',':p_status' => 'Y'];
        foreach ($param_bv as $key => $value) {
          oci_bind_by_name($proc, $key, $param_bv[$key]);
        }
        $DB->execute();
      ...
      <label style = "text-align:right;">Kondisi Bahaya &nbsp;</label>		 	    				
        <select size="1" id="jkk_kode_kondisi_bahaya" name="jkk_kode_kondisi_bahaya" value="<?=$ls_jkk_kode_kondisi_bahaya;?>" tabindex="2" class="select_format" style="width:455px;" >
        <option value="">-- Pilih Kondisi Bahaya --</option>
        <? 
        $sql = "select kode, keterangan from sijstk.ms_lookup where tipe=:p_tipe and nvl(aktif,'T')= :p_status order by seq";
        $proc = $DB->parse($sql);
        $pram_bv = [':p_tipe' => 'KLMKONDBHY',':p_status' => 'Y'];
        foreach ($param_bv as $key => $value) {
          oci_bind_by_name($proc, $key, $param_bv[$key]);
        }
        $DB->execute();
      ...
      <label style = "text-align:right;">Corak &nbsp;</label>
        <input type="text" id="jkk_kode_corak" name="jkk_kode_corak" value="<?=$ls_jkk_kode_corak;?>" tabindex="3" size="7" maxlength="10" readonly>
        <input type="text" id="jkk_nama_corak" name="jkk_nama_corak" value="<?=$ls_jkk_nama_corak;?>" size="50" readonly class="disabled">
        <a href="#" onclick="NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5041_lovtapjkk1_corak.php?p=pn5040_tabdataklaim_jkktkionsite.php&a=formreg&b=jkk_kode_corak&c=jkk_nama_corak','',800,500,1)">
      ...
      <input type="text" id="jkk_kode_sumber_cedera" name="jkk_kode_sumber_cedera" value="<?=$ls_jkk_kode_sumber_cedera;?>" tabindex="4" size="7" maxlength="10" readonly>
      ...
      <a href="#" onclick="NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5041_lovtapjkk1_sumbercedera.php?p=pn5040_tabdataklaim_jkktkionsite.php&a=formreg&b=jkk_kode_sumber_cedera&c=jkk_nama_sumber_cedera','',800,500,1)">
      ...
      <input type="text" id="jkk_kode_bagian_sakit" name="jkk_kode_bagian_sakit" value="<?=$ls_jkk_kode_bagian_sakit;?>" tabindex="5" size="7" maxlength="10" readonly>
      ...
      <a href="#" onclick="NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5041_lovtapjkk1_bagiansakit.php?p=pn5040_tabdataklaim_jkktkionsite.php&a=formreg&b=jkk_kode_bagian_sakit&c=jkk_nama_bagian_sakit','',800,500,1)">
      ...
      <input type="text" id="jkk_kode_akibat_diderita" name="jkk_kode_akibat_diderita" value="<?=$ls_jkk_kode_akibat_diderita;?>" tabindex="6" size="7" maxlength="10" readonly style="background-color:#ffff99;">
      ...
      <a href="#" onclick="NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5041_lovtapjkk1_akibatdiderita.php?p=pn5040_tabdataklaim_jkktkionsite.php&a=formreg&b=jkk_kode_akibat_diderita&c=jkk_nama_akibat_diderita','',800,500,1)">
	...
	<span id="jkk_span_penyakit_akibat_kerja" style="display:none;">
      ...
      <select size="1" id="jkk_kode_lama_bekerja" name="jkk_kode_lama_bekerja" value="<?=$ls_jkk_kode_lama_bekerja;?>" tabindex="7" class="select_format" style="width:455px;" >
      <option value="">-- Pilih --</option>
      <? 
      $sql = "select kode, keterangan from sijstk.ms_lookup where tipe=:p_tipe and nvl(aktif,'T')=:p_status order by seq";
      $proc = $DB->parse($sql);
      $pram_bv = [':p_tipe' => 'KLMLAMAKRJ',':p_status' => 'Y'];
      foreach ($param_bv as $key => $value) {
        oci_bind_by_name($proc, $key, $param_bv[$key]);
      }
      $DB->execute();
      ...
      <input type="text" id="jkk_kode_penyakit_timbul" name="jkk_kode_penyakit_timbul" value="<?=$ls_jkk_kode_penyakit_timbul;?>" tabindex="8" size="7" maxlength="10" readonly>
      ...
      <a href="#" onclick="NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5041_lovtapjkk1_penyakittimbul.php?p=pn5040_tabdataklaim_jkktkionsite.php&a=formreg&b=jkk_kode_penyakit_timbul&c=jkk_nama_penyakit_timbul','',800,500,1)">
      <img src="../../images/help.png" alt="LOV Bagian yang Sakit" border="0" align="absmiddle"></a>													
      </div>																																										
      <div class="clear"></div>			
		</fieldset>			
  ...
  <fieldset><legend>Tempat Tenaga Kerja Dirawat</legend>
    ...
    <label style = "text-align:right;">Tempat Perawatan &nbsp;</label>		 	    				
      <select size="1" id="jkk_kode_tempat_perawatan" name="jkk_kode_tempat_perawatan" value="<?=$ls_jkk_kode_tempat_perawatan;?>" tabindex="9" class="select_format" style="width:350px;background-color:#ffff99;" onchange="fl_js_jkk_kode_tempat_perawatan();" >
      <option value="">-- Pilih --</option>
      <? 
      $sql = "select kode, keterangan from sijstk.ms_lookup where tipe=:p_tipe and nvl(aktif,'T')=:p_status order by seq";
      $proc = $DB->parse($sql);
      $pram_bv = [':p_tipe' => 'KLMTMPTRWT',':p_status' => 'Y'];
      foreach ($param_bv as $key => $value) {
        oci_bind_by_name($proc, $key, $param_bv[$key]);
      }
      $DB->execute();
    ...
    <label style = "text-align:right;">Berobat Jalan &nbsp;</label>		 	    				
      <select size="1" id="jkk_kode_berobat_jalan" name="jkk_kode_berobat_jalan" value="<?=$ls_jkk_kode_berobat_jalan;?>" tabindex="10" class="select_format" style="width:350px;background-color:#ffff99;" >
      <option value="">-- Pilih --</option>
      <? 
      $sql = "select kode, keterangan from sijstk.ms_lookup where tipe=:p_tipe and nvl(aktif,'T')=:p_status order by seq";
      $proc = $DB->parse($sql);
      $pram_bv = [':p_tipe' => 'KLMBOBTJLN',':p_status' => 'Y'];
      foreach ($param_bv as $key => $value) {
        oci_bind_by_name($proc, $key, $param_bv[$key]);
      }
      $DB->execute();
    ...
    <label  style = "text-align:right;">Fasilitas Kesehatan</label>
      <input type="text" id="jkk_kode_ppk" name="jkk_kode_ppk" value="<?=$ls_jkk_kode_ppk;?>" tabindex="11" size="7" maxlength="20" readonly style="background-color:#ffff99;" >
      <input type="text" id="jkk_nama_ppk" name="jkk_nama_ppk" value="<?=$ls_jkk_nama_ppk;?>" size="35" readonly class="disabled">
      <a href="#" onclick="NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5041_lovtapjkk1_faskes.php?p=pn5040_tabdataklaim_jkktkionsite.php&a=formreg&b=jkk_kode_ppk&c=jkk_nama_ppk','',800,500,1)">
    ...
    <span id="jkk_span_reimburse" style="display:none;">						
      ...
      <input type="text" id="jkk_nama_faskes_reimburse" name="jkk_nama_faskes_reimburse" value="<?=$ls_jkk_nama_faskes_reimburse;?>" tabindex="12" size="40" maxlength="100" style="background-color:#ffff99;">
      ...
      <tbody>
        ...
        if ($ls_kode_klaim!="")
        {
          ...
          ?>																				
            ...
            <td>
            	<a href="#" onclick="removeperbrs3(this.parentNode.parentNode.rowIndex);" id="link_delete"><img src="../../images/file_cancel.gif" border="0" align="absmiddle"> Delete </a>	
            </td>									
            </tr>
            <?				    							
            $i++;//iterasi i
    ...
    <label style = "text-align:right;">Kondisi Terakhir &nbsp;</label>		 	    				
      ...
      <? 
        $param_bv = [];
				if ($ls_status_submit_agenda =="Y")  	
  			{
          $param_bv[':p_jkk_kode_kondisi_terakhir'] = $ls_jkk_kode_kondisi_terakhir;
				  $sql = "select kode_kondisi_terakhir, nama_kondisi_terakhir from sijstk.pn_kode_kondisi_terakhir where kode_kondisi_terakhir = :p_jkk_kode_kondisi_terakhir"; 			
				}else
				{
          $param_bv[':p_kode_tipe_klaim'] = $ls_kode_tipe_klaim;
          $param_bv[':p_status'] = 'T';

  ...
  <div id="actmenu">
  	<div id="actbutton">
        ...
        ?>        
        <div style="float:left;"><div class="icon">
        	<a id="btn_close" href="http://<?=$HTTP_HOST;?>/mod_pn/form/pn5007.php?mid=<?=$mid;?>"><img src="http://<?=$HTTP_HOST;?>/images/file_cancel.gif" align="absmiddle" border="0"> Close</a> 
        </div></div>
        <?PHP
      } 
  		else 
  ...
  if($btn_task=="Gen_Penetapan_Ulang")
  {
		...
		//cek kelayakan ----------------------------------------------------------
    $qry = "BEGIN SIJSTK.P_PN_PN5001.X_GEN_PENETAPAN_ULANG('$ls_kode_klaim_induk','$gs_kantor_aktif','$username',:p_kode_klaim_anak,:p_sukses,:p_mess);END;";											 	
    $proc = $DB->parse($qry);				
  ...
  if($btn_task=="submit_penetapan_tanpa_otentikasi")
  {
    ...
    $qry = "BEGIN SIJSTK.P_PN_PN5001.X_POST_PENETAPAN('$ls_kode_klaim','$username',:p_sukses,:p_mess);END;";											 	
    $proc = $DB->parse($qry);				
    oci_bind_by_name($proc, ":p_sukses", $p_sukses,32);
    oci_bind_by_name($proc, ":p_mess", $p_mess,1000);
    $DB->execute();				
  ...
  {
    //cek kelayakan ----------------------------------------------------------
    $qry = "BEGIN SIJSTK.P_PN_PN5001.X_HITUNG_MNFULANG_JHTRINCI('$ls_kode_klaim','$username',:p_sukses,:p_mess);END;";											 	
    $proc = $DB->parse($qry);				
    oci_bind_by_name($proc, ":p_sukses", $p_sukses,32);
    oci_bind_by_name($proc, ":p_mess", $p_mess,1000);
    $DB->execute();				

...
$TYPE       = $_POST["TYPE"];
$ACTION     = !isset ($_POST["ACTION_TYPE"]) ? $_GET["ACTION_TYPE"] : $_POST["ACTION_TYPE"] ;
$USER       = $_SESSION["USER"];
...
if ($TYPE == "New") {
    ...
    $ls_nom_upah            = str_replace(",","",$ls_nom_upah);
  ...
  $qry_agenda = "UPDATE  PN.PN_AGENDA_KOREKSI
   ...
   $sql = "
    ...
    $DB->parse($sql);
    if($DB->execute()){
      $sql = "BEGIN PN.P_PN_PN60010209.X_INSERT_TDL_KLAIM_JHT
                    (   '$ls_kode_agenda',
                        '$ls_kode_klaim',
        ...
        if($DB->execute()){
            ...
            echo '{"ret":-1,"msg":"Gagal, Gagal Eksekusi Prosedur PN.P_PN_PN60010209.X_INSERT_TDL_KLAIM_JHT!"}';
        }         
    }
    ...
    $sql = "SELECT  COUNT(1)
              ...
              WHERE   KODE_AGENDA = '$ls_kode_agenda'
              AND EXISTS (SELECT 1 FROM PN.PN_KURANG_BAYAR_DATA
                            WHERE KODE_AGENDA_TINDAKLANJUT = A.KODE_AGENDA)";
    ...
    if ($recordsTotal <= 0) {
        ...
        $sql = "BEGIN PN.P_PN_PN60010209.x_approve_tdl_klaim_jht
				(
					'$ls_kode_agenda',
                    '$KD_KANTOR',
					'$USER',
        ...
        OCIBindByName($proc, ":p_sukses", $p_sukses,32);
        OCIBindByName($proc, ":p_mess", $p_mess,1000);
        OCIBindByName($proc, ":p_approval_selesai", $p_approval_selesai,1000);
        OCIBindByName($proc, ":p_kode_agenda_out", $p_kode_agenda_out,1000);
        OCIBindByName($proc, ":p_kode_klaim_out", $p_kode_klaim_out,1000);
		...
		if($DB->execute()){
            ...
            if($ls_sukses == '1'){
                ...
                $result = api_json_call($wsIp . "/JSDS/StoreDocument", $headers, $data);
                ...
                if($result->ret == "0") {
                    ...
                    if($ls_is_kcp > 0){
                        ...
                        $result = api_json_call($wsIp . "/JSDS/SignDocument", $headers, $dataSign);
                }
            }
   ...
   // cetak penetapan koreksi
...
else if ($ACTION == "CETAK_BERITA_ACARA") {
    $ls_report = $_GET['report'];

    if($ls_report=='kcp'){

        $ls_kode_agenda     = $_GET['kode_agenda'];
        $ls_kode_klaim      = $_GET['kode_klaim'];
        $ls_user            = $_GET['user'];
    
        $file = file_get_contents($ipReportServer.'/reports/rwservlet/setauth?button=Submit&username='.$nc_rpt_user_arsip.'&password='.$nc_rpt_pass_arsip.'&authtype=D&mask=GQ%253D%253D&isjsp=no&database='.$nc_rpt_sid.'&nextpage=destype%3Dcache%26desformat%3DPDF%26report%3DPNR310002.rdf%26userid%3D%2Fdata%2Freports%2Fpn%26%26P_KODE_KLAIM%3D%27'.$ls_kode_klaim.'%27%26P_KODE_AGENDA%3D%27'.$ls_kode_agenda.'%27%26P_USER%3D%27'.$ls_user.'%27');
    ...
    if ($recordsTotal <= 0) {
		...
		$sql = "BEGIN PN.P_PN_PN60010209.X_BATAL_TDL_KLAIM_JHT
				(
					'$ls_kode_agenda',
					'$USER',
					:p_sukses,
		...
		if($DB->execute()){
            ...
            echo '{"ret":-1,"msg":"Gagal, Gagal Eksekusi Prosedur PN.P_PN_PN60010209.X_BATAL_TDL_KLAIM_JHT!"}';
        }
    }
}

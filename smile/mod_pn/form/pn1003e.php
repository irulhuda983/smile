...
<div id="formKiri"> 
	<fieldset id="fieldset_form_kondisi_terakhir" style="display: none"><legend>Form Kondisi Terakhir </legend>
		<form name="form_kondisi_terakhir" id="form_kondisi_terakhir" role="form" method="post">
			<div class="clear"></div>
			<div class="form-row_kiri">
				<label>Kode Kondisi Terakhir</label>
				<input type="text" id="v_kode_kondisi_terakhir" name="v_kode_kondisi_terakhir" size="30" maxlength="100" tabindex="3">
				<input type="hidden" name="type">
			</div>
			<div class="clear"></div>
			<div class="form-row_kiri">
				<label>Nama Kondisi Terakhir</label>
				<input type="text" id="v_nama_kondisi_terakhir" name="v_nama_kondisi_terakhir" size="30" maxlength="100" tabindex="3">
			</div>
			<div class="clear"></div>
			<div class="form-row_kiri">
				<label>No Urut</label>
		...
		<input type="button" class="btn green" id="btn_simpan_kondisi_terakhir" name="btn_simpan_kondisi_terakhir" value="SIMPAN" title="Klik Untuk Simpan data">
	</fieldset>

	<!--Fieldset Table Sebab Klaim -->
	<fieldset id="fieldset_kondisi_terakhir" style="display: block"><legend>Data Kondisi Terakhir</legend>
		<div class="form-row_kiri">
			<button class="btn green" id="btn_tambah_kondisi_terakhir" name="btn_tambah_kondisi_terakhir"  title="Klik Untuk Menambahkan data"><i class="fa fa-plus" aria-hidden="true"></i> TAMBAH</button>
		</div>
		<div class="form-row_kanan">
			...
			<select name="f_pilihan" id="f_pilihan">
				...
				<option value="nama_kondisi_terakhir">Nama Kondisi Terakhir</option>
		...
		<table class="table table-striped table-bordered row-border hover" id="table_kondisi_terakhir" cellspacing="0" width="100%">
			<thead>
				<tr>
					<th scope="col" width="5%" class="align-left">Kode Kondisi Terakhir</th>
					<th scope="col" width="25%" class="align-left">Nama Kondisi Terakhir</th>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('input[name*="btn_kembali"]').click(function() {
			...
			$('#fieldset_kondisi_terakhir').css("display","block");
		});
		$('#btn_tambah_kondisi_terakhir').click(function(){
			resetForm();
			$('input[name*="btn_simpan"]').show();
			$('input[name="type"]').val('new');
			$('fieldset[id*="fieldset_"]').css("display","none");
			$('#fieldset_form_kondisi_terakhir').css("display","block");
		});

		//ajax save for sebab klaim
		$('#btn_simpan_kondisi_terakhir').click(function() {
			// if(validasiForm()){
				
				preload(true);
				$.ajax({
					type: 'POST',
					url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn1003e_action.php?'+Math.random(),
					data: $('#form_kondisi_terakhir').serialize(),
					success: function(data) {
						...
						if(jdata.ret == '0'){
							...
							$('fieldset[id*="fieldset_"').css("display","none");
							$('#fieldset_kondisi_terakhir').css("display","");
							window.table_kondisi_terakhir.columns.adjust().draw();
						} else {
...
function loadTable(){
	...
	window.table_kondisi_terakhir = $('#table_kondisi_terakhir').DataTable({
		"scrollCollapse"	: true,
		"paging"			: true,
		...
		"destroy"			: true,
        "ajax"				: {
        	"url"	: "http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn1003e_action.php",
        	"type": "POST",
        	"data" : function(e) { 
        		e.type = 'query';
	...
	// window.table_kondisi_terakhir.columns.adjust().draw();
	window.table_kondisi_terakhir.on( 'draw.dt', function () {
		$('button[name*="edit_kondisi_terakhir"]').click(function() {
			window.kode_kondisi_terakhir = $(this).attr('kode_kondisi_terakhir');
			getDataRowKondisiTerakhir('edit',window.kode_kondisi_terakhir);
		});
		$('button[name*="view_kondisi_terakhir"]').click(function() {
			window.kode_kondisi_terakhir = $(this).attr('kode_kondisi_terakhir');
			getDataRowKondisiTerakhir('view',window.kode_kondisi_terakhir);
		});
		$('button[name*="hapus_kondisi_terakhir"]').click(function() {
			window.kode_kondisi_terakhir = $(this).attr('kode_kondisi_terakhir');
			deleteDataKondisiTerakhir(window.kode_kondisi_terakhir);
		});
	});
}


function deleteDataKondisiTerakhir(kode_kondisi_terakhir){
	if(confirm("Anda yakin ingin menghapus data ini ? ")){
		...
		$.ajax({
			...
			data: {
				type : "delete",
				v_kode_kondisi_terakhir : kode_kondisi_terakhir
			},
			success: function(data) {
				preload(false);
				jdata = JSON.parse(data);
...
function getDataRowKondisiTerakhir(type, kode_kondisi_terakhir){
	preload(true);
	window.type = type;
	...
	$.ajax({
		...
		data: { 
			...
			if(window.type=='view'){
				...
				$("#form_kondisi_terakhir :input").attr('disabled', 'disabled');
				$('#v_kode_kondisi_terakhir').css('background', '');
				$('input[name*="btn_simpan"]').hide();
			}else{
				$("#form_kondisi_terakhir :input").removeAttr('disabled', 'disabled');
				$('#v_kode_kondisi_terakhir').attr('readonly','readonly');
				$('#v_kode_kondisi_terakhir').attr('title', 'Field ini adalah kunci jadi tidak bisa diupdate');
				$('#v_kode_kondisi_terakhir').css('background', '#dcdada');
				$('input[name*="btn_simpan"]').show();
			}
			$('#v_kode_kondisi_terakhir').val(jdata.KODE_KONDISI_TERAKHIR);
			$('#v_nama_kondisi_terakhir').val(jdata.NAMA_KONDISI_TERAKHIR);
			...
			//aktif form show 
			$('fieldset[id*="fieldset_"]').css("display","none");
			$('#fieldset_form_kondisi_terakhir').css("display","");

<?php
$pagetype = "report";
$gs_pagetitle = "Dashboard Pelayanan - Pembayaran Klaim JHT Nasional Per Sebab Klaim";
...
for($i=0;$i<ExtendedFunction::count($resultArray->JHT_GRAPH1);$i++)
{
  //grafik bar -----------------------------------------------------------------
	$ln_graph1_bar_val = 0;
  $ln_graph1_bar_val = $resultArray->JHT_GRAPH1[$i]->NOM_PEMBAYARAN == "" ? "0" : $resultArray->JHT_GRAPH1[$i]->NOM_PEMBAYARAN;
  $ln_graph1_bar_val_tot = $ln_graph1_bar_val_tot + (float)$ln_graph1_bar_val;
  $ls_graph1_bar_xAxisCategory1_temp .= "'".$resultArray->JHT_GRAPH1[$i]->KEYWORD_SEBAB_KLAIM."',";
  $ls_graph1_bar_val_temp .= $ln_graph1_bar_val.",";
	
	//grafik pie -----------------------------------------------------------------
	$ls_graph_pie_val_x1 = $resultArray->JHT_GRAPH1[$i]->KODE_SEBAB_KLAIM;
  $ls_graph_pie_val_x2 = $resultArray->JHT_GRAPH1[$i]->KEYWORD_SEBAB_KLAIM;
  $ln_graph_pie_val_x3 = $resultArray->JHT_GRAPH1[$i]->NOM_PEMBAYARAN;
	...
	<div class="content">
		<div class="container">
          ...
          <div class="row">
    				<div class="col-lg-12">
              <div class="card-box">
    						<h4 class="header-title m-t-0 m-b-30"><b>Pembayaran Klaim JHT Per Sebab Klaim</b></h4>
          ...
          <div class="row">
    				<div class="col-lg-12">
              <div class="card-box">
    						<h4 class="header-title m-t-0 m-b-30"><b>Komposisi Pembayaran Klaim JHT Per Sebab Klaim</b></h4> 
                <div class="widget-chart text-center">
                    ...
                    for($i=0;$i<ExtendedFunction::count($resultArray->JHT_GRAPH1);$i++)
                    {
                      ?>
                        ...
                        <td style="text-align:left">
													<a href="#" onClick="NewWindow4('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn9010_jht_graph02dtl.php?task=view&kode_kantor=<?=$ls_kode_kantor;?>&kode_segmen=<?=$ls_kode_segmen;?>&tgl=<?=$ld_tgl;?>&tgl_mmddyyyy=<?=$ld_tgl_mmddyyyy;?>&kode_sebab_klaim=<?=$resultArray->JHT_GRAPH1[$i]->KODE_SEBAB_KLAIM;?>&sender=pn9010.php&sender_mid=<?=$mid;?>','Drilldown',1250,680,'yes');" href="javascript:void(0);">
          									 <?=$resultArray->JHT_GRAPH1[$i]->KEYWORD_SEBAB_KLAIM;?> - <?=$resultArray->JHT_GRAPH1[$i]->NAMA_SEBAB_KLAIM;?>
          								</a>
												</td>
                        <td style="text-align:right"><?=number_format((float)$resultArray->JHT_GRAPH1[$i]->JML_KASUS,0,".",",");?></td>  									
                        <td style="text-align:right;"><?=number_format((float)$resultArray->JHT_GRAPH1[$i]->NOM_PEMBAYARAN,2,".",",");?></td>						
                      </tr>
                      <?							    							
                      //$i++;//iterasi i
											$ln_tot_kasus_jht  += (float)$resultArray->JHT_GRAPH1[$i]->JML_KASUS;
											$ln_tot_pembayaran_jht += (float)$resultArray->JHT_GRAPH1[$i]->NOM_PEMBAYARAN;
                    }	//end while

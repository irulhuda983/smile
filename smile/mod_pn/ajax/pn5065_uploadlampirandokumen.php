...
$ls_data_id					= $ls_kode_klaim."-".$ln_no_konfirmasi;
...
{      
      if (isset($_POST['upload']))
        ...
        if($status_tipe==1 && $status_size==0)
          ...
          $sql = "UPDATE SMILE.PN_KLAIM_BERKALA_DOKUMEN
                       SET   
                          NAMA_FILE='$FILENAME',
        ...
        if($STATUS_UPLOAD==0 && $status_size==0 && $status_tipe==1){
            
            $msg = "Upload dokumen berhasil, session dilanjutkan...";  
                echo "<script language=\"JavaScript\" type=\"text/javascript\">";
                echo "window.close();";
                echo "window.opener.reloadFormUtama();";
...
<?php
    ...
    $kode_klaim  = $_GET['kode_klaim'];
		$no_konfirmasi  = $_GET['no_konfirmasi'];
    $kode_agenda = $_GET['kode_dokumen'];
    $no_urut     = $_GET['no_urut'];
    $isi_teks = $kode_klaim.'#'.$no_konfirmasi.'#'.$kode_agenda.'#'.$no_urut;
    $namafile = $kode_klaim.'_'.$no_konfirmasi.'_'.$kode_agenda.'_'.$no_urut.".png";
...
<form name="fpop" method="post" action="pn5065_uploadlampirandokumen.php" enctype="multipart/form-data">
    <!-- LOCAL JAVASCRIPTS -->	

<?php
$pagetype = "report";
$gs_pagetitle = "PN9011 - LAPORAN REALISASI SLA KLAIM";
...
$sql2 = " SELECT a.rowid, a.kode_report, b.nama_jenis_report_detil, a.parameter_report
		  ...
		  WHERE     a.kode_report = b.kode_report
		       ...
		       AND b.kode_jenis_report = 'PN9011'
...
$tgl_periode2 = $_POST["periode2"];

if(isset($tgl_periode2)&&$jenis_report=='KPI003'){
	$query_check = "select case when to_date('".$_POST['periode2']."','dd/mm/rrrr') < trunc(sysdate) then 1 else 0 end hasil from dual";
	$DB->parse($query_check);
	$DB->execute();
	while ($row = $DB->nextrow()){
		$hasil = $row['HASIL'];
	}
	if($hasil == 0||$hasil == '0'){
		echo "<script type='text/javascript'>alert('Periode maksimal hanya sampai dengan H-1');</script>";
	}
}

if (isset($jenis_report) && (isset($iscetak) || isset($iscetak_excel)) && ($hasil == 1 || $hasil == '1')) {
	...
	foreach(array_keys($REPORT[$jenis_report]["PARAMS_REPORT"]) as $key){
		...
		if ($field == "periode" || $field == "periode1" || $field == "periode2") {
        	...
        	if($_POST[$field]=='901'||$_POST[$field]=='902'||$_POST[$field]=='903'||$_POST[$field]=='904'||$_POST[$field]=='905'||$_POST[$field]=='906'||$_POST[$field]=='907'||$_POST[$field]=='908'||$_POST[$field]=='909'||$_POST[$field]=='910'||$_POST[$field]=='911'){
        		$value = $_POST[$field] - 900;
        	}else{
        		$value = $_POST[$field];
        	}
        }else {
	...
	// var_dump($ls_nama_rpt);
	// die();
	if ($ls_error=="1") {

	} else {
		exec_rpt_enc_kpi(1, $ls_modul, $ls_nama_rpt, $ls_user_param, $tipe);
	}
}
?>
				...
				<?php
					if(isset($REPORT[$jenis_report])) {
						foreach ($REPORT[$jenis_report]["FIELDS_VIEW"] as $field) {
							include "../../mod_kpi/form/kpi9001_fields_view.php";
						}
					}
				?>

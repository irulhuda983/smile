    ...
    $kd_agenda = $_GET['kd_agenda'];
        ...
        $sql = "
                SELECT AA.nama_lengkap,
                    ...
                    AA.status_batal,
                    AA.status_approval,
                    AA.PETUGAS_SUBMIT_TINDAK_LANJUT
        ...
        $ls_petugas_submit_tindak_lanjut= $row["PETUGAS_SUBMIT_TINDAK_LANJUT"]; 


        $sql_daftar_kartu = "SELECT DISTINCT
                x.* 
            FROM
                (
                SELECT
                    aa.kode_tk,
                    ( SELECT b.kpj FROM kn.kn_tk b WHERE b.kode_tk = aa.kode_tk AND ROWNUM = 1 ) kpj,
                    ( SELECT b.npp FROM kn.kn_perusahaan b WHERE b.kode_perusahaan = aa.kode_perusahaan AND ROWNUM = 1 ) npp,
                    aa.nama_perusahaan,
                    (
                    SELECT
                        b.aktif 
                    FROM
                        kn.kn_kepesertaan_tk b 
                    WHERE
                        b.kode_tk = aa.kode_tk 
                        AND b.kode_kepesertaan = aa.kode_kepesertaan --AND b.kode_perusahaan = aa.kode_perusahaan
                ...
                FROM
                    PN.PN_AGENDA_VERIFIKASI_JHT_KEPS aa 
                WHERE
                    aa.kode_agenda = '$kd_agenda' 
                ) x 
            ORDER BY
                x.kpj
        ";
        ...
        $sql_daftar_kartu_tdk_proses = "SELECT
                A.* 
            FROM
                (
                SELECT
                    ...
                    aa.nom_saldo_jht jml_saldo 
                FROM
                    PN.PN_AGENDA_VERIFIKASI_JHT_TUKEP aa 
                WHERE
                    aa.kode_agenda = '$kd_agenda' 
                ) a 
            ORDER BY
        ...
        $sql_dokumen = "SELECT kode_tk,
                                kpj,
                               (SELECT DOK.KETERANGAN FROM PN.PN_KODE_DOKUMEN DOK WHERE DOK.KODE_DOKUMEN = A.KODE_DOKUMEN) agenda_detil,
                                path_url,
                                flag_mandatory,
                                TO_CHAR(tgl_upload,'DD-MM-YYYY') tgl_upload,
                                petugas_rekam,
                                tgl_rekam
                        FROM (SELECT a.kode_tk,
                                ...
                                WHERE a.kode_agenda = '$kd_agenda') a order by kpj";
        ...
        $sql_daftar_kartu = "SELECT DISTINCT
            ...
            FROM
                ...
                SELECT
                    ...
                    (
                    SELECT
                        SUM( bb.nilai_transaksi ) 
                    FROM
                        kn.kn_tk_saldo bb 
                    WHERE
                        bb.kode_tk = aa.kode_tk 
                        ...
                        FROM
                            kn.kn_tk_saldo x 
                        WHERE
                            x.kode_tk = bb.kode_tk 
                            AND x.kode_perusahaan = bb.kode_perusahaan 
                        ) 
                    ) jml_saldo,
                    RANK ( ) OVER (
                        PARTITION BY aa.kode_tk,
                        aa.kpj,
                        aa.kode_perusahaan,
                        aa.kode_divisi 
                    ORDER BY
        ...
        <tbody>	
            ...
            while ($row = $DB->nextrow()){	
                ...
                if($row['PATH_URL'] != ''){
                    ...
                    $btn_download_template      = '<input type="button" onclick="cetakDokTemplate()" name="btnCetak" style="font-weight:bold; color:#0000CD;text-decoration: underline;cursor:pointer" title="Download Template Surat pernyataan peserta terverifikasi tidak memliki kpj" value="Download"/>';
                }
...
<div class="form-row_kiri" width="100%">
    ...
    <?php if(($ses_reg_role == '3' || $ses_reg_role == '4' || $ses_reg_role == '27' || $ses_reg_role == '7' || $ses_reg_role == '8'  || $ses_reg_role == '9' || $ses_reg_role == '90' || $ses_reg_role == '360') && $p_viewfrom == '' ){?>
    ...
    <a id="btn_close_detail" href="http://<?=$HTTP_HOST;?>/mod_pn/form/pn6001.php?mid=<?=$mid;?>"><input type="button" style="width: 100px;" class="btn green" value="Tutup" /></a> 
</div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        var form = "<?=$_GET['form']?>"
        if (form == 'tdl') {
            var url_pn6002 = "http://<?=$HTTP_HOST;?>/mod_pn/form/pn6002.php?mid=<?=$mid;?>"
            $('#btn_close_detail').attr('href', url_pn6002);
        }
        $('#btn_submit').hide();
        $('#btn_tolak').hide();
        $('#btn_save').hide();
        ...
        if(('<?=$ls_status_submit_tindak_lanjut;?>' == 'Y' || '<?=$ls_petugas_submit_tindak_lanjut;?>' != '') || '<?=$ls_status_batal;?>' == 'Y'){
            $('#btn_simpan').hide();
            ...
            $('#tb_keterangan').prop('disabled',true);
        ...
        var status_agenda = $('#detil_status').val()
        if (status_agenda == 'SUBMIT') {
            $('#detil_status').val('MENUNGGU APPROVAL')
        }
    });
    ...
    function hapus_kartu(kode_agenda, kode_tk){
        var tanya = confirm("Apakah anda yakin akan menghapus data ini ?");
         ...
         if(tanya === true) {
            ...
            $.ajax({
                type: 'POST',
                url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn6001_verifikasi_kpj_dengan_satu_nomor_keps_action.php?'+Math.random(),
                data: {
                    TYPE				    : 'HAPUS_KARTU',
    ...
    function downloadFileSmile(path_url){
        ...
        $.ajax({
            type: 'POST',
            url: "../ajax/pn6001_verifikasi_kpj_dengan_satu_nomor_keps_action.php?"+Math.random(),
            data: {
    ...
    function submit_koreksi(){
        ...
        if('<?=$_GET['kd_agenda'];?>' && parseInt('<?=$total_rows;?>') < 1){
        ...
        if(parseInt('<?=$total_rows;?>') < 1){
            alert('"Daftar Kartu Kepesertaan Tidak Diproses" tidak boleh kosong.');
            return;
        }
        
        // if(parseInt('<?=$total_rows;?>') >= parseInt('<?=$total_rows_daftar_kartu;?>')){
        ...
        for(i=1; i<='<?=$total_rows_dok;?>'; i++){
            ...
            if($('#dok_pernyataan'+i).val() == ''){
                alert('Lengkapi dokumen pendukung.')
        } else {            
    ...
    function submit_setuju(){
		...
		$.ajax({
			...
			success: function(data){
				...
				if (jdata.ret == 0){
                    window.parent.Ext.notify.msg('Penyimpanan data berhasil dengan kode agenda '+jdata.dataid+', session dilanjutkan...', jdata.msg);
                    window.location='pn6001.php?task=View&path='+path+'&kd_agenda='+jdata.dataid+'&kd_perihal_detil='+kd_perihal_detil+'&form=tdl';
...
function gw_newWindow(mypage, myname, w, h, scroll) { // confirm(gw_openwin_param1);
    ...
    function fl_js_get_lov_by(lov) { 
        if (lov == "NO_IDENTITAS") {
            NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/form/pn6001_verifikasi_kpj_dengan_satu_nomor_keps_lov.php?','',800,500,1);
        } 
    }
    ...
    function fl_js_get_lov_kartu(lov,kode_agenda) { 
        if (lov != "") {
            ...
            NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/form/pn6001_verifikasi_kpj_dengan_satu_nomor_keps_lov_tambah_kartu.php?'+params,'',800,500,1);
        } 
    }
    ...
    function fl_ls_get_lov_by_selected(lov, obj) {
        if (lov == "NOMOR_IDENTITAS") {
            if (obj.NOMOR_IDENTITAS != '') {
                var new_location = window.location.href.replace("#", "") + '&kdf=' + obj.NOMOR_IDENTITAS + '&kdtk=' + obj.KODE_TK;
    ...
    function cetakDokTemplate()
    {
        var file_path = '../../downloads/Surat pernyataan peserta terverifikasi tidak memliki kpj.pdf';

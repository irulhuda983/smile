<?php
$pagetype = "form";
$gs_pagetitle = "PN5014 - ENTRI PENGAJUAN KEGIATAN PROMOTIF/PREVENTIF";
require_once "../../includes/header_app_nosql.php";	
$mid = $_REQUEST["mid"];
$gs_kode_segmen = "PU";

/* ============================================================================
Ket : Form ini digunakan sebagai form input Entri Pengajuan Kegiatan Promotif/Preventif
...
<div id="actmenu" >
	<div id="actbutton">
		...
		?>
			...
			<div style="float:left;">
				<div class="icon">
					<a id="btn_close" href="http://<?=$HTTP_HOST;?>/mod_pn/form/pn5014.php?mid=<?=$mid;?>">
    ...
    <div id="formKiri" style="width:1000px">
		<form name="formreg" id="formreg" role="form" method="post">
			<div id="konten">
							...
							<tr>
								<td width="50%" valign="top">
									...
									<div class="form-row_kiri">
										...
										<a href="#" onclick="NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5014_lov_sub_jenis_kegiatan.php?p=pn5014.php&a=formreg&b=kode_jenis_kegiatan&d=nama_jenis_kegiatan&e=kode_sub_jenis_kegiatan&f=nama_sub_jenis_kegiatan&g='+document.getElementById('tgl_kegiatan').value,'',1000,600,1)">
											<img id="img_jenis_kegiatan" src="../../images/help.png" alt="Cari Sub Jenis Kegiatan" border="0" align="absmiddle">
										</a>
									</div>
									<div class="clear"></div>
									<div class="form-row_kiri">
				...
				<div id="formsplit">
					<fieldset>
						<legend>Data Pengajuan Kegiatan</legend>
						<table>
							<tr>
								<td width="50%" valign="top">
									<!-- start entri pengajuan kegiatan -->
				...
				<div id="formsplit">
					<fieldset>
						<legend>Daftar Pengajuan Kegiatan Promotif/Preventif</legend>	
						<div class="clear"></div>
					...
					<fieldset style="background: #ededed;">
						...
						<div class="form-row_kiri">
							<br>
							<input type="button" style="display:none" class="btn green" id="btncetakdaftarpengajuan" name="btncetakdaftarpengajuan" tabindex="41" value="CETAK DAFTAR PENGAJUAN" title="Klik Untuk Cetak Daftar Pengajuan">	
							&nbsp;
				...
				<div id="formsplit">
					<fieldset style="background: #ededed;">
						<br>
						<input type="button" name="btnsubmitentripengajuan" id="btnsubmitentripengajuan" class="btn green"  value="SUBMIT ENTRI PENGAJUAN" title="Klik untuk submit entri pengajuan kegiatan">
						&nbsp;
					</fieldset>
					<br>
					<div class="clear"></div>
        ...
        <script type="text/javascript">
			$(document).ready(function(){
				...
				$('#btn_view').click(function() {
					if(window.dataid != ''){
						window.location='pn5014.php?task=View&tasktk=View&dataid='+window.dataid+'&dataid2='+window.kodeperusahaan+'&mid=<?=$mid;?>';
					} else {
				...
				$('#btn_edit').click(function() {	
					if(window.dataid != ''){
						window.location='pn5014.php?task=Edit&tasktk=Edit&dataid='+window.dataid+'&dataid2='+window.kodeperusahaan+'&mid=<?=$mid;?>';
					} else {
						alert('Silahkan pilih terlebih dahulu data yang akan di proses!');
					}
				});
				...
				$('#btn_new').click(function() {
					NewWindow4('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5014_eligible.php?kode_user=<?php echo $ls_user_login?>&kode_kantor=<?php echo $ls_kode_kantor?>','pn501401 - Daftar Perusahaan Eligible','1024','600','yes');
				});
				...
				$("#btncetakdaftarpengajuan").click(function() {						
					cetakDaftarDokumenPdf("PNR500816","<?php echo $ls_kode_kantor; ?>");
				});
				
				$('#btnsubmitentripengajuan').attr('style','display:none');
				...
				<?PHP
					if($_REQUEST["task"] == "View"){
						...
						$.ajax({
							type: 'POST',
							url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5014_action.php?'+Math.random(),
				...
				<?PHP
					if($_REQUEST["task"] == "Edit"){
						...
						$.ajax({
							...
							success: function(data) {
								...
								if(jdata.ret == '0'){
									...
									if(jdata.data[0].NAMA_DETIL_KEGIATAN === null)
									{
										$('#btnsubmitentripengajuan').attr('style','display:none');
									}
									else
									{
										$('#btnsubmitentripengajuan').removeAttr('style','display:none');
									}
									
									if(jdata.data[0].TGL_SUBMIT_PENGAJUAN === null)
									{
										$('#btnsubmitentripengajuan').removeAttr('disabled');
										$('#btnsubmitentripengajuan').removeClass('disabled');
									}
									else
									{
										$('#btnsubmitentripengajuan').attr('disabled','disabled');
										$('#btnsubmitentripengajuan').addClass('disabled');

										$('#nama_kegiatan').attr('disabled','disabled');
										$('#nama_kegiatan').addClass('disabled');
										$('#tgl_kegiatan').attr('disabled','disabled');
										$('#tgl_kegiatan').addClass('disabled');
										$('#kode_jenis_kegiatan').attr('disabled','disabled');
										$('#kode_jenis_kegiatan').addClass('disabled');
										$('#latar_belakang_pengajuan').attr('disabled','disabled');
										$('#latar_belakang_pengajuan').addClass('disabled');
										$('#jumlah_tk').attr('disabled','disabled');
										$('#jumlah_tk').addClass('disabled');
										$('#jumlah_kasuskk').attr('disabled','disabled');
										$('#jumlah_kasuskk').addClass('disabled');
										$('#jenis_usaha').attr('disabled','disabled');
										$('#jenis_usaha').addClass('disabled');
						...
						$('#btn_save').click(function() {
							if($('#nama_kegiatan').val() != '' && $('#tgl_kegiatan').val() != '' && $('#kode_jenis_kegiatan').val() != '' && $('#kode_sub_jenis_kegiatan').val() != ''
								&& $('#latar_belakang_pengajuan').val() != '' && $('#jumlah_tk').val() != '' && $('#jumlah_kasuskk').val() != '' && $('#jenis_usaha').val() != ''){
								preload(true);
								$.ajax({
									type: 'POST',
									url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5014_action.php?'+Math.random(),
								...
								alert('Silahkan mengisi data mandatori sesuai dengan formulir pengajuan!');
							}
							
						});
						
						$('#btnsubmitentripengajuan').click(function() {
							submitPengajuan(window.dataid);
						});
				<?PHP
					};
				?>				
			...
			function submitPengajuan(p_kode_promotif){
			...
			function loadData(){
				...
				$.ajax({
					type: 'POST',
					url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5014_query.php?'+Math.random(),

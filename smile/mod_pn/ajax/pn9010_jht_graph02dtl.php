<?php
$pagetype = "report";
$gs_pagetitle = "Dashboard Pelayanan - Pembayaran Klaim JHT Nasional Per Kantor Wilayah";
...
{
	$sql = "select keyword keyword_sebab_klaim, keyword||' - '||nama_sebab_klaim nama_sebab_klaim from pn.pn_kode_sebab_klaim where kode_sebab_klaim = '$ls_kode_sebab_klaim' "; 		
  $DB->parse($sql);
  $DB->execute();
  $row = $DB->nextrow();
...
//get data from WS -----------------------------------------------------------
global $wsIp;
$ipDev  = $wsIp."/JSPN9010/GetDataJHTGraph02";
$url    = $ipDev;
$chId   = 'CORE';
...
$ls_graph_pie_val = "";
for($i=0;$i<ExtendedFunction::count($resultArray->JHT_GRAPH2);$i++)
{
  //grafik bar -----------------------------------------------------------------
	$ln_graph1_bar_val = 0;
  $ln_graph1_bar_val = $resultArray->JHT_GRAPH2[$i]->NOM_PEMBAYARAN == "" ? "0" : $resultArray->JHT_GRAPH2[$i]->NOM_PEMBAYARAN;
  $ln_graph1_bar_val_tot = $ln_graph1_bar_val_tot + (float)$ln_graph1_bar_val;
  $ls_graph1_bar_xAxisCategory1_temp .= "'".$resultArray->JHT_GRAPH2[$i]->NAMA_KANTOR_SINGKAT."',";
  $ls_graph1_bar_val_temp .= $ln_graph1_bar_val.",";
	
	//grafik pie -----------------------------------------------------------------
	$ls_graph_pie_val_x1 = $resultArray->JHT_GRAPH2[$i]->KODE_KANTOR;
  $ls_graph_pie_val_x2 = $resultArray->JHT_GRAPH2[$i]->NAMA_KANTOR_SINGKAT;
  $ln_graph_pie_val_x3 = $resultArray->JHT_GRAPH2[$i]->NOM_PEMBAYARAN;
	...
	<div class="content">
		<div class="container">
			...
			<div class="row">
				<div class="col-12">
					<!--<div class="page-title-box">-->
            <h5 class="page-title float-center"><b>KATEGORI PENGAMBILAN JHT : <?=$ls_nama_sebab_klaim;?></b></h5>
          <!--</div>-->	 
				</div>
			</div>
			<!-- end row -->
          ...
          <div class="row">
    				<div class="col-lg-12">
              <div class="card-box">
    						<h4 class="header-title m-t-0 m-b-30"><b>Pembayaran Klaim JHT <?=$ls_keyword_sebab_klaim;?> Per Kantor Wilayah</b></h4>
          ...
          <div class="row">
    				<div class="col-lg-12">
              <div class="card-box">
    						<h4 class="header-title m-t-0 m-b-30"><b>Komposisi Pembayaran Klaim JHT <?=$ls_keyword_sebab_klaim;?> Kantor Wilayah</b></h4> 
                <div class="widget-chart text-center">
          ...
          <div class="card-box">
						...
						<div class="row">	 					
							<h4 class="m-t-0 header-title"><b>Data Pembayaran Klaim JHT <?=$ls_keyword_sebab_klaim;?></b></h4>
										...
										for($i=0;$i<ExtendedFunction::count($resultArray->JHT_GRAPH2);$i++)			
                    {
                      ?>
                        ...
                        <td style="text-align:left">
													<a href="#" onClick="NewWindow4('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn9010_jht_graph02dtl.php?task=view&kode_kantor=<?=$resultArray->JHT_GRAPH2[$i]->KODE_KANTOR;?>&kode_segmen=<?=$ls_kode_segmen;?>&tgl=<?=$ld_tgl;?>&tgl_mmddyyyy=<?=$ld_tgl_mmddyyyy;?>&kode_sebab_klaim=<?=$ls_kode_sebab_klaim;?>&sender=pn9010.php&sender_mid=<?=$mid;?>','Drilldown',1250,680,'yes');" href="javascript:void(0);">
          									 <?=$resultArray->JHT_GRAPH2[$i]->NAMA_KANTOR_SINGKAT2;?>
          								</a>
												</td>
                        <td style="text-align:right"><?=number_format((float)$resultArray->JHT_GRAPH2[$i]->JML_KASUS,0,".",",");?></td>  									
                        <td style="text-align:right;"><?=number_format((float)$resultArray->JHT_GRAPH2[$i]->NOM_PEMBAYARAN,2,".",",");?></td>						
                      </tr>
                      <?							    							
                      //$i++;//iterasi i
											$ln_tot_kasus_jht  += (float)$resultArray->JHT_GRAPH2[$i]->JML_KASUS;
											$ln_tot_pembayaran_jht += (float)$resultArray->JHT_GRAPH2[$i]->NOM_PEMBAYARAN;
                    }	//end while

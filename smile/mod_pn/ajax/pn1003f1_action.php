...
//form serialize
$ls_kode_diagnosa			= $_POST['c_kode_diagnosa'];
$ls_kode_diagnosa_detil	= $_POST['v_kode_diagnosa_detil'];
$ls_nama_diagnosa_detil	= $_POST['v_nama_diagnosa_detil'];
...
if($ls_type=="new"){
	$sql = "insert into sijstk.pn_kode_diagnosa_detil (
				kode_diagnosa_detil,
				kode_diagnosa,
				nama_diagnosa_detil,
                no_urut,
                ...
                petugas_rekam)
			values(
				'$ls_kode_diagnosa_detil',
				'$ls_kode_diagnosa',
				'$ls_nama_diagnosa_detil',
			    '$ls_no_urut',
			    '$ls_status_nonaktif',
	...
	$sql = "update sijstk.pn_kode_diagnosa_detil
			   set nama_diagnosa_detil = '$ls_nama_diagnosa_detil',
			       no_urut = '$ls_no_urut',
			 ...
			 where kode_diagnosa_detil = '$ls_kode_diagnosa_detil'";
	$DB->parse($sql);
	if($DB->execute()){ 
	...
	$sql="select kode_diagnosa_detil,
		       nama_diagnosa_detil,
		       no_urut,
		       status_nonaktif,
		       to_char(tgl_nonaktif,'dd/mm/yyyy') tgl_nonaktif,
		       keterangan
		  from sijstk.pn_kode_diagnosa_detil
		 where kode_diagnosa_detil='".$ls_search_keyword."'";
    $DB->parse($sql);
    if($DB->execute()){ 
	...
	$sql="delete from sijstk.pn_kode_diagnosa_detil
      		where kode_diagnosa_detil ='".$ls_kode_diagnosa_detil."'";
    $DB->parse($sql);
	if($DB->execute()){ 
	...
	if($order[0]['column']=='0'){
	    ...
	    $order = "ORDER BY KODE_DIAGNOSA_DETIL ";
	}else if($order[0]['column']=='2'){
	    $order = "ORDER BY NAMA_DIAGNOSA_DETIL ";
	...
	$sql = "";
	$sql="select * from (select rownum no, kode_diagnosa_detil,
		       nama_diagnosa_detil,
		       status_nonaktif,
		       tgl_nonaktif,
		       keterangan
		  from sijstk.pn_kode_diagnosa_detil
		 where 1 = 1 and kode_diagnosa='$ls_kode_diagnosa'  ".$condition." ".$order.")
		 where 1=1 and no between ".$start." and ".$end;

	$queryTotal = "SELECT COUNT (1) FROM SIJSTK.PN_KODE_diagnosa_detil WHERE 1=1 AND kode_diagnosa='$ls_kode_diagnosa' ".$condition;
	// print_r($sql);
	// print_r($querySql);
	...
	if($DB->execute()){ 
		...
		{
			$data['ACTION'] = '<button class="btn green" name="view_diagnosa_detil_'.$data['KODE_DIAGNOSA_DETIL'].'" id="view_diagnosa_detil_'.$data['KODE_DIAGNOSA_DETIL'].'" kode_diagnosa_detil="'.$data['KODE_DIAGNOSA_DETIL'].'"  title="Klik untuk view detil data"><i class="fa fa-search" aria-hidden="true"></i></button>'.' '.' <button class="btn green" name="edit_diagnosa_detil_'.$data['KODE_DIAGNOSA_DETIL'].'" id="edit_diagnosa_detil_'.$data['KODE_DIAGNOSA_DETIL'].'" kode_diagnosa_detil="'.$data['KODE_DIAGNOSA_DETIL'].'"  title="Klik untuk edit data"><i class="fa fa-pencil" aria-hidden="true"></i></button> <button class="btn hapus" name="hapus_diagnosa_detil_'.$data['KODE_DIAGNOSA_DETIL'].'" kode_diagnosa_detil="'.$data['KODE_DIAGNOSA_DETIL'].'" title="Klik untuk hapus data"><i class="fa fa-trash" aria-hidden="true"></i></button>
				';

<?php
//------------------------------------------------------------------------------
// Menu untuk display data histori pembayaran jp berkala
		...
		$.ajax({
			...
			success: function(data){
				try {
					...
					if (jdata.ret == 0) 
					{ 
						$("#span_page_title").html('PN5006 - HISTORI PEMBAYARAN MANFAAT PENSIUN BERKALA');
						$("#span_page_title_right").html('KODE KLAIM : '+v_kode_klaim);
						
						//set layout -------------------------------------------------------
						$('#formtab').load('../ajax/pn5069_view_jpn_pembayaranberkala.php', getValueArr(jdata.dataKonfByrBerkala));

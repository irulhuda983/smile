...
if ($TASK=='DOWNLOAD_DOK'){
    ...
    $sql_check_doc ="SELECT URL AS PATH_URL, NAMA_FILE FROM SIJSTK.PN_KLAIM_DOKUMEN where kode_klaim='$KODE_KLAIM' and kode_dokumen='$KODE_DOKUMEN'";

    $DB->parse($sql_check_doc);
    if($DB->execute()){
      if($row=$DB->nextrow()){
  ...
  if($ls_path_url!=""){
    $file = file_get_contents($wsIpStorage.$ls_path_url);
    header('Content-Description: File Transfer'); 
    header('Content-disposition: attachment;filename='.$ls_nama_file);
    echo $file;
  }else{

<?php
$pagetype = "report";
$gs_pagetitle = "Dashboard Pelayanan - Pembayaran Klaim JP Nasional Per Sebab Klaim";
...
if ($ls_jenis_grafik=="")
{
 	$ls_jenis_grafik = "graph02"; 
}

//graph01 : Pembayaran Klaim JP Per Kantor Wilayah
//graph02 : Pembayaran Klaim JP Per Sebab Klaim
														
if ($ls_jenis_grafik=="graph01")
{
	echo "<script language=\"JavaScript\" type=\"text/javascript\">";
  echo "window.location.replace('../ajax/pn9010_jpn_graph01.php?task=view&kode_kantor=$ls_kode_kantor&kode_segmen=$ls_kode_segmen&tgl=$ld_tgl&tgl_mmddyyyy=$ld_tgl_mmddyyyy&sender=pn9010.php&sender_mid=$mid');";
  echo "</script>";	
}
...
<?
//1. grafik pembayaran klaim jht berdasarkan sebab klaim -----------------------
//get data from WS -----------------------------------------------------------
global $wsIp;
...
$ls_graph_pie_val = "";
for($i=0;$i<ExtendedFunction::count($resultArray->JPN_GRAPH01);$i++)
{
  //grafik bar -----------------------------------------------------------------
	$ln_graph1_bar_val = 0;
  $ln_graph1_bar_val = $resultArray->JPN_GRAPH01[$i]->NOM_PEMBAYARAN == "" ? "0" : $resultArray->JPN_GRAPH01[$i]->NOM_PEMBAYARAN;
  $ln_graph1_bar_val_tot = $ln_graph1_bar_val_tot + (float)$ln_graph1_bar_val;
  $ls_graph1_bar_xAxisCategory1_temp .= "'".$resultArray->JPN_GRAPH01[$i]->NAMA_SEBAB_KLAIM."',";
  $ls_graph1_bar_val_temp .= $ln_graph1_bar_val.",";
	
	//grafik pie -----------------------------------------------------------------
	$ls_graph_pie_val_x1 = $resultArray->JPN_GRAPH01[$i]->KODE_SEBAB_KLAIM;
  $ls_graph_pie_val_x2 = $resultArray->JPN_GRAPH01[$i]->NAMA_SEBAB_KLAIM;
  $ln_graph_pie_val_x3 = $resultArray->JPN_GRAPH01[$i]->NOM_PEMBAYARAN;
  $ls_graph_pie_val_temp .= "{ name: '{$ls_graph_pie_val_x2}', y: {$ln_graph_pie_val_x3}, url: '{$ls_link_call_page2}kode_segmen={$ls_kode_segmen}&tgl={$ld_tgl}&kode_sebab_klaim={$ls_graph_pie_val_x1}' },";
}
	...
	<div class="content">
		<div class="container">
          ...
          <div class="row">
    				<div class="col-lg-12">
              <div class="card-box">
    						<h4 class="header-title m-t-0 m-b-30"><b>Pembayaran Klaim JP Per Sebab Klaim</b></h4>
          ...
          <div class="row">
    				<div class="col-lg-12">
              <div class="card-box">
    						<h4 class="header-title m-t-0 m-b-30"><b>Komposisi Pembayaran Klaim JP Per Sebab Klaim</b></h4> 
                <div class="widget-chart text-center">
                  ...
                  <div class="form-group m-b-20">
                    ...
                    <div>
                        <select class="selectpicker show-tick" id="jenis_grafik" name="jenis_grafik" data-style="btn-dafault" onchange="this.form.submit()">
                          ...
                          {
                          ?>
                          Pembayaran Klaim JP Per Kantor Wilayah
                          <? 
                          }else
                          {
                          ?>
                          Pembayaran Klaim JP Per Kantor Cabang/KCP
                          <? 
                          }
                          ?>				
                          </option>
                          <option value="graph02" <?=$graph02;?>>Pembayaran Klaim JP Berdasarkan Sebab Klaim</option>	
                        </select>
                    </div>
                	</div>
              </div>						
						...
						<div class="row">	 					
							...
							<div class="table-responsive">
								<table class="table table-hover m-0 mails table-actions-bar">
									<thead>
                    <tr>
                      <th colspan="3" style="text-align:center">PEMBAYARAN KLAIM JP</th>
                    </tr>
										<tr>
                      <th style="text-align:center">Sebab Klaim</th>
                    ...
                    for($i=0;$i<ExtendedFunction::count($resultArray->JPN_GRAPH01);$i++)
                    {
                      ?>
                        ...
                        <td style="text-align:left">
													<a href="#" onClick="NewWindow4('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn9010_jpn_graph02dtl.php?task=view&kode_kantor=<?=$ls_kode_kantor;?>&kode_segmen=<?=$ls_kode_segmen;?>&tgl=<?=$ld_tgl;?>&tgl_mmddyyyy=<?=$ld_tgl_mmddyyyy;?>&kode_sebab_klaim=<?=$resultArray->JPN_GRAPH01[$i]->KODE_SEBAB_KLAIM;?>&sender=pn9010.php&sender_mid=<?=$mid;?>','Drilldown',1250,680,'yes');" href="javascript:void(0);">
          									 <?=$resultArray->JPN_GRAPH01[$i]->NAMA_SEBAB_KLAIM;?>
          								</a>
												</td>
                        <td style="text-align:right"><?=number_format((float)$resultArray->JPN_GRAPH01[$i]->JML_KASUS,0,".",",");?></td>  									
                        <td style="text-align:right;"><?=number_format((float)$resultArray->JPN_GRAPH01[$i]->NOM_PEMBAYARAN,2,".",",");?></td>						
                      </tr>
                      <?							    							
                      //$i++;//iterasi i
											$ln_tot_kasus_jht  += (float)$resultArray->JPN_GRAPH01[$i]->JML_KASUS;
											$ln_tot_pembayaran_jht += (float)$resultArray->JPN_GRAPH01[$i]->NOM_PEMBAYARAN;
                    }	//end while

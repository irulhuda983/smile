...
<div id="formKiri"> 
	<form name="form_1" id="form_1" role="form" method="post">
		<fieldset>
			<legend>Form Tipe Penerima </legend>
			<div class="clear"></div>
...
?>
	<div id="formKiri">
		<fieldset><legend>Data Setup Jenis Penerima</legend>
			...
			<table class="table table-striped table-bordered row-border hover" id="table_1" cellspacing="0" width="100%">
				<thead>
					<tr>
						...
						<th scope="col" width="5%" class="align-left">Nama Tipe Penerima</th>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		?>	
			window.kode_tipe_penerima = '<?=$_REQUEST["id"];?>';
			window.type = '<?=$_REQUEST["task"];?>';
			$.ajax({
				type: 'POST',
				url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn1005_action.php?'+Math.random(),
				data: { 
						type : "view",
						keyword : window.kode_tipe_penerima
		...
		$('#btn_view').click(function() {
			if($('input[name^="CHECK"][type=checkbox]:checked').length>0)
			{
				if($('input[name^="CHECK"][type=checkbox]:checked').length<2){
					window.kode_tipe_penerima = $('input[name^="CHECK"][type=checkbox]:checked').attr("kode_tipe_penerima");
					window.location='pn1005.php?task=view&id='+window.kode_tipe_penerima;
				}else{
		...
		$('#btn_edit').click(function() {
			if($('input[name^="CHECK"][type=checkbox]:checked').length>0)
			{
				if($('input[name^="CHECK"][type=checkbox]:checked').length<2){	
					window.kode_tipe_penerima = $('input[name^="CHECK"][type=checkbox]:checked').attr("kode_tipe_penerima");
						window.location='pn1005.php?task=edit&id='+window.kode_tipe_penerima;
				}else{
		...
		$("#btn_delete").click(function() {
			if($('input[name^="CHECK"][type=checkbox]:checked').length>0)
			{
				if($('input[name^="CHECK"][type=checkbox]:checked').length<2){	
					window.kode_tipe_penerima = $('input[name^="CHECK"][type=checkbox]:checked').attr("kode_tipe_penerima");
					deleteData(window.kode_tipe_penerima);
				}else{
					alert('Untuk melakukan edit hanya bisa dipilih satu record');
				}
			} else {
		...
		$('#btn_new').click(function() {
			window.location='pn1005.php?task=new';
		});
				...
				$.ajax({
					...
					success: function(data) {
						...
						if(jdata.ret == '0'){
							...
							window.location='pn1005.php?task=view&id='+$('#v_kode_tipe_penerima').val();
						} else {
...
function loadTable(){
	...
	window.table_1 = $('#table_1').DataTable({
		...
		"destroy"			: true,
        "ajax"				: {
        	"url"	: "http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn1005_action.php",
        	"type": "POST",
        	"data" : function(e) { 
        		e.type = 'query';
...
function deleteData(kode_tipe_penerima){
...
function detailTipePenerima(kode_tipe_penerima){
	win = new window.parent.Ext.window.Window({
          ...
          modal: true,
          autoLoad: {
            url: 'http://<?=$HTTP_HOST;?>/mod_pn/form/pn1001a.php?id='+kode_tipe_penerima+"&"+Math.random(),
            scripts: true
          },
          dockedItems: [{
            xtype: 'toolbar',

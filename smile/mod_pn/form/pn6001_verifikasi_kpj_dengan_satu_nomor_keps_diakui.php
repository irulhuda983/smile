...
} else if($kd_perihal_detil == 'PP0306'){
    $prm_perihal_detil  = 'PP0305';
}
...
<script type="text/javascript">
    ...
    function batal_koreksi(){
        ...
        if(tanya === true) {
            ...
            $.ajax({
                type: 'POST',
                url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn6001_verifikasi_kpj_dengan_satu_nomor_keps_diakui_action.php?'+Math.random(),
                data: {
                    TYPE				    : 'BATAL',
...
function gw_newWindow(mypage, myname, w, h, scroll) { // confirm(gw_openwin_param1);
    ...
    function fl_js_get_lov_by(lov) { 
        ...
        if (lov == "NO_IDENTITAS") {
            NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/form/pn6001_verifikasi_kpj_dengan_satu_nomor_keps_diakui_lov.php?'+params,'',800,500,1);
        } 
    }

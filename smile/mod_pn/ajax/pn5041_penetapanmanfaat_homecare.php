...
/*--------------------- Form History -----------------------------------------
File: pn5041_penetapanmanfaat_homecare.php

Deskripsi:
-----------
File ini dipergunakan untuk entry manfaat homecare
...
<div id="formframe">
  <div id="formKiri" style="width:1000px">
    ...
    <table id="mydata2" cellspacing="0" style="text-align:center;width:89%;">
      <tbody>			

				<tr>
        	<th colspan="8"><hr style="border:0; height:1px; background-image: linear-gradient( to right, rgba(0,0,0,0), rgba(0,0,0,0.80), rgba(0,0,0,0));"/></th>	
        </tr>	

        <tr>
          <th style="text-align:center;font: 12px Arial, Helvetica, sans-serif;" colspan="3">&nbsp;</th>
          <th style="text-align:center;font: 12px Arial, Helvetica, sans-serif;" colspan="4">Penetapan Nilai Manfaat</th>
        ...
        <tr>
          ...
          <th style="text-align:center;font: 12px Arial, Helvetica, sans-serif;width:150px;">Tipe Penerima</th>
					<th style="text-align:center;font: 12px Arial, Helvetica, sans-serif;width:150px;">Periode Homecare</th>
          <th style="text-align:center;font: 12px Arial, Helvetica, sans-serif;width:70px;">Biaya Diajukan</th>
					<th style="text-align:center;font: 12px Arial, Helvetica, sans-serif;width:70px;">Biaya Diverifikasi</th>
        ...
        {			
          $sql = "select 
                      ...
                      a.kode_tipe_penerima, c.nama_tipe_penerima, a.kd_prg, a.nom_biaya_diajukan, a.nom_biaya_diverifikasi, a.nom_biaya_disetujui,
											to_char(a.homecare_tgl_awal,'dd/mm/yyyy') homecare_tgl_awal, to_char(a.homecare_tgl_akhir,'dd/mm/yyyy') homecare_tgl_akhir,  
          ...
          while ($row = $DB->nextrow())
          {
            ...
            <?echo "<tr bgcolor=#".($i%2 ? "f3f3f3" : "ffffff").">";?>								
              ...
              <td style="text-align:center"><?=$row["NAMA_TIPE_PENERIMA"];?></td>
							<td style="text-align:center"><?=$row["HOMECARE_TGL_AWAL"];?> s/d <?=$row["HOMECARE_TGL_AKHIR"];?></td>
              <td style="text-align:right"><?=number_format((float)$row["NOM_BIAYA_DIAJUKAN"],2,".",",");?></td>  
							<td style="text-align:right"><?=number_format((float)$row["NOM_BIAYA_DIVERIFIKASI"],2,".",",");?></td>									
              ...
              <td>
							...
							{
  							?>
                <a href="#" onClick="NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5041_penetapanmanfaat_homecareentry.php?task=view&kode_klaim=<?=$ls_kode_klaim;?>&kode_manfaat=<?=$ls_kode_manfaat;?>&no_urut=<?=$row["NO_URUT"];?>&form_penetapan=<?=$ls_form_penetapan;?>&sender=pn5041_penetapanmanfaat_homecare.php','Entry Rincian Manfaat',850,600,'no')"><img src="../../images/check.png" border="0" alt="Ubah Divisi" align="absmiddle" />&nbsp;View </a>
  							<?
							}else
							{
  							?>
                <a href="#" onClick="window.location.replace('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5041_penetapanmanfaat_homecareentry.php?task=edit&kode_klaim=<?=$ls_kode_klaim;?>&kode_manfaat=<?=$ls_kode_manfaat;?>&no_urut=<?=$row["NO_URUT"];?>&form_penetapan=<?=$ls_form_penetapan;?>&sender=pn5041_penetapanmanfaat_homecare.php','Entry Rincian Manfaat',850,600,'no')"><img src="../../images/check.png" border="0" alt="Ubah Divisi" align="absmiddle" />&nbsp;Edit </a>	|
                <a href="javascript:confirmDelete('?delete_manfaatrinci=DEL&kode_klaim=<?=$ls_kode_klaim;?>&kode_manfaat=<?=$ls_kode_manfaat;?>&no_urut=<?=$row["NO_URUT"];?>&form_penetapan=<?=$ls_form_penetapan;?>&sender=pn5041_penetapanmanfaat_homecare.php')"><img src="../../images/minus.png" alt="Delete" border="0" align="absmiddle" title="Delete">Delete</a>										
  							<?							
							}
							?>							
              </td>								
            ...
            $i++;//iterasi i
						...
						$ln_tot_biaya_diverifikasi  += $row["NOM_BIAYA_DIVERIFIKASI"];
      ...
      <tr>
      	<th colspan="8"><hr style="border:0; height:1px; background-image: linear-gradient( to right, rgba(0,0,0,0), rgba(0,0,0,0.50), rgba(0,0,0,0));"/></th>	
      </tr>
      ...
      <tr>
        ...
        <td style="text-align:center"><?=number_format((float)$ln_tot_biaya_diajukan,0,".",",");?></td>
				<td style="text-align:center"><?=number_format((float)$ln_tot_biaya_diverifikasi,0,".",",");?></td>  									
        <td style="text-align:right"><?=number_format((float)$ln_tot_biaya_disetujui,2,".",",");?></td>				
    			...
    			{
      			?>
               <a href="#" onClick="window.location.replace('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5041_penetapanmanfaat_homecareentry.php?task=new&kode_klaim=<?=$ls_kode_klaim;?>&kode_manfaat=<?=$ls_kode_manfaat;?>&form_penetapan=<?=$ls_form_penetapan;?>&sender=pn5041_penetapanmanfaat_homecare.php','Entry Rincian Manfaat',850,600,'no');" href="javascript:void(0);"><img src="../../images/plus.png" border="0" alt="Tambah" align="absmiddle" />&nbsp;Entry</a>
            <?
    			}
    			?>									
        </td>																					

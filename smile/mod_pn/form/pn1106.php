<?php
$pagetype = "form";
$gs_pagetitle = "Setup - Balai Harta Peninggalan (BHP)";
require_once "../../includes/header_app_nosql.php";	
$mid = $_REQUEST["mid"]; 
$php_file_name = "pn1106";
//$gs_kode_segmen = "PU";
/* ============================================================================
...
<div id="actmenu" style="padding-top:10px;">
  ...
  <div style="float:left;">
    ...
    <?php  }  ?>
    <div style="float:left;"><div class="icon">
      <a id="btn_close" href="<?=$php_file_name;?>.php?mid=<?=$mid;?>"><img src="http://<?=$HTTP_HOST;?>/images/file_cancel.gif" align="absmiddle" border="0"> Close</a> 
    </div></div>
    <?php }  else   {  ?>
    <div style="float:left;"><div class="icon">
    ...
    <div style="float:left;"><div class="icon">
      <a id="btn_new" href="<?=$php_file_name;?>.php?task=New&mid=<?=$mid;?>"><img src="http://<?=$HTTP_HOST;?>/images/app_form_add.png" align="absmiddle" border="0"> New</a>
    </div></div> 
    <div style="float:left;"><div class="icon">
...
<div>
<span class="gw_tab_title"  id="t1" >Balai Harta Peninggalan (BHP)</span>
</div>
...
$(document).ready(function(){
  $('#btn_edit').click(function() {
    if(window.dataid=='')
      ...
      window.location ='<?=$php_file_name;?>.php?task=Edit&dataid='+window.dataid+'&mid=<?=$mid;?>';
  });
  $('#btn_del').click(function() { 
    if(window.dataid=='')
      alert("Pilih data yang akan dihapus terlebih dahulu!");
    else if(confirm('Hapus data?'))
       delData(window.dataid);
  });
  $('#btn_view').click(function() {
    if(window.dataid=='')
      alert("Pilih data yang akan diedit terlebih dahulu!");
    else
      window.location ='<?=$php_file_name;?>.php?task=View&dataid='+window.dataid+'&mid=<?=$mid;?>';
  });
});
function delData(p_id)
{
  preload(true);
  $.ajax({
    type: 'POST',
    url: "../ajax/<?=$php_file_name;?>_action.php?"+Math.random(),
    data: {formregact:'Delete',kode:p_id},

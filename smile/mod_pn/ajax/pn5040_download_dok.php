...
if ($TASK=='DOWNLOAD_DOK'){
  ...
  $sql_SelectBlob="SELECT NAMA_FILE, DOC_FILE FROM SIJSTK.PN_KLAIM_DOKUMEN WHERE kode_klaim= :P_KODE_KLAIM and kode_dokumen= :P_KODE_DOKUMEN"; 

  $statement=oci_parse($DB->conn,$sql_SelectBlob); 
  oci_bind_by_name($statement, ':P_KODE_KLAIM', $KODE_KLAIM);
  oci_bind_by_name($statement, ':P_KODE_DOKUMEN', $KODE_DOKUMEN);

  ...
  <div id="actmenu">
  	<div id="actbutton">
        ...
        ?>        
        <div style="float:left;"><div class="icon">
        	<a id="btn_close" href="http://<?=$HTTP_HOST;?>/mod_pn/form/pn5036.php?mid=<?=$mid;?>"><img src="http://<?=$HTTP_HOST;?>/images/file_cancel.gif" align="absmiddle" border="0"> Close</a> 
        </div></div>
        <?PHP
      } 
  		else 
  ...
  {
    //cek kelayakan ----------------------------------------------------------
    $qry = "BEGIN SIJSTK.P_PN_pn5036.X_CEK_KELAYAKAN('$ls_kode_klaim','$username',:p_sukses,:p_mess);END;";											 	
    $proc = $DB->parse($qry);				
  ...
  {
   	//submit data klaim --------------------------------------------------------																					  
		$qry = "BEGIN SIJSTK.P_PN_pn5036.X_POST_SUBMIT('$ls_kode_klaim','$username',:p_sukses,:p_mess);END;";											 	
    $proc = $DB->parse($qry);				
    oci_bind_by_name($proc, ":p_sukses", $p_sukses,32);
    oci_bind_by_name($proc, ":p_mess", $p_mess,1000);
    $DB->execute();				

                ...
                <table id="tblMnf" width="100%" class="table-data2">
                  <thead>								
                    <tr>
                      ...
                      <th style="text-align:center;" colspan="5" class="hr-single-bottom">PENETAPAN NILAI MANFAAT URAIAN CACAT</th>
                      <th style="text-align:center;" colspan="2"></th>
                    </tr>					
                    <tr class="hr-double-bottom">
                      ...
                      <th style="text-align:center;">Uraian Cacat</th>
            					...
            					<th style="text-align:center;">Biaya Disetujui</th>
                    ...
                    <tr class="nohover-color">
                    	<td colspan="9" style="text-align: center;">-- data tidak ditemukan --</td>
									...
									<tfoot>
                    <tr>
                      <td style="text-align:right;" colspan="6"><i>Total Manfaat:</i>
		...
		$.ajax({
			...
			success: function(data){
				try {
					...
					if (jdata.ret == 0) 
					{ 
						$("#span_page_title").html('PN5030 - RINCIAN MANFAAT - URAIAN CACAT');
            ...
            { 
              for(var i = 0; i < jdata.data.DATA.length; i++)
              {
                ...
                html_mnf += '<td style="text-align: center;">' + getValue(jdata.data.DATA[i].NAMA_TIPE_PENERIMA) + '</td>';
								html_mnf += '<td style="text-align: center;">' + getValue(jdata.data.DATA[i].CACAT_NAMA_KEADAAN) + '</td>';
								html_mnf += '<td style="text-align: center;white-space:normal;word-wrap:break-word;">' + getValue(jdata.data.DATA[i].NAMA_MANFAAT_DETIL) + '</td>';
                html_mnf += '<td style="text-align: right;">' + format_uang(getValueNumber(jdata.data.DATA[i].CACAT_PERSEN_DOKTER)) + '</td>';
								html_mnf += '<td style="text-align: right;">' + format_uang(getValueNumber(jdata.data.DATA[i].CACAT_PERSEN_TABLE)) + '</td>';
              ...
              if (html_mnf == "") {
                ...
                html_mnf += '<td colspan="9" style="text-align: center;">-- data tidak ditemukan --</td>';
                html_mnf += '</tr>';
              }
...
<script language="javascript">
	...
	{		
		var c_mid = '<?=$mid;?>';
		showForm('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5069_view_penetapanmanfaat_uraiancacatrinci.php?kode_klaim='+p_kode_klaim+'&kode_manfaat='+p_kode_manfaat+'&no_urut='+p_no_urut+'&mid='+c_mid+'','',980,550,'yes');
	}
</script>

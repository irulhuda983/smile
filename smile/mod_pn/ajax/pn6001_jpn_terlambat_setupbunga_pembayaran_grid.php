<?
/* ============================================================================
Ket : Form ini digunakan untuk tab Biaya dan Santunan Koreksi Klaim
    ...
    <table id="tblrincian1" width="95%" cellspacing="0" border="0" align="center" bordercolor="#C0C0C0" background-color= "#ffffff">
      <tbody>	
        ...
        {			
          $sql = "select
                 ...
                 nama_penerima, kode_bank_penerima, bank_penerima, no_rekening_penerima, npwp, 
                 nom_manfaat_gross, nom_pph, nom_pembulatan, nom_netto, nom_sudah_bayar, nom_sisa, kode_pembayaran,
				 status_bayar, kode_kantor_pembayar,
				 (select kode_pointer_asal from pn.pn_klaim b where b.kode_klaim=a.kode_klaim) kode_pointer_asal	 
          ...
          ?>
            <?echo "<tr bgcolor=#".($i%2 ? "f3f3f3" : "ffffff").">";?>	
              <td style="text-align:center; font: 10px Verdana, Arial, Helvetica, sans-serif;"><?=$n;?></td>
							...
							<td style="text-align:center;">
  							<?
  							if ($row['NOM_SISA']>0 && ($_GET['form']=='tdlbyr'))
  							{
  							?>
								<a href="#" onClick="NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn6001_jht_terlambat_setupbunga_pembayaran_entry.php?task=New&kode_agenda=<?=$row['KODE_AGENDA'];?>&kode_klaim=<?=$row['KODE_KLAIM'];?>&kode_tipe_penerima=<?=$row["KODE_TIPE_PENERIMA"];?>&kd_prg=<?=$row["KD_PRG"];?>&form_root=<?=$ls_form_root;?>&root_sender=pn6001.php&sender=pn6001_jht_terlambat_setupbunga_pembayaran_grid.php&id_pointer_asal=<?=$ls_id_pointer_asal;?>&kode_pointer_asal=<?=$ls_kode_pointer_asal;?>&tglawaldisplay=<?=$ld_tglawaldisplay;?>&tglakhirdisplay=<?=$ld_tglakhirdisplay;?>','Entry Pembayaran Koreksi Klaim',850,600,'no')"><img src="../../images/check.png" border="0" alt="Entry Pembayaran" align="absmiddle" />&nbsp;Entry </a>									
  							<?
  							}
  							?>
  							<?
  							if ($row['KODE_PEMBAYARAN']!="")
  							{
    							?>
                  |
									<a href="#" onClick="NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn6001_jht_terlambat_setupbunga_pembayaran_entry.php?task=view&kode_pembayaran=<?=$row["KODE_PEMBAYARAN"];?>&form_root=<?=$ls_form_root;?>&root_sender=pn6001.php&sender=pn6001_jht_terlambat_setupbunga_pembayaran_grid.php&id_pointer_asal=<?=$ls_id_pointer_asal;?>&kode_pointer_asal=<?=$ls_kode_pointer_asal;?>&tglawaldisplay=<?=$ld_tglawaldisplay;?>&tglakhirdisplay=<?=$ld_tglakhirdisplay;?>','View Pembayaran Klaim',850,600,'no')"><img src="../../images/check.png" border="0" alt="Entry Pembayaran" align="absmiddle" />&nbsp;View </a>
									<a href="#" onClick="NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn6001_jht_terlambat_setupbunga_pembayaran_cetak.php?kode_klaim=<?=$row["KODE_KLAIM"];?>&kode_pembayaran=<?=$row["KODE_PEMBAYARAN"];?>&mid=<?=$mid;?>&tglawaldisplay=<?=$ld_tglawaldisplay;?>&tglakhirdisplay=<?=$ld_tglakhirdisplay;?>','Cetak Penetapan Koreksi Klaim',700,480,'no')"><img src="../../images/printx.png" border="0" alt="Entry Pembayaran" align="absmiddle" style="height:20px;"/>&nbsp;Cetak </a>									
    							<?
  							}
  							?>								
              </td>
            ...
            $i++;//iterasi i
						...
						$ls_kode_bank_penerima_x =  $row["KODE_BANK_PENERIMA"];
						$ls_kode_tipe_penerima_x =  $row["KODE_TIPE_PENERIMA"];
						$ls_kode_kantor_pembayar_x =  $row["KODE_KANTOR_PEMBAYAR"];
						$ls_kode_klaim_x =  $row["KODE_KLAIM"];
						$ls_kode_pointer_asal_x =  $row["KODE_POINTER_ASAL"];
          }	//end while
          $ln_dtl=$i;
        }						
        ?>									             																
      ...
      <tr>
        <td style="text-align:right;" colspan="7"><i>Total Keseluruhan :<i>
		  ...
		  <input type="hidden" id="kode_tipe_penerima_x" name="kode_tipe_penerima_x" value="<?=$ls_kode_tipe_penerima_x;?>">
		  <input type="hidden" id="kode_bank_penerima_x" name="kode_bank_penerima_x" value="<?=$ls_kode_bank_penerima_x;?>">
		  <input type="hidden" id="kode_kantor_pembayar_x" name="kode_kantor_pembayar_x" value="<?=$ls_kode_kantor_pembayar_x;?>">
		  <input type="hidden" id="kode_klaim_x" name="kode_klaim_x" value="<?=$ls_kode_klaim_x;?>">
		  <input type="hidden" id="kode_pointer_asal_x" name="kode_pointer_asal_x" value="<?=$ls_kode_pointer_asal_x;?>">

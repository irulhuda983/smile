...
else if ($ls_type == "SaveMnfBeasiswaPP82")
{
  ...
  }else
  {
	...
	if ($ls_sukses == "1")
	{
		...
		$ls_mess = "Data Penetapan Beasiswa berhasil disimpan, session dilanjutkan..";
...
//get max tahun beasiswa permenaker RI no 4 tahun 2023--------------------------
else if($ls_type == "fjq_ajax_val_get_max_thn_beasiswa")
{	
	$ls_kode_klaim 				= $_POST["v_kode_klaim"];
	$ls_jenis 			      = $_POST["v_jenis"];
  $ls_jenjang 			    = $_POST["v_jenjang"];
	$ls_nik_penerima 			= $_POST["v_nik_penerima"];
  ...
  $ipDev ="";
  global $wsIp;
  $ipDev  	= $wsIp."/JSPN5041/CheckMaxTahunBeasiswa";
  $url    	= $ipDev;
  ...
  // set POST params -----------------------------------
    ...
    'reqId'				 =>$gs_kode_user,
    'KODE_KLAIM'	 =>$ls_kode_klaim,
    'BEASISWA_NIK_PENERIMA' =>$ls_nik_penerima,
    'JENIS'			   => $ls_jenis,
    'JENJANG'		 =>$ls_jenjang
  );
  ...
  //end get data from ws tipe penerima -----------------------------------------
    ...
    echo json_encode($resultArray_dt);		

...
if($action=='New' || $action=='Edit' )
{
    ...
    $ls_alamat_ntr          = strtoupper($_POST["alamat"]);
    ...
    $ls_alamat_pemilik_ntr = strtoupper($_POST["alamat_pemilik"]);
    ...
    $ls_kelas_perawatan     = strtoupper($_POST["kelas_perawatan"]);
    $ls_bagian_pic          = strtoupper($_POST["bagian_pic"]);
    $ls_kelas_perawatan     = $ls_kode_jenis=='J02'?$ls_kelas_perawatan:'';
    $ls_handphone_pic_kacab   = strtoupper($_POST["handphone_pic_kacab"]);
    $ls_latitude   = strtoupper($_POST["latitude"]);
    $ls_longitude  = strtoupper($_POST["longitude"]);
}
if($action=='New')
{ 
    $ls_kode_new_faskes="";
    ...
    if($ls_daftar=='Y')
        ...
        KELAS_PERAWATAN,BAGIAN_PIC,NO_FASKES,KODE_PIC_CABANG,KONTAK_PIC_CABANG,LATITUDE,LONGITUDE)
        values('{$ls_kode_new_faskes}','{$ls_kode_tipe}','{$ls_kode_kantor}','{$ls_kode_pembina}'
            ...
            ,'{$ls_bankcode}','{$ls_namarek}','{$ls_norek}',sysdate,'{$_SESSION["USER"]}',sysdate,'{$_SESSION["USER"]}','{$ls_telepon_area_pic}',
            '{$ls_telepon_pic}','{$ls_telepon_ext_pic}','{$ls_kelas_perawatan}','{$ls_bagian_pic}','{$ls_kode_new_faskes}','{$kode_pic_cabang}','{$ls_handphone_pic_kacab}','{$ls_latitude}','{$ls_longitude}')";
    else
        $sql = "insert into {$schema}.tc_faskes(KODE_FASKES, KODE_TIPE, KODE_KANTOR, KODE_PEMBINA, NAMA_FASKES, ALAMAT, RT, RW,KODE_KELURAHAN,
        KODE_KECAMATAN, KODE_KABUPATEN, KODE_POS, NAMA_PIC, HANDPHONE_PIC,  KODE_STATUS, KODE_JENIS, KODE_JENIS_DETIL, NO_IJIN_PRAKTEK, NPWP,
        MAX_TERTANGGUNG, FLAG_UMUM,FLAG_GIGI, FLAG_SALIN, FLAG_REG_WEBSITE, KETERANGAN,  NAMA_PEMILIK,  ALAMAT_PEMILIK,  RT_PEMILIK,  RW_PEMILIK,
        KODE_KELURAHAN_PEMILIK, KODE_KECAMATAN_PEMILIK, KODE_KABUPATEN_PEMILIK,  KODE_POS_PEMILIK, TELEPON_AREA_PEMILIK, TELEPON_PEMILIK, TELEPON_EXT_PEMILIK,
        FAX_AREA_PEMILIK, FAX_PEMILIK, EMAIL_PEMILIK, KODE_KEPEMILIKAN, KODE_METODE_PEMBAYARAN,  KODE_BANK_PEMBAYARAN, NAMA_REKENING_PEMBAYARAN,
        NO_REKENING_PEMBAYARAN, TGL_REKAM,PETUGAS_REKAM,TELEPON_AREA_PIC,TELEPON_PIC,TELEPON_EXT_PIC,KELAS_PERAWATAN,BAGIAN_PIC,NO_FASKES,KODE_PIC_CABANG,KONTAK_PIC_CABANG,LATITUDE,LONGITUDE)
        values('{$ls_kode_new_faskes}','{$ls_kode_tipe}','{$ls_kode_kantor}','{$ls_kode_pembina}'
            ,'{$ls_nama_faskes}','{$ls_alamat}','{$ls_rt}','{$ls_rw}','{$ls_kode_kelurahan}','{$ls_kode_kecamatan}','{$ls_kode_kabupaten}',
            ...
            ,'{$ls_kelas_perawatan}','{$ls_bagian_pic}','{$ls_kode_new_faskes}','{$kode_pic_cabang}','{$ls_handphone_pic_kacab}','{$ls_latitude}','{$ls_longitude}')";
...
{  
    if($ls_daftar=='Y')
        ...
        KODE_STATUS=case when KODE_STATUS='ST1' then 'ST2' else KODE_STATUS end,
        KELAS_PERAWATAN='{$ls_kelas_perawatan}',BAGIAN_PIC='{$ls_bagian_pic}',KODE_PIC_CABANG='{$kode_pic_cabang}',KONTAK_PIC_CABANG='{$ls_handphone_pic_kacab}',LATITUDE='{$ls_latitude}',LONGITUDE='{$ls_longitude}'
...
{  
    if($action_no=='New' || $action_no=='Edit'){
        $ls_kode_faskes     = strtoupper($_POST["kode_faskes"]); 
...
/*else if($action=='UnNonAktif' )
{
    $ls_kode_faskes     = strtoupper($_POST["key1"]);
    $sql = "update {$schema}.tc_faskes set STATUS_REQUEST='B',TGL_REQUEST=sysdate,PETUGAS_REQUEST='{$_SESSION["USER"]}'
        where KODE_FASKES='{$ls_kode_faskes}'";
    $DB->parse($sql); //echo $sql;
    if(!$DB->execute()){	
        echo "Gagal penyimpanan data, ulangi lagi proses save!";
...
{
    $ls_kode_faskes     = strtoupper($_POST["key1"]);
    $sql = "update {$schema}.tc_faskes set STATUS_REQUEST='N',TGL_REQUEST=sysdate,PETUGAS_REQUEST='{$_SESSION["USER"]}',status_approve_request='T'      
        where KODE_FASKES='{$ls_kode_faskes}'";
    $DB->parse($sql); //echo $sql;
    if(!$DB->execute()){    
        echo "Gagal penyimpanan data, ulangi lagi proses save!";

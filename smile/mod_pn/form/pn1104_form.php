...
{
    if($task_no=="Jenis")
        $sql = "SELECT rownum NO_URUT,KODE_JENIS KODE,NAMA_JENIS NAMA,STATUS_NONAKTIF,TO_CHAR(TGL_NONAKTIF,'DD/MM/YYY') TGL_NONAKTIF,
        ...
        FROM {$schema}.TC_KODE_JENIS
        WHERE KODE_JENIS='{$dataid}'"; // echo $sql;
    else if($task_no=="JenisDetil")
        $sql = "SELECT rownum NO_URUT,KODE_JENIS_DETIL KODE,NAMA_JENIS_DETIL NAMA,STATUS_NONAKTIF,TO_CHAR(TGL_NONAKTIF,'DD/MM/YYY') TGL_NONAKTIF,
        TO_CHAR(TGL_REKAM,'DD/MM/YYYY') TGL_REKAM, PETUGAS_REKAM,NVL(TO_CHAR(TGL_UBAH,'DD/MM/YYYY'),' ') TGL_UBAH, 
        NVL(PETUGAS_UBAH,' ') PETUGAS_UBAH,KODE_JENIS KODE_PARENT
        FROM {$schema}.TC_KODE_JENIS_DETIL
        WHERE KODE_JENIS_DETIL='{$dataid}'"; // echo $sql;
    else if($task_no=="Tipe")
        $sql = "SELECT rownum NO_URUT,KODE_TIPE KODE,NAMA_TIPE NAMA,STATUS_NONAKTIF,TO_CHAR(TGL_NONAKTIF,'DD/MM/YYY') TGL_NONAKTIF,
        TO_CHAR(TGL_REKAM,'DD/MM/YYYY') TGL_REKAM, PETUGAS_REKAM,NVL(TO_CHAR(TGL_UBAH,'DD/MM/YYYY'),' ') TGL_UBAH, 
        NVL(PETUGAS_UBAH,' ') PETUGAS_UBAH
        FROM {$schema}.TC_KODE_TIPE
        WHERE KODE_TIPE='{$dataid}'"; // echo $sql;
...
$l_name = $task_no=="Jenis"?"Jenis":"Jenis Detil";
$l_name = $task_no=="Tipe"?"Tipe Faskes":$l_name;
$l_name = $task_no=="Kepemilikan"?"Kepemilikan Faskes":$l_name;
...
<table border='0' width="100%">
    <tr>
        <td width="50%" valign="top">
            ...
            <?php if($task_no=='JenisDetil'){?>
            <div class="f_1" ><label for="kode_parent">Kode Jenis Detil* :</label></div>
            <div class="f_2"><select id="kode_parent" name="kode_parent"></select></div>
            <?php }?>
            <!--
            <div class="f_1" ><label for="nourut">No Urut* :</label></div>
            <div class="f_2"><input type="text" id="nourut" name="nourut" style="text-align:right;width:60px;background-color:<?=$i_nama_color;?>" tabindex="3"   value="<?=$ls_no;?>" <?=$i_nama_readonly;?>/></div>
            -->
...
$(document).ready(function(){ 
    ...
    <?php if($task_no == "JenisDetil"){?>
        lov_subData('getJenis','getJenis','<?=$ls_kode_parent;?>','');
    <?php }?>
});
</script>
<?php /*****end LOCAL JAVASCRIPT*********************************/?>
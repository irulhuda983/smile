...
if ($ls_jenis_grafik=="")
{
 	$ls_jenis_grafik = "graph01"; 
}

//graph01 : Pembayaran Klaim JP Per Kantor Wilayah
//graph02 : Pembayaran Klaim JP Per Sebab Klaim
														
if ($ls_jenis_grafik=="graph02")
{
	echo "<script language=\"JavaScript\" type=\"text/javascript\">";
  echo "window.location.replace('../ajax/pn9010_jpn_graph02.php?task=view&kode_kantor=$ls_kode_kantor&kode_segmen=$ls_kode_segmen&tgl=$ld_tgl&tgl_mmddyyyy=$ld_tgl_mmddyyyy&sender=pn9010.php&sender_mid=$mid');";
  echo "</script>";	
}
...
<?
//1. grafik pembayaran klaim jp per kanwil ------------------------------------
//get data from WS -----------------------------------------------------------
global $wsIp;
$ipDev  = $wsIp."/JSPN9010/GetDataJPNGraph01";
$url    = $ipDev;
$chId   = 'CORE';
...
for($i=0;$i<ExtendedFunction::count($resultArray->JPN_GRAPH01);$i++)
{
  ...
  $ls_graph1_bar_xAxisCategory1_temp .= "'".$resultArray->JPN_GRAPH01[$i]->NAMA_KANTOR_SINGKAT."',";
  $ls_graph1_bar_val_temp .= $ln_graph1_bar_val.",";
	
	//grafik pie -----------------------------------------------------------------
	$ls_graph_pie_val_x1 = $resultArray->JPN_GRAPH01[$i]->KODE_KANTOR;
  $ls_graph_pie_val_x2 = $resultArray->JPN_GRAPH01[$i]->NAMA_KANTOR_SINGKAT;
...
?>
<script type="text/javascript">
	var chart1_bar; // grafik 1 - bar - pembayaran klaim jp per kanwil ----------
  $(document).ready(
  	function() 
  	{
	...
	var chart1_pie; // grafik 1 - pie - pembayaran klaim jp per kanwil ----------
  $(document).ready(
  	function() 
  	{
...
<!-- end 1. grafik pembayaran klaim jp per kanwil ---------------------------->
	...
	<div class="content">
		<div class="container">
          ...
          <div class="row">
    				<div class="col-lg-12">
              <div class="card-box">
    						<h4 class="header-title m-t-0 m-b-30"><b>Pembayaran Klaim JP Per Kantor Wilayah</b></h4>
          ...
          <div class="row">
    				<div class="col-lg-12">
              <div class="card-box">
    						<h4 class="header-title m-t-0 m-b-30"><b>Komposisi Pembayaran Klaim JP Kantor Wilayah</b></h4> 
                <div class="widget-chart text-center">
                    ...
                    for($i=0;$i<ExtendedFunction::count($resultArray->JPN_GRAPH01);$i++)
                    {
                      ?>
                        ...
                        <td style="text-align:left">
  												<a href="#" onClick="NewWindow4('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn9010_jpn_graph01.php?task=view&kode_kantor=<?=$resultArray->JPN_GRAPH01[$i]->KODE_KANTOR;?>&kode_segmen=<?=$ls_kode_segmen;?>&tgl=<?=$ld_tgl;?>&tgl_mmddyyyy=<?=$ld_tgl_mmddyyyy;?>&sender=pn9010.php&sender_mid=<?=$mid;?>','Drilldown',1250,680,'yes');" href="javascript:void(0);">
          									 <?=$resultArray->JPN_GRAPH01[$i]->NAMA_KANTOR_SINGKAT2;?>
          								</a>
												</td>
                      ...
                      <?							    							
                      //$i++;//iterasi i
											$ln_tot_kasus_jpn  += (float)$resultArray->JPN_GRAPH01[$i]->JML_KASUS;
											$ln_tot_pembayaran_jpn += (float)$resultArray->JPN_GRAPH01[$i]->NOM_PEMBAYARAN;
                    }	//end while

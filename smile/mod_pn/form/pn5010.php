...
$pagetype = "form";
$gs_pagetitle = "PN5010 - PERSETUJUAN VERIFIKASI KEGIATAN PROMOTIF/PREVENTIF";
require_once "../../includes/header_app_nosql.php";	
$mid = $_REQUEST["mid"];
$gs_kode_segmen = "PU";

/* ============================================================================
Ket : Form ini digunakan sebagai form input Persetujuan Verifikasi Kegiatan Promotif/Preventif
...
<div id="actmenu" >
	<div id="actbutton">
		...
		?>
			...
			<div style="float:left;">
				<div class="icon">
					<a id="btn_close" href="http://<?=$HTTP_HOST;?>/mod_pn/form/pn5010.php?mid=<?=$mid;?>">
    ...
    <div id="formKiri" style="width:1000px">
		<form name="formreg" id="formreg" role="form" method="post">
			<div id="konten">
				...
				<div id="formsplit">
					<fieldset>
						<legend>Data Persetujuan Verifikasi Data Perusahaan</legend>
						...
						<div class="clear"></div>
						<div class="form-row_kiri">
							<label>Keterangan Persetujuan Verifikasi<span style="color:#ff0000;">*</span></label>
							<textarea cols="255" rows="2" id="keterangan_persetujuan_verifikasi" name="keterangan_persetujuan_verifikasi" value="<?=$ls_keterangan_persetujuan_verifikasi;?>" tabindex="8" style="width:225px;" onkeypress="return canAddCharacter(this, 300)" onchange="trimLength(this, 300)">
							</textarea>   
						</div>
						<div class="clear"></div>
					</fieldset>
				...
				<div id="formsplit">
					<fieldset>
						<legend>Detail Persetujuan Verifikasi Data Perusahaan</legend>	
						<div class="form-row_kiri">
							<label>Kode Promotif</label>	
						...
						<div class="clear"></div>	
						<div class="form-row_kiri">
							<label>Keterangan Persetujuan Verifikasi </label>
				...
				<div id="formsplit">
					<fieldset>
						<legend>Daftar Persetujuan Verifikasi Perusahaan</legend>	
						<div class="clear"></div>
						<table class="table responsive-table" id="mydata" cellspacing="0" width="100%">
							<thead>
								<tr>
									...
									<th scope="col" width="10%">Kode User  KBP/Wasrik</th>
									<th scope="col" width="10%">Tanggal Verifikasi KBP/Wasrik</th>
									<th scope="col" width="10%">Status Verifikasi KBP/Wasrik</th>								
								</tr>
							</thead>
							<tbody id="listdata">
							</tbody>
				...
				<div id="formsplit">
					<fieldset style="background: #ededed;">
						<br>
						<input type="button" name="btnsetujuverifikasi" id="btnsetujuverifikasi" class="btn green"  value="DATA VERIFIKASI PERUSAHAAN DISETUJUI" title="Klik untuk submit persetujuan hasil verifikasi perusahaan yang layak">
						&nbsp;
						<input type="button" name="btntolakverifikasi" id="btntolakverifikasi" class="btn green"  value="DATA VERIFIKASI PERUSAHAAN DITOLAK" title="Klik untuk submit persetujuan hasil verifikasi perusahaan yang tidak layak">																																																	
					</fieldset>
					<br>
					<div class="clear"></div>
				</div>
        ...
        <script type="text/javascript">
			$(document).ready(function(){
				...
				$('#btn_view').click(function() {
					if(window.dataid != ''){
						window.location='pn5010.php?task=View&tasktk=View&dataid='+window.dataid+'&dataid2='+window.kodeperusahaan+'&mid=<?=$mid;?>';
					} else {
				...
				$('#btn_edit').click(function() {	
					if(window.dataid != ''){
						window.location='pn5010.php?task=Edit&tasktk=Edit&dataid='+window.dataid+'&dataid2='+window.kodeperusahaan+'&mid=<?=$mid;?>';
					} else {
						alert('Silahkan pilih terlebih dahulu data yang akan di proses!');
					}
				});
				...
				$('#btn_delete').click(function() {
					if(window.dataid != ''){
						...
						if (r == true) {
							$.ajax({
								type: 'POST',
								url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5010_action.php?'+Math.random(),
				...
				$('#btn_new').click(function() {
					NewWindow4('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5010_eligible.php?kode_user=<?php echo $ls_user_login?>&kode_kantor=<?php echo $ls_kode_kantor?>','pn501001 - Daftar Perusahaan Eligible','1024','600','yes');
				});
				...
				<?PHP
					if($_REQUEST["task"] == "Edit"){
						...
						$.ajax({
							...
							success: function(data) {
								...
								if(jdata.ret == '0'){
									...
									if(jdata.data[0].STATUS_VERIF_APPROVAL === null)
									{
									...
									else
									{
										...
										$('#keterangan_persetujuan_verifikasi').attr('disabled','disabled');
										$('#keterangan_persetujuan_verifikasi').addClass('disabled');
						...
						$('#btn_save').click(function() {
							if($('#keterangan_persetujuan_verifikasi').val() != ''){
								preload(true);
								$.ajax({
									type: 'POST',
									url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5010_action.php?'+Math.random(),
								...
								alert('Silahkan mengisi keterangan pesetujuan verifikasi perusahaan!');
							}
						...
						$('#btnsetujuverifikasi').click(function() {
							if($('#keterangan_persetujuan_verifikasi').val() != ''){
								...
								alert('Silahkan mengisi keterangan persetujuan verifikasi perusahaan!');
							}
			...
			function approval(p_status){
				...
				if (r == true) {
					$.ajax({
						type: 'POST',
						url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5010_action.php?'+Math.random(),
						data: { TYPE:'APPROVE', DATAID:window.dataid, STATUSAPPROVAL:p_status, KETERANGANAPPROVAL:$('#keterangan_persetujuan_verifikasi').val()},
						success: function(data) {
							preload(false);
							jdata = JSON.parse(data);
							if(jdata.ret == '0'){
			...
			function loadData(){
				...
				$.ajax({
					type: 'POST',
					url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5010_query.php?'+Math.random(),
						...
						if(jdata.ret == 0){
							for (i = 0; i < jdata.data.length; i++) { 
								window.table.row.add( [
									...
									jdata.data[i].PETUGAS_VERIF_PERMINTAAN,
									jdata.data[i].TGL_VERIF_PERMINTAAN,
			...
			function cetakDokumenPdf(p_kode_report,p_kode_promotif) { 
				...
				preload(true);
				$.ajax({
					url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5010_cetak.php?'+Math.random(),
					type:'POST',

                ...
                <table id="tblMnf" width="100%" class="table-data2">
                  <thead>								
                    <tr>
                      ...
                      <th style="text-align:center;" colspan="4" class="hr-single-bottom">PENETAPAN NILAI MANFAAT BIAYA PROTHESE/ORTHESE</th>
                    ...
                    <tr class="hr-double-bottom">
                      ...
                      <th style="text-align:center;width:150px;">Jenis Penggantian</th>
                      <th style="text-align:right;width:100px;">Biaya Diajukan</th>
                      <th style="text-align:right;width:100px;">Biaya Diverifikasi</th>
                      <th style="text-align:right;width:100px;">Biaya Disetujui</th>
                    ...
                    <tr>
                      ...
                      <td style="text-align:right"><span id="mnf_tot_biaya_diajukan"></span></td>
											<td style="text-align:right"><span id="mnf_tot_biaya_diverifikasi"></span></td>
		...
		$.ajax({
			...
			success: function(data){
				try {
					...
					if (jdata.ret == 0) 
					{ 
						$("#span_page_title").html('PN5030 - RINCIAN MANFAAT - PROTHESE/ORTHESE');
						$("#span_page_title_right").html('KODE KLAIM : '+v_kode_klaim);
						
            //set data manfaat jp berkala --------------------------------------
            var html_mnf = "";
						var v_tot_d_nom_biaya_diajukan = 0;
						var v_tot_d_nom_biaya_diverifikasi = 0;
            ...
            { 
              for(var i = 0; i < jdata.data.DATA.length; i++)
              {
                ...
                html_mnf += '<td style="text-align: center;">' + getValue(jdata.data.DATA[i].NAMA_MANFAAT_DETIL) + '</td>';
								html_mnf += '<td style="text-align: right;">' + format_uang(getValueNumber(jdata.data.DATA[i].NOM_BIAYA_DIAJUKAN)) + '</td>';
                html_mnf += '<td style="text-align: right;">' + format_uang(getValueNumber(jdata.data.DATA[i].NOM_BIAYA_DIVERIFIKASI)) + '</td>';
                ...
                v_tot_d_nom_biaya_diajukan  += parseFloat(getValueNumber(jdata.data.DATA[i].NOM_BIAYA_DIAJUKAN),4);
								v_tot_d_nom_biaya_diverifikasi  += parseFloat(getValueNumber(jdata.data.DATA[i].NOM_BIAYA_DIVERIFIKASI),4);
              ...
              $("#mnf_tot_biaya_diajukan").html(format_uang(v_tot_d_nom_biaya_diajukan));
							$("#mnf_tot_biaya_diverifikasi").html(format_uang(v_tot_d_nom_biaya_diverifikasi));
...
<script language="javascript">
	...
	{		
		var c_mid = '<?=$mid;?>';
		showForm('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5069_view_penetapanmanfaat_protheserinci.php?kode_klaim='+p_kode_klaim+'&kode_manfaat='+p_kode_manfaat+'&no_urut='+p_no_urut+'&mid='+c_mid+'','',980,550,'yes');
	}
</script>

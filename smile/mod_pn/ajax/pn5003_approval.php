...
<div id="formframe">
    ...
    if ($ls_jenis_klaim=="JHT")
    {
     	include "../ajax/pn5003_approval_jht.php";	  	 
    }else if ($ls_jenis_klaim=="JHM")
    {
     	include "../ajax/pn5003_approval_jhm.php";  	  	 
    }else if ($ls_jenis_klaim=="JKK")
    {
     	include "../ajax/pn5003_approval_jkk.php";		  	 
    }else if ($ls_jenis_klaim=="JKM")
    {
     	include "../ajax/pn5003_approval_jkm.php";  	  	 
    }else if ($ls_jenis_klaim=="JPN")
    {
     	include "../ajax/pn5003_approval_jpn.php";  	 
    }
		?>
    ...
    <div id="buttonbox" style="width:930px; text-align:center;">
      ...
      {
       	?>
  			<input type="button" class="btn green" id="btnapvtolak" name="btnapvtolak" onclick="window.location.replace('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5003_approval_tolak.php?&kode_klaim=<?=$ls_kode_klaim;?>&no_level=<?=$ln_no_level;?>&root_sender=pn5003.php&sender=pn5003_approval.php&sender_mid<?=$mid;?>','Penolakan',950,520,'no')" value="                 TOLAK                  " />
				&nbsp;
				<?
        ...
        $ln_cnt_otorisasi = $row['V_CNT'];				
    			...
    			{
      			?>
      			<input type="button" class="btn green" id="btnapvsubmit" name="btnapvsubmit" onclick="if(confirm('Apakah anda yakin akan mensubmit data klaim ke tahap selanjutnya..?')) doSubmitTanpaOtentikasi();;" value="       SUBMIT UNTUK PERMOHONAN PERSETUJUAN        " />			                 
      			<?  			
    			}else
    			{
      			?>
      			<input type="button" class="btn green" id="btnapvsubmit" name="btnapvsubmit" onclick="if(confirm('Apakah anda yakin akan mensubmit data klaim ke tahap selanjutnya..?')) doSubmitTanpaOtentikasi();" value="       SUBMIT DATA UNTUK PEMBAYARAN        " />			                 
      			<?
    			}
				}else
				{
    			...
    			{
      			?>
      			<input type="button" class="btn green" disabled id="btnapvsubmit" name="btnapvsubmit" onclick="alert('User tidak memiliki wewenang untuk melakukan persetujuan klaim..!!!');" value="       SUBMIT UNTUK PERMOHONAN PERSETUJUAN        " />			                 
      			<?  			
    			}else
    			{
      			?>
      			<input type="button" class="btn green" disabled id="btnapvsubmit" name="btnapvsubmit" onclick="alert('User tidak memiliki wewenang untuk melakukan persetujuan klaim..!!!');" value="       SUBMIT DATA UNTUK PEMBAYARAN        " />			                 
      			<?
    			}				
				}
      }

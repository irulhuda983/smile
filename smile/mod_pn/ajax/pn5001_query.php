...
if($TYPE!=''){
  ...
  if($DB->execute())
    ...
    while($data = $DB->nextrow())
    {
        ...
        $data['KODE_KLAIM'] = '<a href="#" onClick="window.location.replace(\'http://'.$HTTP_HOST.'/mod_pn/form/pn5001.php?task=View&dataid='.$data['KODE_KLAIM'].'&kode_klaim='.$data['KODE_KLAIM'].'\',\'PN5001 - Agenda Klaim\')"><font color="#009999">'.$data['KODE_KLAIM'].'</font> </a>';

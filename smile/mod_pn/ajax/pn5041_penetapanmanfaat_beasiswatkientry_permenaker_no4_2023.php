	...
	<div id="formKiri" style="width:97%; margin:0px 0px 0px 0px;">
		<form name="formreg" id="formreg" role="form" method="post" enctype="multipart/form-data">
            ...
            <span id="span_footer_keterangan_new" style="display:block;">
			  ...
			  <ul>
              <li style="margin-left:15px;">Klik <img src="../../images/help.png" alt="Cari Data Peserta" border="0" align="absmiddle"> untuk <span style="color:#ff0000">memilih</span> NIK Anak penerima manfaat beasiswa.</li>
			  </ul>
              </div>								
            </span>					
					</div>						 
...
<script language="javascript">
	function fl_js_New()
	{
		...
		$.ajax({
			type: 'POST',
			url: "../ajax/pn5041_action.php?"+Math.random(),
			data: {
			...
			success: function(data){
				try {
					...
					if (jdata.ret == 0) 
					{
						...
						html_input += '	               	<strong><span style="vertical-align: middle;" id="span_info_tidakada_tahun_beasiswa"></span></strong>';
						html_input += '	               </div>';
						html_input += '	             </div>';								
						html_input += '	           </span>';
...
<script language="javascript">
	...
	{			 					
		...
		NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5041_lov_nikpenerimabeasiswatki_permenakerno4_2023.php?p=pn5041.php&a=formreg&b=beasiswa_nik_penerima&kode_klaim='+v_kode_klaim+'&c='+v_nik_tk+'&d='+v_nama_tk+'','',900,500,1);
  	}
	...
	function fjq_ajax_val_get_tahun_beasiswa()
	{
		...
		{								
			...
			if (v_getdt == "Y")
			{
				...
				{
					type: 'POST',
					url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5041_action.php?'+Math.random(),
					data: { 
	...
	function fl_js_entry_detilbeasiswa(v_i, v_tahun, v_dtl_data)
	{
		...
		{
		 	showForm('http://<?= $HTTP_HOST;?>/mod_pn/ajax/pn5041_penetapanmanfaat_beasiswaentrytki_permenakerno4_2023.php?' + params,'',800,650,'no'); 
		}else
		{
		 	NewWindow('http://<?= $HTTP_HOST;?>/mod_pn/ajax/pn5041_penetapanmanfaat_beasiswaentrytki_permenakerno4_2023.php' + params,'',800,650,'no');		 
		}
	}

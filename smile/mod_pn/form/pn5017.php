<?php
$pagetype = "form";
$gs_pagetitle = "PN5017 - PENETAPAN PELAKSANA KEGIATAN PROMOTIF/PREVENTIF";
require_once "../../includes/header_app_nosql.php";	
$mid = $_REQUEST["mid"];
$gs_kode_segmen = "PU";

/* ============================================================================
Ket : Form ini digunakan sebagai form input Penetapan Pelaksana Kegiatan Promotif/Preventif
...
<div id="actmenu" >
	<div id="actbutton">
		...
		?>
			...
			<div style="float:left;">
				<div class="icon">
					<a id="btn_close" href="http://<?=$HTTP_HOST;?>/mod_pn/form/pn5017.php?mid=<?=$mid;?>">
    ...
    <div id="formKiri" style="width:1000px">
		<form name="formreg" id="formreg" role="form" method="post">
			<div id="konten">
				...
				<div id="formsplit">
					<fieldset>
						<legend>Data Penetapan Pelaksana Kegiatan</legend>
						<table>
							<tr>
								<td width="50%" valign="top">
									...
									<div class="clear"></div>
									<div class="form-row_kiri">	
									<label>Jenis Pelakasana <span style="color:#ff0000;">*</span></label>
									<select size="1" id="jenis_pelaksana_kegiatan" name="jenis_pelaksana_kegiatan" value="<?=$ls_jenis_pelaksana_kegiatan;?>" tabindex="7" class="select_format" style="width:230px;background-color:#ffff99;" >
										<option value="">-- Pilih --</option>
									...
									<div class="form-row_kiri">
										...
										<textarea cols="255" rows="2" id="keterangan_penetapan_pelaksana" name="keterangan_penetapan_pelaksana" value="<?=$ls_keterangan_penetapan_pelaksana;?>" tabindex="8" style="width:225px;" onkeypress="return canAddCharacter(this, 300)" onchange="trimLength(this, 300)" >
										</textarea>   
									</div>
									<div class="clear"></div>
				...
				<div id="div_penambahantkpengajuan">
					<div id="formsplit">
						<fieldset style="background: #ededed;">
							<br>
							<div class="form-row_kiri">
								<input type="button" name="btnsubmitpenetapanpelaksanakegiatan" id="btnsubmitpenetapanpelaksanakegiatan" class="btn green"  value="SUBMIT PENETAPAN PELAKSANA KEGIATAN" title="Klik untuk submit penetapan pelaksana kegiatan">	
							</div>
						</fieldset>
					<br>
					</div>
							...
							<table class="table responsive-table" id="" cellspacing="0" width="100%">
								...
								?>
									<tr>
										...
										{
											$linkhapus ='pn5017.php?task=Edit&tasktk=Edit&dataid='+window.dataid+'&dataid2='+window.kodeperusahaan+'&mid=<?=$mid;?>';
        ...
        <script type="text/javascript">
			$(document).ready(function(){
				...
				$('#btn_view').click(function() {
					if(window.dataid != ''){
						window.location='pn5017.php?task=View&tasktk=View&dataid='+window.dataid+'&dataid2='+window.kodeperusahaan+'&mid=<?=$mid;?>';
					} else {
				...
				$('#btn_edit').click(function() {	
					if(window.dataid != ''){
						window.location='pn5017.php?task=Edit&tasktk=Edit&dataid='+window.dataid+'&dataid2='+window.kodeperusahaan+'&mid=<?=$mid;?>';
					} else {
						alert('Silahkan pilih terlebih dahulu data yang akan di proses!');
					}
				});
				...
				<?PHP
					if($_REQUEST["task"] == "View"){
						...
						$.ajax({
							type: 'POST',
							url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5017_action.php?'+Math.random(),
				...
				<?PHP
					if($_REQUEST["task"] == "Edit"){
						...
						$.ajax({
							...
							success: function(data) {
								...
								if(jdata.ret == '0'){
									...
									if(jdata.data[0].TGL_SUBMIT_PELAKSANA_KEGIATAN === null)
									{
										$('#btnsubmitpenetapanpelaksanakegiatan').removeAttr('disabled');
										$('#btnsubmitpenetapanpelaksanakegiatan').removeClass('disabled');
									}
									else
									{
										$('#btnsubmitpenetapanpelaksanakegiatan').attr('disabled','disabled');
										$('#btnsubmitpenetapanpelaksanakegiatan').addClass('disabled');
										
										$('#jenis_pelaksana_kegiatan').attr('disabled','disabled');
										$('#jenis_pelaksana_kegiatan').addClass('disabled');
										$('#keterangan_penetapan_pelaksana').attr('disabled','disabled');
										$('#keterangan_penetapan_pelaksana').addClass('disabled');
						...
						$('#btn_save').click(function() {
							if($('#jenis_pelaksana_kegiatan').val() != '' && $('#keterangan_penetapan_pelaksana').val() != ''){
								preload(true);
								$.ajax({
									type: 'POST',
									url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5017_action.php?'+Math.random(),
								...
								alert('Silahkan mengisi data mandatori penetapan pelaksana kegiatan!');
							}
							
						});
						
						$('#btnsubmitpenetapanpelaksanakegiatan').click(function() {
							if($('#jenis_pelaksana_kegiatan').val() != '' && $('#keterangan_penetapan_pelaksana').val() != ''){
								submitPenetapanPelaksanaKegiatan(window.dataid);
							}else {
								alert('Silahkan mengisi data mandatori penetapan pelaksana kegiatan!');
							}
						});
			...
			function submitPenetapanPelaksanaKegiatan(p_kode_promotif){
				var r = confirm("Apakah anda yakin?");
				if (r == true) {
					$.ajax({
						type: 'POST',
						url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5017_action.php?'+Math.random(),
						data: { TYPE:'SUBMIT', DATAID:p_kode_promotif, JENISPELAKSANASUBMIT:$('#jenis_pelaksana_kegiatan').val(), KETERANGANSUBMIT:$('#keterangan_penetapan_pelaksana').val()},
						success: function(data) {
							preload(false);
							jdata = JSON.parse(data);
							if(jdata.ret == '0'){
			...
			function loadData(){
				...
				$.ajax({
					type: 'POST',
					url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5017_query.php?'+Math.random(),

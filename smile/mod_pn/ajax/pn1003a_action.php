...
$ls_nama_tipe_penerima	= $_POST['v_nama_tipe_penerima'];
...
if($ls_type=="new"){
	$sql = "insert into pn.pn_kode_tipe_penerima (
				kode_tipe_penerima,
				nama_tipe_penerima,
                no_urut,
                status_nonaktif,
	...
	$sql = "update sijstk.pn_kode_tipe_penerima
			   set nama_tipe_penerima = '$ls_nama_tipe_penerima',
			       no_urut = '$ls_no_urut',
			 ...
			 where kode_tipe_penerima = '$ls_kode_tipe_penerima'";
	$DB->parse($sql);
	if($DB->execute()){ 
	...
	$sql="select kode_tipe_penerima,
		 ...
		 where kode_tipe_penerima='".$ls_search_keyword."'";
    $DB->parse($sql);
    if($DB->execute()){ 
	...
	if($order[0]['column']=='0'){
	    ...
	    $order = "ORDER BY KODE_TIPE_PENERIMA ";
	}else if($order[0]['column']=='2'){
	    $order = "ORDER BY NAMA_TIPE_PENERIMA ";
	...
	$sql = "";
	$sql="select * from (select rownum no, kode_tipe_penerima,
		       nama_tipe_penerima,
		       status_nonaktif,
		       tgl_nonaktif,
		       keterangan
	...
	$queryTotal = "SELECT COUNT (1) FROM SIJSTK.PN_KODE_TIPE_PENERIMA WHERE 1=1  ".$condition;
	// print_r($sql);
	// print_r($querySql);
	...
	if($DB->execute()){ 
		...
		// ';
			$data['ACTION'] = '<button class="btn green" name="view_tipe_penerima_'.$data['KODE_TIPE_PENERIMA'].'" id="view_sebab_klaim_'.$data['KODE_TIPE_PENERIMA'].'" kode_tipe_penerima="'.$data['KODE_TIPE_PENERIMA'].'"  title="Klik untuk view detil data"><i class="fa fa-search" aria-hidden="true"></i></button>'.' '.' <button class="btn green" name="edit_tipe_penerima_'.$data['KODE_TIPE_PENERIMA'].'" id="edit_tipe_penerima_'.$data['KODE_TIPE_PENERIMA'].'" kode_tipe_penerima="'.$data['KODE_TIPE_PENERIMA'].'"  title="Klik untuk edit data"><i class="fa fa-pencil" aria-hidden="true"></i></button> <button class="btn hapus" name="hapus_tipe_penerima_'.$data['KODE_TIPE_PENERIMA'].'" kode_tipe_penerima="'.$data['KODE_TIPE_PENERIMA'].'" title="Klik untuk hapus data"><i class="fa fa-trash" aria-hidden="true"></i></button>
				';

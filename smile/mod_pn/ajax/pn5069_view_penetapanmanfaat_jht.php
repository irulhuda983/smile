<?php
//------------------------------------------------------------------------------
// Menu untuk display rincian manfaat jht
...
<script language="javascript">
	function loadSelectedRecord(v_kode_klaim, v_kode_manfaat, fn)
	{
		...
		//utk rincian manfaat jht set v_no_urut_mnf = 1
		var v_no_urut_mnf = 1;
			...
			success: function(data){
				try {
					...
					{ 
						...
						/*
						 31122021, penyesuaian untuk tampilan PN5030 - RINCIAN MANFAAT PERINCIAN JHT untuk mengakomodir manfaat JHT syariah di tampilan menu Daftar Klaim
						 - meload ulang lagi data dengan no_urut = 2 sebagai manfaat JHT syariah di PN_KLAIM_MANFAAT_DETIL
						*/
						// start rincian iuran JHT syariah
						if (v_kode_manfaat == 18)
						{
							var mnf_nom_manfaat_sudahdiambil_kon = getValue(jdata.data.DATA['NOM_MANFAAT_SUDAHDIAMBIL']);
							var mnf_nom_biaya_diajukan_kon = getValue(jdata.data.DATA['NOM_BIAYA_DIAJUKAN']);
							var mnf_nom_saldo_awaltahun_kon = getValue(jdata.data.DATA['NOM_SALDO_AWALTAHUN']);
							var mnf_nom_diambil_thnberjalan_kon = getValue(jdata.data.DATA['NOM_DIAMBIL_THNBERJALAN']);
							var mnf_nom_saldo_pengembangan_kon = getValue(jdata.data.DATA['NOM_SALDO_PENGEMBANGAN']);
							var mnf_nom_manfaat_maxbisadiambil_kon = getValue(jdata.data.DATA['NOM_MANFAAT_MAXBISADIAMBIL']);
							var mnf_nom_saldo_total_kon = getValue(jdata.data.DATA['NOM_SALDO_TOTAL']);
							var mnf_nom_manfaat_diambil_kon = getValue(jdata.data.DATA['NOM_MANFAAT_DIAMBIL']);
							var mnf_nom_iuran_thnberjalan_kon = getValue(jdata.data.DATA['NOM_IURAN_THNBERJALAN']);
							var mnf_nom_manfaat_gross_kon = getValue(jdata.data.DATA['NOM_MANFAAT_GROSS']);
							var mnf_nom_pph_kon = getValue(jdata.data.DATA['NOM_PPH']);
							var mnf_nom_iuran_pengembangan_kon = getValue(jdata.data.DATA['NOM_IURAN_PENGEMBANGAN']);
							var mnf_nom_pembulatan_kon = getValue(jdata.data.DATA['NOM_PEMBULATAN']);
							var mnf_nom_iuran_total_kon = getValue(jdata.data.DATA['NOM_IURAN_TOTAL']);
							var mnf_nom_manfaat_netto_kon = getValue(jdata.data.DATA['NOM_MANFAAT_NETTO']);
							var mnf_nom_saldo_iuran_total_kon = getValue(jdata.data.DATA['NOM_SALDO_IURAN_TOTAL']);
													
							loadSelectedRecordSyariah
							(
								v_kode_klaim, v_kode_manfaat, fn,
								mnf_nom_manfaat_sudahdiambil_kon,
								mnf_nom_biaya_diajukan_kon,
								mnf_nom_saldo_awaltahun_kon,
								mnf_nom_diambil_thnberjalan_kon,
								mnf_nom_saldo_pengembangan_kon,
								mnf_nom_manfaat_maxbisadiambil_kon,
								mnf_nom_saldo_total_kon,
								mnf_nom_manfaat_diambil_kon,
								mnf_nom_iuran_thnberjalan_kon,
								mnf_nom_manfaat_gross_kon,
								mnf_nom_pph_kon,
								mnf_nom_iuran_pengembangan_kon,
								mnf_nom_pembulatan_kon,
								mnf_nom_iuran_total_kon,
								mnf_nom_manfaat_netto_kon,
								mnf_nom_saldo_iuran_total_kon
							)
							;
						}
						// end rincian iuran JHT syariah
	...
	function loadSelectedRecordSyariah(v_kode_klaim, v_kode_manfaat, fn, 
										mnf_nom_manfaat_sudahdiambil_kon,
										mnf_nom_biaya_diajukan_kon,
										mnf_nom_saldo_awaltahun_kon,
										mnf_nom_diambil_thnberjalan_kon,
				...
				try {
					...
					{ 
						...
						*/
						...
						var mnf_nom_manfaat_sudahdiambil_syr = getValue(jdata.data.DATA['NOM_MANFAAT_SUDAHDIAMBIL']);
						var mnf_nom_biaya_diajukan_syr = getValue(jdata.data.DATA['NOM_BIAYA_DIAJUKAN']);
						var mnf_nom_saldo_awaltahun_syr = getValue(jdata.data.DATA['NOM_SALDO_AWALTAHUN']);
						var mnf_nom_diambil_thnberjalan_syr = getValue(jdata.data.DATA['NOM_DIAMBIL_THNBERJALAN']);
						var mnf_nom_saldo_pengembangan_syr = getValue(jdata.data.DATA['NOM_SALDO_PENGEMBANGAN']);
						var mnf_nom_manfaat_maxbisadiambil_syr = getValue(jdata.data.DATA['NOM_MANFAAT_MAXBISADIAMBIL']);
						var mnf_nom_saldo_total_syr = getValue(jdata.data.DATA['NOM_SALDO_TOTAL']);
						var mnf_nom_manfaat_diambil_syr = getValue(jdata.data.DATA['NOM_MANFAAT_DIAMBIL']);
						var mnf_nom_iuran_thnberjalan_syr = getValue(jdata.data.DATA['NOM_IURAN_THNBERJALAN']);
						var mnf_nom_manfaat_gross_syr = getValue(jdata.data.DATA['NOM_MANFAAT_GROSS']);
						var mnf_nom_pph_syr = getValue(jdata.data.DATA['NOM_PPH']);
						var mnf_nom_iuran_pengembangan_syr = getValue(jdata.data.DATA['NOM_IURAN_PENGEMBANGAN']);
						var mnf_nom_pembulatan_syr = getValue(jdata.data.DATA['NOM_PEMBULATAN']);
						var mnf_nom_iuran_total_syr = getValue(jdata.data.DATA['NOM_IURAN_TOTAL']);
						var mnf_nom_manfaat_netto_syr = getValue(jdata.data.DATA['NOM_MANFAAT_NETTO']);
						var mnf_nom_saldo_iuran_total_syr = getValue(jdata.data.DATA['NOM_SALDO_IURAN_TOTAL']);
						
						
						// menghitung ulang manfaat total konvensiaonal dan syariah JHT untuk ditampilkan di form
						var mnf_nom_manfaat_sudahdiambil = mnf_nom_manfaat_sudahdiambil_kon + mnf_nom_manfaat_sudahdiambil_syr;
						var mnf_nom_biaya_diajukan = mnf_nom_biaya_diajukan_kon + mnf_nom_biaya_diajukan_syr;
						var mnf_nom_saldo_awaltahun = mnf_nom_saldo_awaltahun_kon + mnf_nom_saldo_awaltahun_syr;
						var mnf_nom_diambil_thnberjalan = mnf_nom_diambil_thnberjalan_kon + mnf_nom_diambil_thnberjalan_syr;
						var mnf_nom_saldo_pengembangan = mnf_nom_saldo_pengembangan_kon + mnf_nom_saldo_pengembangan_syr;
						var mnf_nom_manfaat_maxbisadiambil = mnf_nom_manfaat_maxbisadiambil_kon + mnf_nom_manfaat_maxbisadiambil_syr;
						var mnf_nom_saldo_total = mnf_nom_saldo_total_kon + mnf_nom_saldo_total_syr;
						var mnf_nom_manfaat_diambil = mnf_nom_manfaat_diambil_kon + mnf_nom_manfaat_diambil_syr;
						var mnf_nom_iuran_thnberjalan = mnf_nom_iuran_thnberjalan_kon + mnf_nom_iuran_thnberjalan_syr;
						var mnf_nom_manfaat_gross = mnf_nom_manfaat_gross_kon + mnf_nom_manfaat_gross_syr;
						var mnf_nom_pph = mnf_nom_pph_kon + mnf_nom_pph_syr;
						var mnf_nom_iuran_pengembangan = mnf_nom_iuran_pengembangan_kon + mnf_nom_iuran_pengembangan_syr;
						
						if (((mnf_nom_manfaat_gross - mnf_nom_pph) % 10) == 0)
						{
							var mnf_nom_pembulatan = 0;
						}
						else
						{
							var mnf_nom_pembulatan = (10 - ((mnf_nom_manfaat_gross - mnf_nom_pph) % 10));
						}
						
						var mnf_nom_iuran_total = mnf_nom_iuran_total_kon + mnf_nom_iuran_total_syr;
						var mnf_nom_manfaat_netto = mnf_nom_manfaat_gross - mnf_nom_pph + mnf_nom_pembulatan;
						var mnf_nom_saldo_iuran_total = mnf_nom_saldo_iuran_total_kon + mnf_nom_saldo_iuran_total_syr;
						
						// menampilkan data di form
						
						$('#mnf_nom_manfaat_sudahdiambil').val(format_uang(mnf_nom_manfaat_sudahdiambil));
						$('#mnf_nom_biaya_diajukan').val(format_uang(mnf_nom_biaya_diajukan));
						
						$('#mnf_nom_saldo_awaltahun').val(format_uang(mnf_nom_saldo_awaltahun));
						$('#mnf_nom_diambil_thnberjalan').val(format_uang(mnf_nom_diambil_thnberjalan));
						$('#mnf_nom_saldo_pengembangan').val(format_uang(mnf_nom_saldo_pengembangan));
						$('#mnf_nom_manfaat_maxbisadiambil').val(format_uang(mnf_nom_manfaat_maxbisadiambil));
						
						$('#mnf_nom_saldo_total').val(format_uang(mnf_nom_saldo_total));
						$('#mnf_nom_manfaat_diambil').val(format_uang(mnf_nom_manfaat_diambil));
						
						$('#mnf_nom_iuran_thnberjalan').val(format_uang(mnf_nom_iuran_thnberjalan));
						$('#mnf_nom_pph').val(format_uang(mnf_nom_pph));
						$('#mnf_nom_iuran_pengembangan').val(format_uang(mnf_nom_iuran_pengembangan));
						$('#mnf_nom_pembulatan').val(format_uang(mnf_nom_pembulatan));
						$('#mnf_nom_iuran_total').val(format_uang(mnf_nom_iuran_total));
						$('#mnf_nom_manfaat_netto').val(format_uang(mnf_nom_manfaat_netto));
						$('#mnf_nom_saldo_iuran_total').val(format_uang(mnf_nom_saldo_iuran_total));

                ...
                <table id="tblMnf" width="100%" class="table-data2">
                  <thead>								
                    <tr>
                      ...
                      <th style="text-align:center;" colspan="5" class="hr-single-bottom">PENETAPAN NILAI MANFAAT BIAYA OBAT/RAWAT</th>
                    ...
                    <tr class="hr-double-bottom">
                      ...
                      <th style="text-align:center;width:150px;">Tipe Penerima</th>
            					<th style="text-align:center;width:150px;">Jenis Obat/Rawat</th>
		...
		$.ajax({
			...
			success: function(data){
				try {
					...
					if (jdata.ret == 0) 
					{ 
						$("#span_page_title").html('PN5030 - RINCIAN MANFAAT - BIAYA OBAT/RAWAT');
            ...
            { 
              for(var i = 0; i < jdata.data.DATA.length; i++)
              {
                ...
                html_mnf += '<td style="text-align: center;">' + getValue(jdata.data.DATA[i].NAMA_RAWAT_JENIS) + '</td>';
								html_mnf += '<td style="text-align: center;">' + getValue(jdata.data.DATA[i].RAWAT_TGL) + '</td>';
...
<script language="javascript">
	...
	{		
		var c_mid = '<?=$mid;?>';
		showForm('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5069_view_penetapanmanfaat_obatrawatrinci.php?kode_klaim='+p_kode_klaim+'&kode_manfaat='+p_kode_manfaat+'&no_urut='+p_no_urut+'&mid='+c_mid+'','',980,550,'yes');
	}
</script>

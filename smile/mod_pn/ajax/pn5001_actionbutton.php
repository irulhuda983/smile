  ...
  <div id="actmenu">
  	<div id="actbutton">
        ...
        ?>        
        <div style="float:left;"><div class="icon">
        	<a id="btn_close" href="http://<?=$HTTP_HOST;?>/mod_pn/form/pn5001.php?mid=<?=$mid;?>"><img src="http://<?=$HTTP_HOST;?>/images/file_cancel.gif" align="absmiddle" border="0"> Close</a> 
        </div></div>
        <?PHP
      } 
  		else 
    ...
    //cek apakah kelengkapan administrasi yg mandatory sudah diserahkan semua --
      ...
      echo "</script>";		
		}else
		{
          ...
          //hitung manfaat pasti -----------------------------------------------
          $qry = "BEGIN SIJSTK.P_PN_PN5001.X_GEN_MNF_PASTI ('$ls_kode_klaim', '$username',:p_sukses,:p_mess);END;";											 	
          $proc = $DB->parse($qry);				
          oci_bind_by_name($proc, ":p_sukses", $p_sukses,32);
					oci_bind_by_name($proc, ":p_mess", $p_mess,1000);
          $DB->execute();				

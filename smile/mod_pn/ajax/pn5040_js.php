  ...
  <script type="text/javascript" src="../../javascript/jquery.dataTables.min.js"></script>
    ...
    { 
      ...
      var v_kode_tipe_klaim_tki = window.document.getElementById('kode_tipe_klaim').value;
			
			if (v_kode_segmen =="TKI" && v_kode_tipe_klaim_tki != 'JHT01')
			{
    ...
    {     			
			...
			var v_kode_sebab_klaim = window.document.getElementById('kode_sebab_klaim').value;
			...
			if (v_jenis_klaim == "JKK" || v_jenis_klaim == "JKM" || v_jenis_klaim == "JHM" || v_flag_meninggal == "Y")
      {
        ...
        if (v_kode_sebab_klaim=="SKK02")
        {
         	window.document.getElementById('span_label_tgl_kejadian').innerHTML = 'Tgl Diagnosis Klinis *';
        }else
        {
        	window.document.getElementById('span_label_tgl_kejadian').innerHTML = 'Tgl Kejadian *';	 
        }
    ...
    function fl_js_cek_tgl_f1a_tgl_kecelakaan(){
      var tgl_kejadian = document.getElementById("tgl_kejadian").value; 
      var tgl_f1a = document.getElementById("tgl_f1a").value;  
      if(tgl_kejadian == tgl_f1a){
        alert('Kecelakaan terjadi pada saat peserta baru didaftarkan, Silakan mengecek Tanggal dan Jam perekaman F1A pada CIF !!!');
      }
    }
      ...
      var v_err_mess, v_count_err;
			...
			if ($('#kode_kanal_pelayanan').val() == '')
			{							
			 	v_err_mess = 'Kanal pelayanan kosong, harap lengkapi data input..!!!';
				v_count_err = '1';	
			...
			else if ($('#kode_segmen').val() == 'PU' && $('#kpj').val() != '' && ( $('#kode_tk').val() == '' || $('#kode_perusahaan').val() == '' ))
			{

			 	v_err_mess = 'Sebelum melakukan save, harap melakukan tab setelah mengisi no. referensi..!!!';
				v_count_err = '1';																
			} 
			//--------------------------
      ...
      /*End Penambahan cek required sistem antrian*/
		...
		function fl_js_cek_required_edit()
		{
			...
			else if ($('#kode_kanal_pelayanan').val() == '')
			{							
			 	v_err_mess = 'Kanal pelayanan kosong, harap lengkapi data input..!!!';
				v_count_err = '1';	
			}
      ...
      if(form.kode_kanal_pelayanan.value==""){
        alert('Kanal Pelayanan kosong, harap lengkapi data input...!!!');
        form.kode_kanal_pelayanan.focus();
      }else if(form.kode_kantor.value==""){
      ...
      {
       form.cek_status_klaim.value = form.status_klaim.value;
      ...
      }else if(form.kode_tipe_klaim.value=='JKK01' && form.kode_kesimpulan_kasus.value=="KK00" && form.status_klaim.value == "AGENDA_TAHAP_II"){
        alert('Proses sumbit agenda tahap II belum bisa dilakukan karena belum ada kesimpulan kasus, silakan melakukan proses tersebut lewat menu PN5074 - Kesimpulan Kasus KK/PAK.');
        form.jkk2_kode_kondisi_terakhir.focus();											      
      } else
      {
       form.cek_status_klaim.value = form.status_klaim.value;
  ...
  var curr_tgl_kondisi_terakhir =<?php echo ($ld_tgl_kondisi_terakhir=='') ? 'false' : "'".$ld_tgl_kondisi_terakhir."'"; ?>;
  ...
  function f_ajax_val_kpj()
  {
    ...
    var c_tgl_kejadian 		= window.document.getElementById('tgl_kejadian').value;
		...
		var c_kpj_old 				= window.document.getElementById('kpj_old').value;

    //if ((c_kpj != curr_kpj) || (c_kode_tk != curr_kode_tk)) {
		if ((c_kpj != c_kpj_old && c_kpj!="")) {
    ...
    f_js_val_nik_antrian(nomor_identitas);
  }    
  ...
  {		 			
		...
		NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5040_lov_kpj.php?p=pn5040.php&a=formreg&b=kode_tk&c=kpj&d=nama_tk&e=kode_perusahaan&f=nama_perusahaan&g=kode_divisi&h=nama_divisi&j='+c_kode_segmen+'&k=npp&l=nomor_identitas&m=jenis_identitas&n=kode_kantor_tk&q='+c_kode_tipe_klaim+'&r='+c_tgl_kejadian+'&s='+c_kode_sebab_klaim+'&pilihsearch=sc_kpj&searchtxt='+c_kpj+'','',1000,500,1);		 
  }
  ...
  {		 			
		var c_kode_segmen = window.document.getElementById('kode_segmen').value;
		var c_kpj 				= window.document.getElementById('kpj').value;
		NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5040_lov_kpj2care.php?p=pn5040.php&a=formreg&b=kode_tk&c=kpj&d=nama_tk&e=kode_perusahaan&f=nama_perusahaan&g=kode_divisi&h=nama_divisi&j='+c_kode_segmen+'&k=npp&l=nomor_identitas&m=jenis_identitas&n=kode_kantor_tk&q='+c_kpj+'','',1000,500,1);		 
  }
  ...
  function f_ajax_val_kode_tipe_klaim()
  {
    ...
    if (c_kode_tipe_klaim != curr_kode_tipe_klaim) {
      ajax.requestFile = '../ajax/pn5040_validasi.php?getClientId=f_ajax_val_kode_tipe_klaim&c_kode_tipe_klaim='+c_kode_tipe_klaim+'&c_kode_segmen='+c_kode_segmen+'&c_kode_sebab_klaim='+c_kode_sebab_klaim+'&c_kode_tk='+c_kode_tk+'&c_kode_klaim='+c_kode_klaim+'&c_tgl_kejadian='+c_tgl_kejadian;
      ajax.onCompletion = showClientData; // Specify function that will be executed after file has been found	
      ajax.runAJAX();	// Execute AJAX function
    ...
    f_js_get_info_token(token_antrian);
  }             

  function fl_js_get_lov_by_kode_tipe_klaim()
  {		 			
		NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5040_lov_tipeklaim.php?p=pn5040.php&a=formreg&b=kode_tipe_klaim&c=nama_tipe_klaim','',800,500,1);		 
  }
  ...
  function f_ajax_val_kode_sebab_klaim()
  {
    ...
    if (c_kode_sebab_klaim != curr_kode_sebab_klaim) {
      ajax.requestFile = '../ajax/pn5040_validasi.php?getClientId=f_ajax_val_kode_sebab_klaim&c_kode_tipe_klaim='+c_kode_tipe_klaim+'&c_kode_segmen='+c_kode_segmen+'&c_tgl_kejadian='+c_tgl_kejadian+'&c_kode_sebab_klaim='+c_kode_sebab_klaim+'&c_kode_tk='+c_kode_tk+'&c_kode_klaim='+c_kode_klaim;
      ajax.onCompletion = showClientData; // Specify function that will be executed after file has been found	
      ajax.runAJAX();	// Execute AJAX function
  ...
  {		 			
    ...
    var c_kode_segmen = window.document.getElementById('kode_segmen').value;
		...
		NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5040_lov_sebabklaim.php?p=pn5040.php&a=formreg&b=kode_sebab_klaim&c=nama_sebab_klaim&d='+c_kode_tipe_klaim+'&e='+c_kode_segmen+'','',800,500,1);		 
  }
  ...
  //tgl kondisi akhir
  //validasi tgl kondisi akhir agenda jkk tahap 2 tidak boleh lebih kecil dari tanggal kejadian --------------------------------------
  function f_ajax_val_tgl_kondisi_akhir_jkk2()
  {   
		var c_tgl_kondisi_terkahir = window.document.getElementById('jkk2_tgl_kondisi_terakhir').value; 
		var c_tgl_kejadian = window.document.getElementById('tgl_kejadian').value;   
		
    if ((c_tgl_kondisi_terkahir != curr_tgl_kondisi_terakhir) && (c_tgl_kondisi_terkahir!='')) { 
      ajax.requestFile = '../ajax/pn5040_validasi.php?getClientId=f_ajax_val_tgl_kondisi_akhir_jkk2&c_tgl_kondisi_terkahir='+c_tgl_kondisi_terkahir+'&c_tgl_kejadian='+c_tgl_kejadian;
      ajax.onCompletion = showClientData; // Specify function that will be executed after file has been found	
      ajax.runAJAX();	// Execute AJAX function
      curr_tgl_kondisi_terakhir = c_tgl_kondisi_terkahir;
    }		
  }
  ...
  function f_ajax_val_tgl_lapor()
  {
    ...
    if ((c_tgl_lapor != curr_tgl_lapor) && (c_tgl_lapor!='')) {
      ajax.requestFile = '../ajax/pn5040_validasi.php?getClientId=f_ajax_val_tgl_lapor&c_tgl_lapor='+c_tgl_lapor+'&c_tgl_kejadian='+c_tgl_kejadian+'&c_tgl_klaim='+c_tgl_klaim+'&c_kode_tk='+c_kode_tk+'&c_kode_tipe_klaim='+c_kode_tipe_klaim;
      ajax.onCompletion = showClientData; // Specify function that will be executed after file has been found	
      ajax.runAJAX();	// Execute AJAX function
  ...
  function f_ajax_val_tglkejadian()
  {
    ...
    ajax.requestFile = '../ajax/pn5040_validasi.php?getClientId=f_ajax_val_tgl_kejadian&c_kode_perusahaan='+c_kode_perusahaan+'&c_kode_segmen='+c_kode_segmen+'&c_kode_divisi='+c_kode_divisi+'&c_kode_tk='+c_kode_tk+'&c_tgl_kejadian='+c_tgl_kejadian+'&c_jenis_klaim='+c_jenis_klaim+'&c_tgl_lapor='+c_tgl_lapor+'&c_tgl_klaim='+c_tgl_klaim;
    ajax.onCompletion = showClientData; // Specify function that will be executed after file has been found	
    ajax.runAJAX();	// Execute AJAX function
    curr_tgl_kejadian = c_tgl_kejadian;
  }
    ...
    if ((c_jhtinput_kode_tipe_penerima == "AW") && (c_jhtinput_kode_hubungan != curr_jhtinput_kode_hubungan)) {
      ajax.requestFile = '../ajax/pn5040_validasi.php?getClientId=f_ajax_jhtinput_val_kode_hubungan&c_kode_tk='+c_kode_tk+'&c_jhtinput_kode_hubungan='+c_jhtinput_kode_hubungan;
      ajax.onCompletion = showClientData; // Specify function that will be executed after file has been found	
      ajax.runAJAX();	// Execute AJAX function

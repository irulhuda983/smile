...
$php_file_name="pn5041";                                                                        
$mid = $_REQUEST["mid"]; 
$dataid= isset($_GET['dataid'])?$_GET['dataid']:'';
$ls_kode_klaim = $_GET['kode_klaim'];

$sql = "select kode_tipe_klaim from pn.pn_klaim where kode_klaim = '$ls_kode_klaim' ";
$DB->parse($sql);
$DB->execute();		
$row = $DB->nextrow();
$tipe_klaim_upload = $row['KODE_TIPE_KLAIM'];					    
...
function encrypt_decrypt($action, $string)
	...
	if($tipe_klaim_upload=='JKP01'){
		$json_spec = array(
			"bucketName" => "siapkerja",

<?php
$pagetype = "report";
$gs_pagetitle = "Dashboard Pelayanan - JHT Siap Bayar Per Kantor Wilayah/Cabang";
...
//get data from WS -----------------------------------------------------------
global $wsIp;
$ipDev  = $wsIp."/JSPN9010/GetDataJHTGraph07";
$url    = $ipDev;
$chId   = 'CORE';
...
$sql_perkanwil = "select 
							 	 	  kolom_01 kd_prg, 
                    kolom_02 kode_kantor, 
                    kolom_03 nama_kantor, 
                    replace(kolom_03, 'GROUP KONSOLIDASI KANWIL','') nama_kantor_singkat,
										replace(kolom_03, 'GROUP KONSOLIDASI','') nama_kantor_singkat2,
										to_number(kolom_04) jml_kasus, 
                    to_number(kolom_05) nom_siap_bayar 
                  from table
                  (
                    PN.P_PN_REPORT.F_PN9002(
                      'DSH_JHTSIAPBYR_BYKTR', 
                      '$ls_kode_kantor', 
                      '$ls_kode_segmen', 
...
for($i=0;$i<ExtendedFunction::count($resultArray->JHT_GRAPH1);$i++) 
{
  //grafik bar nominal ---------------------------------------------------------
	$ln_graph1_bar_val = 0;
  $ln_graph1_bar_val = $resultArray->JHT_GRAPH1[$i]->NOM_SIAP_BAYAR == "" ? "0" : $resultArray->JHT_GRAPH1[$i]->NOM_SIAP_BAYAR;
  ...
  $ln_graph_pie_val_x3 = $resultArray->JHT_GRAPH1[$i]->NOM_SIAP_BAYAR;
...
?>
<script type="text/javascript">
	var chart1_bar; // grafik 1 - bar - nominal jht siap bayar -------------------
  $(document).ready(
  	function() 
  	{
	...
	var chart2_bar; // grafik 1 - bar - jml kasus jht siap bayar -----------------
  $(document).ready(
  	function() 
  	{
          ...
          <div class="row">
              ...
              <div class="card-box">
    						<h4 class="header-title m-t-0 m-b-30">
  									...
  									{
  									 	?>
  											JHT Siap Bayar Per Kantor Wilayah
  										<? 
  									}else
  									{
  									 	?>
  											JHT Siap Bayar Per Kantor Cabang/KCP
  										<? 
  									}
  									?>	
										</b>
          ...
          <div class="row">
              ...
              <div class="card-box">
    						<h4 class="header-title m-t-0 m-b-30">
  									...
  									{
  									 	?>
  											Kasus JHT Siap Bayar Per Kantor Wilayah
  										<? 
  									}else
  									{
  									 	?>
  											Kasus JHT Siap Bayar Per Kantor Cabang/KCP
  										<? 
  									}
  									?>	
										</b>
              ...
              <div class="card-box">
    						<h4 class="header-title m-t-0 m-b-30">
									<b>
  									...
  									{
  									 	?>
  											Komposisi JHT Siap Bayar Kantor Wilayah
  										<? 
  									}else
  									{
  									 	?>
  											Komposisi JHT Siap Bayar Kantor Cabang/KCP
  										<? 
  									}
  									?>	
									</b>
          ...
          <div class="card-box">
              ...
              <div class="col-12">
                  <div class="form-group m-b-20">
						...
						<div class="row">	 					
							...
							<div class="table-responsive">
								<table class="table table-hover m-0 mails table-actions-bar">
									<thead>
                    <tr>
                      <th colspan="3" style="text-align:center">JHT SIAP BAYAR</th>
                    </tr>
										<tr>
                    ...
                    $i=0;		
										$ln_tot_kasus_jht  =0;
										$ln_tot_jht_siap_bayar =0;				
										for($i=0;$i<ExtendedFunction::count($resultArray->JHT_GRAPH1);$i++)
                    {
                      ?>
											<tr>							
                        <td style="text-align:left">
  												<a href="#" onClick="NewWindow4('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn9010_jht_graph07.php?task=view&kode_kantor=<?=$resultArray->JHT_GRAPH1[$i]->KODE_KANTOR;?>&kode_segmen=<?=$ls_kode_segmen;?>&tgl=<?=$ld_tgl;?>&tgl_mmddyyyy=<?=$ld_tgl_mmddyyyy;?>&sender=pn9010.php&sender_mid=<?=$mid;?>','Drilldown',1250,680,'yes');" href="javascript:void(0);">
                        ...
                        <td style="text-align:right;"><?=number_format((float)$resultArray->JHT_GRAPH1[$i]->NOM_SIAP_BAYAR,2,".",",");?></td>						
                      </tr>
                      <?							    							
                      //$i++;//iterasi i
											$ln_tot_kasus_jht  += (float)$resultArray->JHT_GRAPH1[$i]->JML_KASUS;
											$ln_tot_jht_siap_bayar += (float)$resultArray->JHT_GRAPH1[$i]->NOM_SIAP_BAYAR;
                    }	//end while
										
                    if ($i == 0) {
                      echo '<tr bgcolor="'.($i%2 ? "#f3f3f3" : "#ffffff").'" style="height: 26px;">';
                      echo '<td colspan="3" style="text-align:center">-- Tidak Ada JHT Siap Bayar --</td>';
                      echo '</tr>';
                    }	
                    ?>		 
									</tbody>
                    ...
                    <th  style="text-align:right"><?=number_format((float)$ln_tot_jht_siap_bayar,2,".",",");?></th>
                  </tr>		 
								</table>	 
							</div>	
						</div><!-- end row -->

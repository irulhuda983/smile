...
<div id="formKiri" style="width:960px;">
  <fieldset><legend><b><i><font color="#009999">Informasi Kecelakaan pada Laporan JKK Tahap I </font></i></b></legend>
    ...
    <label style = "text-align:right;">Jam Kecelakaan &nbsp; *</label>		 	    				
      ...
      <? 
				$sql = "select kode, keterangan from sijstk.ms_lookup where tipe=:p_tipe and kode=:p_kode order by seq";       				
        $param_bv = [':p_tipe' => 'KLMJAMKERJ',':p_kode' => $ls_jkk1_kode_jam_kecelakaan];
        $proc = $DB->parse($sql);
        foreach ($param_bv as $key => $value) {
         oci_bind_by_name($proc, $key, $param_bv[$key]);
        }       
    ...
    <label style = "text-align:right;">Jenis Kasus &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; *</label>		 	    				
      ...
      <?
        ...
        $proc = $DB->parse($sql);
        oci_bind_by_name($proc, ':p_kode_jenis_kasus', $ls_jkk1_kode_jenis_kasus);
        $DB->execute();
        while($row = $DB->nextrow())
        {
          echo "<option ";
      ...
      <label style = "text-align:right;">Lokasi Kecelakaan &nbsp; *</label>		 	    				
        ...
        <? 
          ...
          $proc = $DB->parse($sql);
          oci_bind_by_name($proc, ':p_kode_lokasi_kecelakaan', $ls_jkk1_kode_lokasi_kecelakaan);
          $DB->execute();
          while($row = $DB->nextrow())
          {
            echo "<option ";

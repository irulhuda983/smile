<?php 
$leaf_name="pn6001_jpn_terlambat_setupbunga";
$php_file_name="pn6001_jpn_terlambat_setupbunga";
...
if($_REQUEST['kd_agenda']!='')
{
	...
	/*
	$sql = "BEGIN PN.P_PN_PN60010206.X_UPDATE_KOREKSI_KLAIM_JPN
				(
				'$ls_kode_agenda',
				'$USER',
				:p_sukses,
  ...
  // Koreksi Pembayaran JPN / Selisih Pembayaran JPN
	...
	//update 12/09/2019, menambahkan informasi penetapa ulang --------------------
	$sql = "select kode_klaim, to_char(tgl_penetapan,'dd/mm/yyyy') tgl_penetapan, status_klaim from pn.pn_klaim ".
         "where kode_pointer_asal = 'KOR' ".
         "and id_pointer_asal = '{$_REQUEST['kd_agenda']}' ".
         "and rownum = 1 ";		
  $DB->parse($sql);
  $DB->execute();
  $row = $DB->nextrow();
  $ls_kode_klaim_tapulang    = $row["KODE_KLAIM"];
	$ls_status_klaim_tapulang  = $row["STATUS_KLAIM"];	
	$ls_tgl_penetapan_tapulang  = $row["TGL_PENETAPAN"];
...
<fieldset ><legend>Tindaklanjut JP Kurang Bayar Karena Keterlambatan Set Up Bunga</legend>
...
<input type="hidden" style="width: 290px;" value="" <?=$readonly;?> id="nom_tingkat_pengembangan_kor" name="nom_tingkat_pengembangan_kor" readonly />
    ...
    <fieldset><legend>Pemilihan Klaim</legend>
        ...
        <div class="f_2" style="width: 430px !Important">
            ...
            <a href="#" onclick="NewWindow('pn6001_jpn_terlambat_setupbunga_lov.php','pn6001_jpn_terlambat_setupbunga',1070,505,1)">
    ...
    <fieldset><legend>Pengambilan JP Awal</legend>
        ...
        <div class="f_1" style="clear: left;color:#009999;">Saldo JP &nbsp;:</div>
    ...
    <div class="f_2"><input type="text" style="background-color:  #F5F5F5; width: 290px;text-align: right;" readonly value="<?=$ls_tingkat_pengembangan;?>"  id="tingkat_pengembangan"/></div>
        ...
        <div class="f_1" style="clear: left;color:#009999;">Iuran JP &nbsp;:</div>
  ...
  <fieldset><legend>Pengambilan JP Seharusnya</legend>
        <div class="f_1">Tipe Penerima &nbsp;:</div>
    ...
    <fieldset><legend>Koreksi Pembayaran JP / Selisih Pembayaran JP</legend>
        <div class="f_1">&nbsp; </div>
        <div class="f_2"">&nbsp;</div>
	...
	?>
		...
		<?
		//------------- pembayaran --------------------------------------------
		include "../ajax/pn6001_jpn_terlambat_setupbunga_pembayaran_grid.php";
		?>
		</div>
	<?php 
	}
	...
	<?
		if ($ls_jml_submit_nokriteria>0)
		{
		?>
		<fieldset style="width:950px;"><legend>Informasi Data Penetapan Ulang Klaim</legend>
        <div class="f_1">Kode Klaim &nbsp;:</div>
				<div class="f_2"><input type="text" style="background-color: #F5F5F5; width: 290px;" value="<?=$ls_kode_klaim_tapulang;?>" readonly id="kode_klaim_tapulang" name="kode_klaim_tapulang" /></div>
        <div class="f_1" style="clear: left">Tgl Penetapan &nbsp;:</div>
				<div class="f_2"><input type="text" style="background-color: #F5F5F5; width: 290px;" value="<?=$ls_tgl_penetapan_tapulang;?>" readonly id="tgl_penetapan_tapulang" name="tgl_penetapan_tapulang" /></div>
        <div class="f_1" style="clear: left">Status Klaim &nbsp;:</div>
        <div class="f_2"><input type="text" style="background-color:  #F5F5F5; width: 250px;" readonly value="<?=$ls_status_klaim_tapulang;?>" id="status_klaim_tapulang" name="status_klaim_tapulang" /></div>
    </fieldset>
		<?
		}
	?>
	...
	<fieldset style="background: #F2F2F2; width: 1024px"><legend style="background: #F2F2F2; border: 1px solid #CCC;">KETERANGAN</legend>
		...
		<li>Klik tombol CETAK PENETAPAN KOREKSI untuk cetak penatapan koreksi JP Kurang Bayar karena keterlambatan set up bunga</li>
		<li>Klik tombol BATAL untuk batal koreksi JP Kurang Bayar karena keterlambatan set up bunga</li>
	</fieldset>
	<br>
	<?php
	}
...
$(document).ready(function(){ 
	...
	//window.onload = updateDataKoreksi();
	...
	$("#btn_cetak_penetapan_koreksi_jht_kurang_bayar").click(function() {
		NewWindow('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn6001_jpn_terlambat_setupbunga_cetak.php?task=Cetak&kd_prg=1&kode_agenda='+$('#kd_agenda').val()+'&mid='+Math.random(),'Cetak Penetapan Agenda Koreksi',810,550,1);
		
	});
	
	function updateDataKoreksi()
	{
		alert("xxx");
	}
});

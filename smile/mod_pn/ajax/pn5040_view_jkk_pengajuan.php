...
{ 
  ...
  }else if (v_kode_jenis_kasus =="KS02") //penyakit akibat kerja ---------------
  {
    ...
    window.document.getElementById("tapjkk1_kode_tindakan_bahaya").value = "";
		...
		<div id="formKiri" style="width:940px;">
            ...
            <fieldset style="height:210px;"><legend><b><i><font color="#009999">Informasi Pengajuan Klaim JKK Tahap I &nbsp;<?=$ls_nama_tk;?></font></i></b></legend>
              ...
              <span id="span_kecelakaan_kerja" style="display:none;">						
                ...
                <label style = "text-align:right;">Tindakan Bahaya &nbsp;</label>		 	    				
                  ...
                  <? 
									$sql = "select kode, keterangan from sijstk.ms_lookup where tipe='KLMTINDBHY' and kode = :P_TAPJKK1_KODE_TINDAKAN_BAHAYA";
                  $proc = $DB->parse($sql);
                  oci_bind_by_name($proc, ':P_TAPJKK1_KODE_TINDAKAN_BAHAYA', $ls_tapjkk1_kode_tindakan_bahaya);
                  $DB->execute();
                ...
                <label style = "text-align:right;">Kondisi Bahaya &nbsp;</label>		 	    				
                  ...
                  <option value="">-- pilih --</option>
                  <? 
                  $sql = "select kode, keterangan from sijstk.ms_lookup where tipe='KLMKONDBHY' and kode = :P_TAPJKK1_KODE_KONDISI_BAHAYA";
                  $proc = $DB->parse($sql);
                  oci_bind_by_name($proc, ':P_TAPJKK1_KODE_KONDISI_BAHAYA', $ls_tapjkk1_kode_kondisi_bahaya);
                  $DB->execute();
                  while($row = $DB->nextrow())
                  {
                  echo "<option ";
            ...
            ?>
                  ...
                  <fieldset style="height:100px;" ><legend><b><i><font color="#009999">Tempat Tenaga Kerja Dirawat</font></i></b></legend>
                    ...
                    <label style = "text-align:right;">Tempat Perawatan &nbsp;</label>		 	    				
                      ...
                      <option value="">-- pilih --</option>
                      <? 
                      $sql = "select kode, keterangan from sijstk.ms_lookup where tipe='KLMTMPTRWT' and kode = :P_TAPJKK1_KODE_TEMPAT_PERAWATAN ";
											$proc = $DB->parse($sql);
                      oci_bind_by_name($proc, ':P_TAPJKK1_KODE_TEMPAT_PERAWATAN',$ls_tapjkk1_kode_tempat_perawatan);
                      $DB->execute();
                      while($row = $DB->nextrow())
                      {
                      echo "<option ";
                    ...
                    <label style = "text-align:right;">Berobat Jalan &nbsp;</label>		 	    				
                      ...
                      $proc = $DB->parse($sql);
                      oci_bind_by_name($proc, ':P_TAPJKK1_KODE_BEROBAT_JALAN', $ls_tapjkk1_kode_berobat_jalan);
                      $DB->execute();
                      while($row = $DB->nextrow())
                      {
                      echo "<option ";
                  ...
                  <span id="tapjkk1_span_upah_tk" style="display:none;">
                    <fieldset style="height:80px;"><legend><b><i><font color="#009999">Upah yang Diterima oleh Tenaga Kerja</font></i></b></legend>								 										 
                      ...
                      <label style = "text-align:right;">Tipe Upah &nbsp;</label>		 	    				
                        ...
                        <option value="">-- pilih --</option>
                        <? 
                        $sql = "select kode, keterangan from sijstk.ms_lookup where tipe='TPUPA'||'$ls_kode_segmen' and kode= :P_TAPJKK1_KODE_TIPE_UPAH ";
												$proc = $DB->parse($sql);
                        oci_bind_by_name($proc, ':P_TAPJKK1_KODE_TIPE_UPAH', $ls_tapjkk1_kode_tipe_upah);
                        $DB->execute();
                        while($row = $DB->nextrow())
                        {
                        echo "<option ";
          ...
          <td valign="top" colspan="2">
            ...
            <fieldset style="width:930px;"><legend><b><i><font color="#009999">Diagnosa</font></i></b></legend>						 
                ...
                <tbody>
                  ...
                  if ($ls_kode_klaim!="")
                  {
                    ...
                    while ($row = $DB->nextrow())
                    {
                      ...
                      <td>
                      	<?=$row['NAMA_TENAGA_MEDIS'];?> 
                      </td>					
                      <td>
                        <?=$row['KODE_DIAGNOSA_DETIL'];?> - <?=$row['NAMA_DIAGNOSA_DETIL'];?>        			 
                      </td>
                      <td>
												<?=$row['KETERANGAN'];?>	
											</td> 										       																			        																			

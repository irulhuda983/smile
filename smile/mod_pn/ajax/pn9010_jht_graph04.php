...
<?
//1. grafik pembayaran klaim jht berdasarkan usia ------------------------------
//get data from WS -----------------------------------------------------------
global $wsIp;
$ipDev  = $wsIp."/JSPN9010/GetDataJHTGraph04";
$url    = $ipDev;
$chId   = 'CORE';
...
$ls_graph_pie_val = "";
for($i=0;$i<ExtendedFunction::count($resultArray->JHT_GRAPH04);$i++) 
{
  //grafik bar -----------------------------------------------------------------
	$ln_graph1_bar_val = 0;
  $ln_graph1_bar_val = $resultArray->JHT_GRAPH04[$i]->NOM_PEMBAYARAN == "" ? "0" : $resultArray->JHT_GRAPH04[$i]->NOM_PEMBAYARAN;
  $ln_graph1_bar_val_tot = $ln_graph1_bar_val_tot + (float)$ln_graph1_bar_val;
  $ls_graph1_bar_xAxisCategory1_temp .= "'".$resultArray->JHT_GRAPH04[$i]->KET_KELOMPOK_USIA."',";
  $ls_graph1_bar_val_temp .= $ln_graph1_bar_val.",";
	
	//grafik pie -----------------------------------------------------------------
	$ls_graph_pie_val_x1 = $resultArray->JHT_GRAPH04[$i]->KODE_KELOMPOK_USIA;
  $ls_graph_pie_val_x2 = $resultArray->JHT_GRAPH04[$i]->KET_KELOMPOK_USIA;
  $ln_graph_pie_val_x3 = $resultArray->JHT_GRAPH04[$i]->NOM_PEMBAYARAN;
	...
	<div class="content">
		<div class="container">
          ...
          <div class="row">
    				<div class="col-lg-12">
              <div class="card-box">
    						<h4 class="header-title m-t-0 m-b-30"><b>Pembayaran Klaim JHT Berdasarkan Usia</b></h4>
          ...
          <div class="row">
    				<div class="col-lg-12">
              <div class="card-box">
    						<h4 class="header-title m-t-0 m-b-30"><b>Komposisi Pembayaran Klaim JHT Berdasarkan Usia</b></h4> 
                <div class="widget-chart text-center">
              ...
              <div class="col-12">
                  <div class="form-group m-b-20">
										...
										for($i=0;$i<ExtendedFunction::count($resultArray->JHT_GRAPH04);$i++)
                    {
                      ?>
											<tr>							
                        <td style="text-align:center"><?=$resultArray->JHT_GRAPH04[$i]->KET_KELOMPOK_USIA;?></td>
                        <td style="text-align:right"><?=number_format((float)$resultArray->JHT_GRAPH04[$i]->JML_KASUS,0,".",",");?></td>  									
                        <td style="text-align:right;"><?=number_format((float)$resultArray->JHT_GRAPH04[$i]->NOM_PEMBAYARAN,2,".",",");?></td>						
                      </tr>
                      <?							    							
                      //$i++;//iterasi i
											$ln_tot_kasus_jht  += (float)$resultArray->JHT_GRAPH04[$i]->JML_KASUS;
											$ln_tot_pembayaran_jht += (float)$resultArray->JHT_GRAPH04[$i]->NOM_PEMBAYARAN;
                    }	//end while

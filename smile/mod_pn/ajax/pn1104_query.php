...
if($TYPE=='queryJenis'){
    ...
    */
    ...
    if($end==0)  $end=100;
    $sql = "";
    $sql = "select * from (select rownum NO_URUT ,KODE_JENIS,NAMA_JENIS,STATUS_NONAKTIF from sijstk.tc_kode_jenis )
            where NO_URUT between {$start} and {$end} {$order}"; //echo $sql;
    $recordsTotal=0;
    $DB->parse($sql);
    if($DB->execute()){ 
        $i = 0;
    ...
    */
    $recordsTotal=0;
    $sql = "select count(*) JML from sijstk.tc_kode_jenis_detil "; //echo $sql;
    $DB->parse($sql);
    if($DB->execute()) 
        if($row = $DB->nextrow())
    ...
    if($order[0]['column']=='0'){
        $order = "ORDER BY KODE_JENIS_DETIL ASC ";
    }else if($order[0]['column']=='1'){
        $order = "ORDER BY KODE_JENIS_DETIL ASC ";
    }else if($order[0]['column']=='2'){
        $order = "ORDER BY NAMA_JENIS_DETIL ASC ";
    }else if($order[0]['column']=='3'){
        $order = "ORDER BY NAMA_JENIS ASC ";
    }else if($order[0]['column']=='4'){
        $order = "ORDER BY STATUS_NONAKTIF ASC ";
    } else $order = "order by A.KODE_JENIS,KODE_JENIS_DETIL ASC";
    ...
    $sql = "";
    $sql = "select * from
            (select rownum NO_URUT ,A.KODE_JENIS_DETIL,A.NAMA_JENIS_DETIL,B.NAMA_JENIS,A.STATUS_NONAKTIF
             from sijstk.tc_kode_jenis_detil A LEFT OUTER JOIN sijstk.TC_KODE_JENIS B
             on A.KODE_JENIS=B.KODE_JENIS {$order}
            )

...
{	
	...
	if ($ls_st_lap_penetapan == "Y")
	{
		...
		if ($ls_flag_berkala == "Y")
		{
          ...
          $qry = "BEGIN SIJSTK.P_PN_PN5040.X_INSERT_AKTIVITAS('$ls_kode_klaim', 'CETAK', '$ls_ket_submit', '$username',:p_sukses,:p_mess);END;";											 	
          $proc = $DB->parse($qry);				
		...
		if ($ls_flag_lumpsum == "Y")
		{
  		...
  		{   
    		...
    		}else if ($ls_jenis_klaim=="JKM")
			{
				...
				$sql_jkp = "SELECT A.BULAN_MANFAAT_KE FROM PN.PN_KLAIM_MANFAAT_DETIL_PHKJKP A WHERE A.KODE_KLAIM = '$ls_kode_klaim'";
				$DB->parse($sql_jkp);
				$DB->execute();
				$row_jkp = $DB->nextrow();	
				$ls_bulan_manfaat_ke = $row_jkp['BULAN_MANFAAT_KE'];
            	...
            	$ls_error        = "0";
            	$ls_ket_submit = "PENCETAKAN PENETAPAN KLAIM JKP PADA SAAT PROSES PENETAPAN KLAIM";
          	}

<?
/* ============================================================================
Ket : Form ini digunakan untuk tab Input jkk Klaim JKK TKI PHK Bukan Akibat Kecelakaan Kerja
      (tanpa melalui pengajuan dan jkk tahap 2, langsung ke penetapan)
Hist: - 11/03/2023 : Pembuatan Form (Tim SIJSTK)								 						 
-----------------------------------------------------------------------------*/
...
<div id="formKiri" style="width:870px;">
  <fieldset><legend>Informasi PHK Bukan Akibat Kecelakaan Kerja</legend>
		</br>												
    <div class="form-row_kiri">
    <label style = "text-align:right;">Penyebab PHK *</label>		 	    				
      ...
      <? 
        $param_bv=[];
  			...
  			{
					$sql = "select kode, keterangan from sijstk.ms_lookup where tipe='TKIPHKBUKANKK_PK' and kode= :P_JKK_KET_TAMBAHAN  order by seq";
          $param_bv[':P_JKK_KET_TAMBAHAN'] =$ls_jkk_ket_tambahan;
        }else
  			{
          $sql = "select kode, keterangan from sijstk.ms_lookup where tipe='TKIPHKBUKANKK_PK' and nvl(aktif,'T')='Y' order by seq";										
  			}        				
        $proc = $DB->parse($sql);

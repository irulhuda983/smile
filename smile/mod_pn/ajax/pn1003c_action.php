...
$ls_nama_lokasi_kecelakaan	= $_POST['v_nama_lokasi_kecelakaan'];
...
if($ls_type=="new"){
	$sql = "insert into sijstk.pn_kode_lokasi_kecelakaan (
				kode_lokasi_kecelakaan,
				nama_lokasi_kecelakaan,
                no_urut,
                status_nonaktif,
                tgl_nonaktif,
                keterangan,
	...
	$sql = "update sijstk.pn_kode_lokasi_kecelakaan
			   set nama_lokasi_kecelakaan = '$ls_nama_lokasi_kecelakaan',
			       no_urut = '$ls_no_urut',
			 ...
			 where kode_lokasi_kecelakaan = '$ls_kode_lokasi_kecelakaan'";
	$DB->parse($sql);
	if($DB->execute()){ 
	...
	$sql="select kode_lokasi_kecelakaan,
		       nama_lokasi_kecelakaan,
		       no_urut,
		       status_nonaktif,
		       to_char(tgl_nonaktif,'dd/mm/yyyy') tgl_nonaktif,
		       keterangan
		  from sijstk.pn_kode_lokasi_kecelakaan
		 where kode_lokasi_kecelakaan='".$ls_search_keyword."'";
    $DB->parse($sql);
    if($DB->execute()){ 
	...
	$sql="delete from sijstk.pn_kode_lokasi_kecelakaan
      		where kode_lokasi_kecelakaan ='".$ls_kode_lokasi_kecelakaan."'";
    $DB->parse($sql);
	if($DB->execute()){ 
	...
	if($order[0]['column']=='0'){
	    ...
	    $order = "ORDER BY NAMA_LOKASI_KECELAKAAN ";
	...
	$sql = "";
	$sql="select * from (select rownum no, kode_lokasi_kecelakaan,
		       nama_lokasi_kecelakaan,
		       status_nonaktif,
		       tgl_nonaktif,
		       keterangan
	...
	$queryTotal = "SELECT COUNT (1) FROM SIJSTK.PN_KODE_LOKASI_KECELAKAAN WHERE 1=1  ".$condition;
	// print_r($sql);
	// print_r($querySql);
	...
	if($DB->execute()){ 
		...
		{
			$data['ACTION'] = '<button class="btn green" name="view_lokasi_kecelakaan_'.$data['KODE_LOKASI_KECELAKAAN'].'" id="view_sebab_klaim_'.$data['KODE_LOKASI_KECELAKAAN'].'" kode_lokasi_kecelakaan="'.$data['KODE_LOKASI_KECELAKAAN'].'"  title="Klik untuk view detil data"><i class="fa fa-search" aria-hidden="true"></i></button>'.' '.' <button class="btn green" name="edit_lokasi_kecelakaan_'.$data['KODE_LOKASI_KECELAKAAN'].'" id="edit_lokasi_kecelakaan_'.$data['KODE_LOKASI_KECELAKAAN'].'" kode_lokasi_kecelakaan="'.$data['KODE_LOKASI_KECELAKAAN'].'"  title="Klik untuk edit data"><i class="fa fa-pencil" aria-hidden="true"></i></button> <button class="btn hapus" name="hapus_lokasi_kecelakaan_'.$data['KODE_LOKASI_KECELAKAAN'].'" kode_lokasi_kecelakaan="'.$data['KODE_LOKASI_KECELAKAAN'].'" title="Klik untuk hapus data"><i class="fa fa-trash" aria-hidden="true"></i></button>
				';

<?php
//------------------------------------------------------------------------------
// Menu untuk display rincian manfaat uraian cacat
		...
		<form name="formreg" id="formreg" role="form" method="post" enctype="multipart/form-data">
          ...
          <fieldset ><legend>Rincian Manfaat</legend>
            ...
            <label style = "text-align:right;">Keadaan *</label>
              <input type="text" id="cacat_nama_keadaan" name="cacat_nama_keadaan" readonly class="disabled" style="width:270px;">
          		<input type="hidden" id="cacat_kode_keadaan" name="cacat_kode_keadaan">																 								
            </div>
            <div class="clear"></div>										
  										
            <div class="form-row_kiri">
            <label style = "text-align:right;">Uraian Cacat *</label>
              <textarea cols="255" rows="1" style="width:270px;height:40px;background-color:#f5f5f5;" id="nama_manfaat_detil" name="nama_manfaat_detil" onkeypress="return canAddCharacter(this, 300)" onchange="trimLength(this, 300)"></textarea>
							<input type="hidden" id="kode_manfaat_detil" name="kode_manfaat_detil">																 								
            </div>
            <div class="clear"></div>
              ...
              <label  style = "text-align:right;">% Dokter </label>
                <input type="text" id="cacat_persen_dokter" name="cacat_persen_dokter" readonly class="disabled" style="width:270px;">				
              </div>																		
              <div class="clear"></div>
  					</span>
            ...
            <label  style = "text-align:right;">% Table </label>
              <input type="text" id="cacat_persen_table" name="cacat_persen_table" readonly class="disabled" style="width:250px;">				
            </div>																		
            <div class="clear"></div>
            ...
            <label  style = "text-align:right;">Total Disetujui </label>
              <input type="text" id="nom_biaya_disetujui" name="nom_biaya_disetujui" readonly class="disabled" style="width:250px;">			
            	<input type="hidden" id="nom_biaya_diajukan" name="nom_biaya_diajukan">
						</div>																		
            <div class="clear"></div>																
			...
			success: function(data){
				try {
					...
					{ 
						...
						$('#cacat_nama_keadaan').val(getValue(jdata.data.DATA['CACAT_NAMA_KEADAAN']));
						$('#cacat_kode_keadaan').val(getValue(jdata.data.DATA['CACAT_KODE_KEADAAN']));
						
						$('#nama_manfaat_detil').val(getValue(jdata.data.DATA['NAMA_MANFAAT_DETIL']));
						$('#kode_manfaat_detil').val(getValue(jdata.data.DATA['KODE_MANFAAT_DETIL']));
						
						$('#cacat_persen_dokter').val(format_uang(getValue(jdata.data.DATA['CACAT_PERSEN_DOKTER'])));
						$('#cacat_persen_table').val(format_uang(getValue(jdata.data.DATA['CACAT_PERSEN_TABLE'])));

						$('#nom_biaya_disetujui').val(format_uang(getValue(jdata.data.DATA['NOM_BIAYA_DISETUJUI'])));
						$('#nom_biaya_diajukan').val(format_uang(getValue(jdata.data.DATA['NOM_BIAYA_DIAJUKAN'])));
						
						$('#keterangan').val(getValue(jdata.data.DATA['KETERANGAN']));	
						//end set value rincian manfaat ------------------------------------

						fl_js_set_persen_dokter();
...
<script language="javascript">
  function fl_js_set_persen_dokter() 
  { 
    var	v_kode_keadaan = $('#cacat_kode_keadaan').val();

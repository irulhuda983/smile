...
function entry_aktivitas_klaim($kode_klaim, $ket_submit, $petugas) {
  ...
  $qry = "
  begin 
    sijstk.p_pn_pn5040.x_insert_aktivitas(:p_kode_klaim, :p_1, :p_ket_submit, :p_petugas,:p_sukses,:p_mess);
  end;";	
  $proc = $DB->parse($qry);				
  $p_1 = 'CETAK';
  oci_bind_by_name($proc, ":p_kode_klaim", $kode_klaim, 32);
  oci_bind_by_name($proc, ":p_1", $p_1, 32);
  oci_bind_by_name($proc, ":p_ket_submit", $ket_submit, 32);
  oci_bind_by_name($proc, ":p_petugas", $petugas, 32);
...
function GetUrlPathDokSigned ($ls_kode_klaim, $ls_kode_jenis_dokumen,$ls_kode_dokumen) {
  //$path_http = $wsIpStorage;//"http://wstest.bpjsketenagakerjaan.go.id:2014";
  ...
  where   exists
          ...
          where   a.kode_klaim = b.id_dokumen
                  and kode_jenis_dokumen = :p_kode_jenis_dokumen
  ...
  $proc = $DB->parse($sql);
  $param_bv = [
    ':p_kode_jenis_dokumen' => $ls_kode_jenis_dokumen,
    ':p_kode_dokumen' => $ls_kode_dokumen,
    ':p_kode_klaim' => $ls_kode_klaim,
    ':p_tipe' => 'KANALKLM',
    ':p_kategori' => 'DOKUMEN_DIGITAL'  
  ];
  ...
  if ($ls_jml_arsip > 0) {
    ...
    from    pn.pn_arsip_dokumen b
    where   b. kode_jenis_dokumen = :p_kode_jenis_dokumen
            and kode_dokumen = :p_kode_dokumen
            and b.id_dokumen = :p_kode_klaim ";
    $proc = $DB->parse($sql);
    $param_bv = [
     ':p_kode_jenis_dokumen' => $ls_kode_jenis_dokumen,
     ':p_kode_dokumen' => $ls_kode_dokumen,
...
if ($ls_kode_klaim != "") {
  ...
  select  status_klaim, 
          ...
          when kanal_pelayanan in (
            select kode from ms.ms_lookup where 1=1 and tipe = :p_tipe and kategori = :p_kategori
          )
  ...
  select  kanal_pelayanan, 
          ...
          where   x.kode_klaim = a.kode_klaim
                  ...
                  and nvl(y.flag_berkala,'T')=:p_flag_berkala
          ...
          where   x.kode_klaim = a.kode_klaim
                  and x.kode_manfaat = y.kode_manfaat
                  and nvl(y.flag_berkala,'T')=:p_flag_berkala2
                  and nvl(x.nom_biaya_disetujui,0)<>0
          ) cnt_lumpsum
  from 		pn.pn_klaim a
  where 	kode_klaim = :p_kode_klaim ";
  $proc = $DB->parse($sql_lumsum_berkala);
  $param_bv = [
   ':p_flag_berkala' => 'Y',
   ':p_flag_berkala2' => 'T',
   ':p_kode_klaim' => $ls_kode_klaim
...
if (isset($_POST["btncetak"])) {
  ...
  if ($ls_st_history_saldo_tk == "Y") {
    $sql = "select kode_perusahaan,kode_segmen, kode_tk, to_char(tgl_klaim,'YYYY') tahun_saldo from sijstk.pn_klaim where kode_klaim = :p_kode_klaim";
    $proc = $DB->parse($sql);
  ...
  <div id="formframe" style="width:300px;">
    ...
    <div id="formKiri" style="width:300px;">
      ...
      <?php
        ...
        where   a.id_dokumen = :p_kode_klaim ";
        $proc = $DB->parse($sql);
        oci_bind_by_name($proc, ':p_kode_klaim', $ls_kode_klaim);
        $DB->execute();
        $row = $DB->nextrow();
        ...
        where   kode_kanal_pelayanan = :p_kanal_pelayanan
                ...
                and status_nonaktif = :p_status_nonaktif
                and status_lumpsum = :p_flag_lumpsum
                and keterangan in (
                  'Dokumen Kelengkapan Persyaratan',
                  'Dokumen F7',
                  'Tanda Terima Dokumen Kelengkapan Administrasi',
                  'Surat Pernyataan Klaim JP Berkala',
                  'Histori Iuran JP',
                  'Rincian Iuran JP',
                  'Tanda Terima Dokumen Agenda JKK Tahap I',
                  'Tanda Terima Dokumen Agenda JKK Tahap II',
                  'Tanda Terima Dokumen Agenda JKM',
                  'Tanda Terima Dokumen Agenda JHT/JKM',
                  'Tanda Terima Dokumen Agenda JHT',
                  'Surat Pernyataan NPWP',
                  'Histori Saldo Tenaga Kerja',
                  'Rincian Saldo Tenaga Kerja',
                  'Penetapan Klaim',
                  'Dokumen F5',
                  'Dokumen F4'
                )
        ...
        $proc = $DB->parse($sql);
        $param_bv = [
          ':p_kanal_pelayanan' => $ls_kanal_pelayanan,
          ':p_kode_tipe_klaim' => $ls_kode_tipe_klaim,
          ':p_status_nonaktif' => 'T',
          ':p_flag_lumpsum' => $ls_flag_lumpsum
        ];
        foreach ($param_bv as $key => $value) {
          oci_bind_by_name($proc, $key, $param_bv[$key]);
        }
      ...
      <legend>
        <?= $ls_error == 1 ? "<legend><font color=#ff0000>Error</font></legend>" : "<font color=#007bb7>Message</font>"; ?>
      </legend>

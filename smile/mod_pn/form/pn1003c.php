...
<div id="formKiri"> 
	<fieldset id="fieldset_form_lokasi_kecelakaan" style="display: none"><legend>Form Lokasi Kecelakaan </legend>
		<form name="form_lokasi_kecelakaan" id="form_lokasi_kecelakaan" role="form" method="post">
			<div class="clear"></div>
			<div class="form-row_kiri">
				<label>Kode Lokasi Kecelakaan</label>
				<input type="text" id="v_kode_lokasi_kecelakaan" name="v_kode_lokasi_kecelakaan" size="30" maxlength="100" tabindex="3">
				<input type="hidden" name="type">
			</div>
			<div class="clear"></div>
			<div class="form-row_kiri">
				<label>Nama Lokasi Kecelakaan</label>
				<input type="text" id="v_nama_lokasi_kecelakaan" name="v_nama_lokasi_kecelakaan" size="30" maxlength="100" tabindex="3">
			</div>
			<div class="clear"></div>
			<div class="form-row_kiri">
				<label>No Urut</label>
		...
		<input type="button" class="btn green" id="btn_simpan_lokasi_kecelakaan" name="btn_simpan_lokasi_kecelakaan" value="SIMPAN" title="Klik Untuk Simpan data">
	</fieldset>

	<!--Fieldset Table Sebab Klaim -->
	<fieldset id="fieldset_lokasi_kecelakaan" style="display: block"><legend>Data Lokasi Kecelakaan</legend>
		<div class="form-row_kiri">
			<button class="btn green" id="btn_tambah_lokasi_kecelakaan" name="btn_tambah_lokasi_kecelakaan"  title="Klik Untuk Menambahkan data"><i class="fa fa-plus" aria-hidden="true"></i> TAMBAH</button>
		</div>
		<div class="form-row_kanan">
			...
			<select name="f_pilihan" id="f_pilihan">
				...
				<option value="nama_lokasi_kecelakaan">Nama Lokasi Kecelakaan</option>
		...
		<table class="table table-striped table-bordered row-border hover" id="table_lokasi_kecelakaan" cellspacing="0" width="100%">
			<thead>
				<tr>
					<th scope="col" width="5%" class="align-left">Kode Lokasi Kecelakaan</th>
					<th scope="col" width="25%" class="align-left">Nama Lokasi Kecelakaan</th>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('input[name*="btn_kembali"]').click(function() {
			...
			$('#fieldset_lokasi_kecelakaan').css("display","block");
		});
		$('#btn_tambah_lokasi_kecelakaan').click(function(){
			resetForm();
			$('input[name*="btn_simpan"]').show();
			$('input[name="type"]').val('new');
			$('fieldset[id*="fieldset_"]').css("display","none");
			$('#fieldset_form_lokasi_kecelakaan').css("display","block");
		});

		//ajax save for sebab klaim
		$('#btn_simpan_lokasi_kecelakaan').click(function() {
			// if(validasiForm()){
				
				preload(true);
				$.ajax({
					type: 'POST',
					url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn1003c_action.php?'+Math.random(),
					data: $('#form_lokasi_kecelakaan').serialize(),
					success: function(data) {
						...
						if(jdata.ret == '0'){
							...
							$('fieldset[id*="fieldset_"').css("display","none");
							$('#fieldset_lokasi_kecelakaan').css("display","");
							window.table_lokasi_kecelakaan.columns.adjust().draw();
						} else {
...
function loadTable(){
	...
	window.table_lokasi_kecelakaan = $('#table_lokasi_kecelakaan').DataTable({
		"scrollCollapse"	: true,
		"paging"			: true,
		...
		"destroy"			: true,
        "ajax"				: {
        	"url"	: "http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn1003c_action.php",
        	"type": "POST",
        	"data" : function(e) { 
        		e.type = 'query';
	...
	// window.table_lokasi_kecelakaan.columns.adjust().draw();
	window.table_lokasi_kecelakaan.on( 'draw.dt', function () {
		$('button[name*="edit_lokasi_kecelakaan"]').click(function() {
			window.kode_lokasi_kecelakaan = $(this).attr('kode_lokasi_kecelakaan');
			getDataRowLokasiKecelakaan('edit',window.kode_lokasi_kecelakaan);
		});
		$('button[name*="view_lokasi_kecelakaan"]').click(function() {
			window.kode_lokasi_kecelakaan = $(this).attr('kode_lokasi_kecelakaan');
			getDataRowLokasiKecelakaan('view',window.kode_lokasi_kecelakaan);
		});
		$('button[name*="hapus_lokasi_kecelakaan"]').click(function() {
			window.kode_lokasi_kecelakaan = $(this).attr('kode_lokasi_kecelakaan');
			deleteDataLokasiKecelakaan(window.kode_lokasi_kecelakaan);
		});
	});
}


function deleteDataLokasiKecelakaan(kode_lokasi_kecelakaan){
	if(confirm("Anda yakin ingin menghapus data ini ? ")){
		...
		$.ajax({
			...
			data: {
				type : "delete",
				v_kode_lokasi_kecelakaan : kode_lokasi_kecelakaan
			},
			success: function(data) {
				preload(false);
				jdata = JSON.parse(data);
...
function getDataRowLokasiKecelakaan(type, kode_lokasi_kecelakaan){
	preload(true);
	window.type = type;
	...
	$.ajax({
		...
		data: { 
				type : "view",
				keyword : kode_lokasi_kecelakaan
		},success: function(data) {
			preload(false);
			// console.log(data);
			jdata = JSON.parse(data);
			...
			if(window.type=='view'){
				...
				$("#form_lokasi_kecelakaan :input").attr('disabled', 'disabled');
				$('#v_kode_lokasi_kecelakaan').css('background', '');
				$('input[name*="btn_simpan"]').hide();
			}else{
				$("#form_lokasi_kecelakaan :input").removeAttr('disabled', 'disabled');
				$('#v_kode_lokasi_kecelakaan').attr('readonly','readonly');
				$('#v_kode_lokasi_kecelakaan').attr('title', 'Field ini adalah kunci jadi tidak bisa diupdate');
				$('#v_kode_lokasi_kecelakaan').css('background', '#dcdada');
				$('input[name*="btn_simpan"]').show();
			}
			$('#v_kode_lokasi_kecelakaan').val(jdata.KODE_LOKASI_KECELAKAAN);
			$('#v_nama_lokasi_kecelakaan').val(jdata.NAMA_LOKASI_KECELAKAAN);
			...
			//aktif form show 
			$('fieldset[id*="fieldset_"]').css("display","none");
			$('#fieldset_form_lokasi_kecelakaan').css("display","");

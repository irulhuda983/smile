                ...
                <table id="tblMnf" width="100%" class="table-data2">
                  <thead>								
                    <tr>
                      ...
                      <th style="text-align:center;" colspan="2" class="hr-single-bottom">PENETAPAN NILAI MANFAAT</th>
                      <th style="text-align:center;" colspan="2"></th>
                    </tr>					
		...
		$.ajax({
			...
			success: function(data){
				try {
					...
					if (jdata.ret == 0) 
					{ 
						$("#span_page_title").html('PN5030 - RINCIAN MANFAAT - BANTUAN PEMULANGAN');
            ...
            { 
              for(var i = 0; i < jdata.data.DATA.length; i++)
              {
                ...
                html_mnf += '</tr>';
								...
								v_tot_d_nom_biaya_diajukan += parseFloat(getValueNumber(jdata.data.DATA[i].NOM_BIAYA_DIAJUKAN),5);
								v_tot_d_nom_biaya_disetujui += parseFloat(getValueNumber(jdata.data.DATA[i].NOM_BIAYA_DISETUJUI),5);
              }
...
<script language="javascript">
	...
	{		
		var c_mid = '<?=$mid;?>';
		showForm('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5069_view_penetapanmanfaat_tkipemulanganrinci.php?kode_klaim='+p_kode_klaim+'&kode_manfaat='+p_kode_manfaat+'&no_urut='+p_no_urut+'&mid='+c_mid+'','',980,550,'yes');
	}
</script>

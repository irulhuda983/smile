<?php
$pagetype = "form";
$gs_pagetitle = "PN5019 - APPROVAL KEGIATAN PROMOTIF/PREVENTIF";
...
<div id="actmenu" >
	<div id="actbutton">
		...
		?>
			...
			<div style="float:left;">
				<div class="icon">
					<a id="btn_close" href="http://<?=$HTTP_HOST;?>/mod_pn/form/pn5019.php?mid=<?=$mid;?>">
    ...
    <div id="formKiri" style="width:1000px">
		<form name="formreg" id="formreg" role="form" method="post">
			<div id="konten">
				...
				<div id="formsplit">
					<fieldset>
						<legend>Data Approval Kegiatan Promotif/Preventif</legend>
						<table>
							<tr>
								<td width="50%" valign="top">
									...
									<div class="form-row_kiri">
										...
										<textarea cols="255" rows="2" id="keterangan_approval_kegiatan" name="keterangan_approval_kegiatan" value="<?=$ls_keterangan_approval_kegiatan;?>" tabindex="8" style="width:225px;" onkeypress="return canAddCharacter(this, 300)" onchange="trimLength(this, 300)" >
										</textarea>   
									</div>
									<div class="clear"></div>
				...
				<div id="formsplit">
					<fieldset>
						<legend>Daftar Penetapan Pelaksana Kegiatan Promotif/Preventif</legend>	
						<div class="clear"></div>
						<table class="table responsive-table" id="mydata" cellspacing="0" width="100%">
							<thead>
								<tr>
									...
									<th scope="col" width="10%">Pelaksana Kegiatan</th>									
								</tr>
							</thead>
							<tbody id="listdata">
							</tbody>
					...
					<fieldset style="background: #ededed;">
						...
						<div class="form-row_kiri">
							<br>
							<input type="button" style="display:none;" class="btn green" id="btncetakdaftarapprovalpengajuankegiatan" name="btncetakdaftarapprovalpengajuankegiatan" tabindex="41" value="CETAK DAFTAR APPROVAL PENGAJUAN KEGIATAN" title="Klik Untuk Cetak Daftar Approval Pengajuan Kegiatan">	
						</div>
					</fieldset>
				...
				<div id="div_penambahantkpengajuan">
					<div id="formsplit">
						<fieldset style="background: #ededed;">
							<br>
							<div class="form-row_kiri">
								<input type="button" name="btnsetujuapprovalkegiatan" id="btnsetujuapprovalkegiatan" class="btn green"  value="PENGAJUAN KEGIATAN DISETUJUI" title="Klik untuk menyetujui pengajuan kegiatan">
								&nbsp;
								<input type="button" name="btntolakapprovalkegiatan" id="btntolakapprovalkegiatan" class="btn green"  value="PENGAJUAN KEGIATAN DITOLAK" title="Klik untuk menolak pengajuan kegiatan">	
							</div>
						</fieldset>
					<br>
					</div>
							...
							<table class="table responsive-table" id="" cellspacing="0" width="100%">
								...
								?>
									<tr>
										...
										{
											$linkhapus ='pn5019.php?task=Edit&tasktk=Edit&dataid='+window.dataid+'&dataid2='+window.kodeperusahaan+'&mid=<?=$mid;?>';
        ...
        <script type="text/javascript">
			$(document).ready(function(){
				...
				$('#btn_view').click(function() {
					if(window.dataid != ''){
						window.location='pn5019.php?task=View&tasktk=View&dataid='+window.dataid+'&dataid2='+window.kodeperusahaan+'&mid=<?=$mid;?>';
					} else {
				...
				$('#btn_edit').click(function() {	
					if(window.dataid != ''){
						window.location='pn5019.php?task=Edit&tasktk=Edit&dataid='+window.dataid+'&dataid2='+window.kodeperusahaan+'&mid=<?=$mid;?>';
					} else {
						alert('Silahkan pilih terlebih dahulu data yang akan di proses!');
					}
				});
				...
				$('#btn_new').click(function() {
					NewWindow4('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5019_eligible.php?kode_user=<?php echo $ls_user_login?>&kode_kantor=<?php echo $ls_kode_kantor?>','pn501901 - Daftar Perusahaan Eligible','1024','600','yes');
				});
				...
				$("#btncetakdaftarapprovalpengajuankegiatan").click(function() {						
				...
				<?PHP
					if($_REQUEST["task"] == "View"){
						...
						$.ajax({
							type: 'POST',
							url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5019_action.php?'+Math.random(),
							...
							success: function(data) {
								...
								if(jdata.ret == '0'){
									...
									if(jdata.data[0].JENIS_PELAKSANA_KEGIATAN == 'PR')
									{
				...
				<?PHP
					if($_REQUEST["task"] == "Edit"){
						...
						$.ajax({
							...
							success: function(data) {
								...
								if(jdata.ret == '0'){
									...
									if(jdata.data[0].STATUS_APPROVAL_PENGAJUAN === null)
									{
										$('#btnsetujuapprovalkegiatan').removeAttr('disabled');
										$('#btnsetujuapprovalkegiatan').removeClass('disabled');
										$('#btntolakapprovalkegiatan').removeAttr('disabled');
										$('#btntolakapprovalkegiatan').removeClass('disabled');
									}
									else
									{
										$('#btnsetujuapprovalkegiatan').attr('disabled','disabled');
										$('#btnsetujuapprovalkegiatan').addClass('disabled');
										$('#btntolakapprovalkegiatan').attr('disabled','disabled');
										$('#btntolakapprovalkegiatan').addClass('disabled');
										
										$('#keterangan_approval_kegiatan').attr('disabled','disabled');
										$('#keterangan_approval_kegiatan').addClass('disabled');
						...
						$('#btn_save').click(function() {
							if($('#keterangan_approval_kegiatan').val() != '' ){
								preload(true);
								$.ajax({
									type: 'POST',
									url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5019_action.php?'+Math.random(),
								...
								alert('Silahkan mengisi keterangan approval pengajuan kegiatan');
							}
							
						});
						
						$('#btnsetujuapprovalkegiatan').click(function() {
							if($('#keterangan_approval_kegiatan').val() != ''){
								approval('DISETUJUI');
							} else {
								alert('Silahkan mengisi keterangan approval pengajuan kegiatan!');
						...
						$('#btntolakapprovalkegiatan').click(function() {
							if($('#keterangan_approval_kegiatan').val() != ''){
								approval('DITOLAK');
							} else {
								alert('Silahkan mengisi keterangan approval pengajuan kegiatan');
			...
			function approval(p_status){
				...
				if (r == true) {
					$.ajax({
						type: 'POST',
						url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5019_action.php?'+Math.random(),
						data: { TYPE:'APPROVE', STATUSAPPROVAL:p_status, DATAID:window.dataid, KETERANGANAPPROVAL:$('#keterangan_approval_kegiatan').val()},
						success: function(data) {
							preload(false);
							jdata = JSON.parse(data);
							if(jdata.ret == '0'){
			...
			function loadData(){
				...
				$.ajax({
					type: 'POST',
					url: 'http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5019_query.php?'+Math.random(),

                  ...
                  <thead>								
                    ...
                    <tr class="hr-double-bottom">
                      ...
                      <th style="text-align:center;">Tipe Penerima</th>
											<th style="text-align:right;">Berat Benda (Kg)</th>
                    ...
                    <tr>
                      <td style="text-align:right;" colspan="2"><i>Total Manfaat:</i>
                        ...
                        <input type="hidden" name="mnf_showmessage" style="border-width: 0;text-align:right" readonly size="5">				
											...
											<td style="text-align:right"><span id="mnf_tot_jml_berat"></span></td>	
		...
		$.ajax({
			...
			success: function(data){
				try {
					...
					if (jdata.ret == 0) 
					{ 
						$("#span_page_title").html('PN5030 - RINCIAN MANFAAT - BANTUAN KERUGIAN ATAS TINDAKAN PIHAK LAIN');
						$("#span_page_title_right").html('KODE KLAIM : '+v_kode_klaim);
						
            //set data manfaat -------------------------------------------------
            var html_mnf = "";	
						var v_tot_d_jml_berat = 0;
            ...
            { 
              for(var i = 0; i < jdata.data.DATA.length; i++)
              {
                ...
                html_mnf += '<td style="text-align: center;">' + getValue(jdata.data.DATA[i].NAMA_TIPE_PENERIMA) + '</td>';
								html_mnf += '<td style="text-align: right;">' + format_uang(getValueNumber(jdata.data.DATA[i].JML_BERAT)) + '</td>';
                ...
                html_mnf += '</tr>';
								...
								v_tot_d_jml_berat += parseFloat(getValueNumber(jdata.data.DATA[i].JML_BERAT),4);
              ...
              $("#data_tblMnf").html(html_mnf);
							...
							$("#mnf_tot_jml_berat").html(format_uang(v_tot_d_jml_berat));					
...
<script language="javascript">
	...
	{		
		var c_mid = '<?=$mid;?>';
		showForm('http://<?=$HTTP_HOST;?>/mod_pn/ajax/pn5069_view_penetapanmanfaat_tkigantikehilanganrinci.php?kode_klaim='+p_kode_klaim+'&kode_manfaat='+p_kode_manfaat+'&no_urut='+p_no_urut+'&mid='+c_mid+'','',980,550,'yes');
	}
</script>

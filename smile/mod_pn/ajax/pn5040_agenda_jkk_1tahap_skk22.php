    ...
    <label style = "text-align:right;">Penyebab Gagal *</label>		 	    				
      ...
      <? 
        $param_bv=[];
  			...
  			{
					$sql = "select kode, keterangan from sijstk.ms_lookup where tipe='TKIGGLTMPT' and kode= :P_JKK_KET_TAMBAHAN order by seq";
          $param_bv[':P_JKK_KET_TAMBAHAN'] = $ls_jkk_ket_tambahan;
        }else
  			{
          $sql = "select kode, keterangan from sijstk.ms_lookup where tipe='TKIGGLTMPT' and nvl(aktif,'T')='Y' order by seq";										
  			}        				
        $proc = $DB->parse($sql);

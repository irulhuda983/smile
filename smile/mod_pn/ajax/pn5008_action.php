...
$KETERANGANPERMINTAAN	= $_POST["KETERANGANPERMINTAAN"];
...
{	
	...
	$DB->parse($sql);
	if($DB->execute()){ 
		echo '{"ret":0,"msg":"Sukses, simpan data permintaan verifikasi perusahaan ke KBP dan Wasrik berhasil!"}';
	} else {
		echo '{"ret":-1,"msg":"Proses gagal, simpan data permintaan verifikasi perusahaan ke KBP dan Wasrik tidak berhasil!"}';
	}
}
...
{	
	$sql = "update sijstk.pn_promotif
			...
			keterangan 				= 'PERMINTAAN VERIFIKASI PERUSAHAAN OLEH KBL ".$ls_user_login."',
			tgl_ubah 				= sysdate,
			petugas_ubah			= '".$ls_user_login."'
			where kode_promotif = '".$DATAID."'";
...
else if(($TYPE =='KIRIMINDIVIDU') && ($DATAID != ''))
{	
	$sql = "update sijstk.pn_promotif
			set status 				= 'PERENCANAAN',
			sub_status 				= 'VERIFIKASI_PERENCANAAN',
			keterangan 				= '".$KETERANGANPERMINTAAN."',
			tgl_permintaan_verifikasi = sysdate,
			tgl_ubah 				= sysdate,
			petugas_ubah			= '".$ls_user_login."'
			where kode_promotif = '".$DATAID."'";

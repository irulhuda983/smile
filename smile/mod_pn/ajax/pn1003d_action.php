...
$ls_nama_akibat_diderita	= $_POST['v_nama_akibat_diderita'];
...
if($ls_type=="new"){
	$sql = "insert into sijstk.pn_kode_akibat_diderita (
				kode_akibat_diderita,
				nama_akibat_diderita,
                no_urut,
                status_nonaktif,
                tgl_nonaktif,
                keterangan,
	...
	$sql = "update sijstk.pn_kode_akibat_diderita
			   set nama_akibat_diderita = '$ls_nama_akibat_diderita',
			       no_urut = '$ls_no_urut',
			 ...
			 where kode_akibat_diderita = '$ls_kode_akibat_diderita'";
	$DB->parse($sql);
	if($DB->execute()){ 
	...
	$sql="select kode_akibat_diderita,
		       nama_akibat_diderita,
		       no_urut,
		       status_nonaktif,
		       to_char(tgl_nonaktif,'dd/mm/yyyy') tgl_nonaktif,
		       keterangan
		  from sijstk.pn_kode_akibat_diderita
		 where kode_akibat_diderita='".$ls_search_keyword."'";
    $DB->parse($sql);
    if($DB->execute()){ 
	...
	$sql="delete from sijstk.pn_kode_akibat_diderita
      		where kode_akibat_diderita ='".$ls_kode_akibat_diderita."'";
    $DB->parse($sql);
	if($DB->execute()){ 
	...
	if($order[0]['column']=='0'){
	    ...
	    $order = "ORDER BY NAMA_AKIBAT_DIDERITA ";
	...
	$sql = "";
	$sql="select * from (select rownum no, kode_akibat_diderita,
		       nama_akibat_diderita,
		       status_nonaktif,
		       tgl_nonaktif,
		       keterangan
	...
	$queryTotal = "SELECT COUNT (1) FROM SIJSTK.PN_KODE_AKIBAT_DIDERITA WHERE 1=1  ".$condition;
	// print_r($sql);
	// print_r($querySql);
	...
	if($DB->execute()){ 
		...
		{
			$data['ACTION'] = '<button class="btn green" name="view_akibat_diderita_'.$data['KODE_AKIBAT_DIDERITA'].'" id="view_akibat_diderita_'.$data['KODE_AKIBAT_DIDERITA'].'" kode_akibat_diderita="'.$data['KODE_AKIBAT_DIDERITA'].'"  title="Klik untuk view detil data"><i class="fa fa-search" aria-hidden="true"></i></button>'.' '.' <button class="btn green" name="edit_akibat_diderita_'.$data['KODE_AKIBAT_DIDERITA'].'" id="edit_akibat_diderita_'.$data['KODE_AKIBAT_DIDERITA'].'" kode_akibat_diderita="'.$data['KODE_AKIBAT_DIDERITA'].'"  title="Klik untuk edit data"><i class="fa fa-pencil" aria-hidden="true"></i></button> <button class="btn hapus" name="hapus_akibat_diderita_'.$data['KODE_AKIBAT_DIDERITA'].'" kode_akibat_diderita="'.$data['KODE_AKIBAT_DIDERITA'].'" title="Klik untuk hapus data"><i class="fa fa-trash" aria-hidden="true"></i></button>
				';

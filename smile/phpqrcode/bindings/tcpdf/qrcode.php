	...
	class QRcode {
		 ...
		 */
		 protected function createBitStream($items) {
			...
			foreach ($items as $key => $item) {
				...
				$bits = ExtendedFunction::count($items[$key]['bstream']);
				$total += $bits;
			}
		 ...
		 */
		 protected function appendPaddingBit($bstream) {
			$bits = ExtendedFunction::count($bstream);
		 ...
		 */
		 protected function appendBitstream($bitstream, $append) {
			if ((!is_array($append)) OR (ExtendedFunction::count($append) == 0)) {
				return $bitstream;
			}
			if (ExtendedFunction::count($bitstream) == 0) {
				return $append;
			}
		 ...
		 */
		 protected function bitstreamToByte($bstream) {
			$size = ExtendedFunction::count($bstream);
			if ($size == 0) {
				return array();
			}

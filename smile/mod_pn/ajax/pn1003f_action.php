...
//form serialize
$ls_kode_diagnosa	= $_POST['v_kode_diagnosa'];
$ls_kode_group_icd		= $_POST['v_kode_group_icd'];
$ls_nama_diagnosa	= $_POST['v_nama_diagnosa'];
...
if($ls_type=="lov_group_icd"){
	$sql="select kode_group_icd, nama_group_icd from sijstk.pn_kode_group_icd";
	// print_r($sql);
	...
	$sql = "insert into sijstk.pn_kode_diagnosa (
				kode_diagnosa,
				nama_diagnosa,
				kode_group_icd,
                no_urut,
	...
	$sql = "update sijstk.pn_kode_diagnosa
			   set nama_diagnosa = '$ls_nama_diagnosa',
			   		kode_group_icd = '$ls_kode_group_icd',
			       no_urut = '$ls_no_urut',
			 ...
			 where kode_diagnosa = '$ls_kode_diagnosa'";
	// print_r($sql);
	$DB->parse($sql);
	if($DB->execute()){ 
	...
	$sql="select kode_diagnosa,
		       ...
		       keterangan
		  from sijstk.pn_kode_diagnosa
		 where kode_diagnosa='".$ls_search_keyword."'";
    $DB->parse($sql);
    if($DB->execute()){ 
	...
	$sql="delete from sijstk.pn_kode_diagnosa
      		where kode_diagnosa ='".$ls_kode_diagnosa."'";
    $DB->parse($sql);
	if($DB->execute()){ 
	...
	if($order[0]['column']=='0'){
	    ...
	    $order = "ORDER BY KODE_DIAGNOSA ";
	}else if($order[0]['column']=='2'){
	    $order = "ORDER BY NAMA_DIAGNOSA ";
	...
	$sql = "";
	$sql="select * from (select rownum no, kode_diagnosa,
		       nama_diagnosa,
		       kode_group_icd,
		       status_nonaktif,
		       tgl_nonaktif,
	...
	$queryTotal = "SELECT COUNT (1) FROM SIJSTK.PN_KODE_DIAGNOSA WHERE 1=1 ".$condition;
	// print_r($sql);
	// print_r($querySql);
	...
	if($DB->execute()){ 
		...
		{
			$data['ACTION'] = '<button class="btn green" name="view_diagnosa_'.$data['KODE_DIAGNOSA'].'" id="view_diagnosa_'.$data['KODE_DIAGNOSA'].'" kode_diagnosa="'.$data['KODE_DIAGNOSA'].'"  title="Klik untuk view detil data"><i class="fa fa-search" aria-hidden="true"></i></button>'.' '.' <button class="btn green" name="edit_diagnosa_'.$data['KODE_DIAGNOSA'].'" id="edit_diagnosa_'.$data['KODE_DIAGNOSA'].'" kode_diagnosa="'.$data['KODE_DIAGNOSA'].'"  title="Klik untuk edit data"><i class="fa fa-pencil" aria-hidden="true"></i></button> <button class="btn hapus" name="hapus_diagnosa_'.$data['KODE_DIAGNOSA'].'" kode_diagnosa="'.$data['KODE_DIAGNOSA'].'" title="Klik untuk hapus data"><i class="fa fa-trash" aria-hidden="true"></i></button> &nbsp <button class="btn green" id="CHECK_'.$data['KODE_DIAGNOSA'].'" name="detil_'.$data['KODE_DIAGNOSA'].'" kode_diagnosa="'.$data['KODE_DIAGNOSA'].'"  title="Pilih untuk Detil Diagnosa"><i class="fa fa-file-text" aria-hidden="true"></i> Detil</button>
				';
